...
static uint8_t g_first_send[4096];
static uint16_t g_first_send_len;
static uint8_t g_last_send[4096];
...
static char g_serial_input[10000];
...
void reader_hal_utest_serial_feed(const char *text) {
  ...
  if ((text == NERO_NFC_NULL) || !nero_nfc_bounded_strlen(text, sizeof(g_serial_input), &len)) {
    return;
  }
  if (len > sizeof(g_serial_input)) {
    len = sizeof(g_serial_input);
  }
  if (!nero_nfc_copy_bytes(g_serial_input, sizeof(g_serial_input), 0u, text, len)) {
    return;
  }
...
bool reader_hal_ccid_send(const uint8_t *buf, uint16_t len, uint32_t deadline_ms) {
  ...
  if (g_send_count == 0u) {
    (void)nero_nfc_copy_bytes(g_first_send, (uint16_t)sizeof(g_first_send), 0u, buf, len);
    g_first_send_len = len;
  }
  (void)nero_nfc_copy_bytes(g_last_send, (uint16_t)sizeof(g_last_send), 0u, buf, len);
  g_last_send_len = len;
  if ((len >= 10u) && ((buf[7] & 0x80u) != 0u)) {
