...
/*
 * reader_ccid_storage.c — PC/SC Part 3 storage-card APDU handlers.
 */
...
enum {
  ISO7816_UPDATE_BINARY_MIN_HDR_LEN = 4u,
  ISO15693_ADDRESS_FLAG = 0x20u,
  ISO15693_HEADER_LEN = 2u,
  ISO15693_UID_LEN = NFC_FRONTEND_ISO15693_UID_LEN,
  ISO15693_SHORT_BLOCK_FIELD_LEN = 1u,
  ISO15693_SHORT_MULTIPLE_BLOCK_FIELD_LEN = 2u,
  ISO15693_EXT_BLOCK_FIELD_LEN = 2u,
  ISO15693_EXT_MULTIPLE_BLOCK_FIELD_LEN = 4u,
  ISO15693_EXT_COUNT_LSB_REL_OFFSET = 2u,
  ISO15693_EXT_COUNT_MSB_REL_OFFSET = 3u,
  ISO7816_SHORT_LE_ZERO_BYTES = 256u,
  TYPE2_RAW_FAST_READ_LAST_PAGE_OFFSET = 2u,
  TYPE2_RAW_READ_SIG_CMD_LEN = 2u,
};
...
static uint16_t reader_ccid_get_data_contactless(uint8_t p1, uint8_t* rsp,
                                                 uint16_t rsp_cap) {
  ...
  switch (p1) {
    case (uint8_t)(NFC_PCSC_GET_DATA_UID):
      if (G_TAG_KIND == READER_TAG_KIND_TYPE5) {
        if (!nero_nfc_copy_bytes(rsp, rsp_cap, 0u, G_UID15,
                                 NFC_FRONTEND_ISO15693_UID_LEN)) {
          return reader_ccid_apdu_failure_response(rsp, rsp_cap);
        }
        return reader_ccid_append_success_status(
            rsp, NFC_FRONTEND_ISO15693_UID_LEN, rsp_cap);
      }
      if (!reader_tags_get_typea_info(&typea) ||
        ...
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
    ...
    case (uint8_t)(NFC_PCSC_GET_DATA_SAK):
      if (!reader_tags_get_typea_info(&typea) ||
          rsp_cap <
              (uint16_t)(NFC_PCSC_GET_DATA_SAK_RSP_LEN + NFC_ISO7816_SW_LEN)) {
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
            (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
      }
      rsp[0] = typea.sak;
      return reader_ccid_append_success_status(
          rsp, NFC_PCSC_GET_DATA_SAK_RSP_LEN, rsp_cap);
    case (uint8_t)(NFC_PCSC_GET_DATA_ATQA):
      if (!reader_tags_get_typea_info(&typea) || !typea.atqa_valid ||
          rsp_cap <
              (uint16_t)(NFC_PCSC_GET_DATA_ATQA_RSP_LEN + NFC_ISO7816_SW_LEN)) {
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
            (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
      }
      ...
      rsp[1] = typea.atqa[1];
      return reader_ccid_append_success_status(
          rsp, NFC_PCSC_GET_DATA_ATQA_RSP_LEN, rsp_cap);
    default:
      return 0u;
  }
}

static uint16_t reader_ccid_get_data_type2(uint8_t p1, uint8_t* rsp,
                                           uint16_t rsp_cap) {
  ...
  switch (p1) {
    case (uint8_t)(NFC_PCSC_GET_DATA_TYPE2_VERSION): {
      ...
      version_len = reader_tags_type2_transceive(
          get_version, (uint16_t)(sizeof(get_version)), version,
      ...
      nfc_tag_type2_apply_version(&type2, version,
                                  (uint8_t)(NFC_TAG_NTAG_VER_REPLY_LEN));
      if (type2.family != NFC_TAG_TYPE2_FAMILY_NTAG21X) {
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
            (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
      }
      if (!nero_nfc_copy_bytes(rsp, rsp_cap, 0u, version,
                               (uint16_t)(NFC_TAG_NTAG_VER_REPLY_LEN))) {
        return reader_ccid_apdu_failure_response(rsp, rsp_cap);
      }
      return reader_ccid_append_success_status(
          rsp, (uint16_t)(NFC_TAG_NTAG_VER_REPLY_LEN), rsp_cap);
    }
    case (uint8_t)(NFC_PCSC_GET_DATA_NTAG_SIGNATURE): {
      const uint8_t read_sig[TYPE2_RAW_READ_SIG_CMD_LEN] = {
          (uint8_t)(CCID_RAW_T2_CMD_READ_SIG), 0x00u};
      ...
      sig_len = reader_tags_type2_transceive(
          read_sig, (uint16_t)(sizeof(read_sig)), signature,
      ...
      if (!nero_nfc_copy_bytes(rsp, rsp_cap, 0u, signature,
                               (uint16_t)(sizeof(signature)))) {
        return reader_ccid_apdu_failure_response(rsp, rsp_cap);
      }
      return reader_ccid_append_success_status(
          rsp, (uint16_t)(sizeof(signature)), rsp_cap);
    }
    case (uint8_t)(NFC_PCSC_GET_DATA_TYPE2_CC):
      ...
      if (!nero_nfc_copy_bytes(rsp, rsp_cap, 0u, type2.cc,
                               (uint16_t)(sizeof(type2.cc)))) {
        return reader_ccid_apdu_failure_response(rsp, rsp_cap);
      }
      return reader_ccid_append_success_status(
          rsp, (uint16_t)(sizeof(type2.cc)), rsp_cap);
    case (uint8_t)(NFC_PCSC_GET_DATA_TYPE2_AUTH0): {
      ...
      if ((G_UID14_LEN == 0u) || (G_UID14[0] != NFC_TAG_MFR_CODE_NXP) ||
          (rsp_cap <
           (uint16_t)(NFC_PCSC_GET_DATA_AUTH0_RSP_LEN + NFC_ISO7816_SW_LEN))) {
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
            (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
      }
      ...
      return reader_ccid_append_success_status(
          rsp, NFC_PCSC_GET_DATA_AUTH0_RSP_LEN, rsp_cap);
    }
    default:
      return 0u;
  }
...
static uint16_t reader_ccid_get_data_type45(uint8_t p1, uint8_t* rsp,
                                            uint16_t rsp_cap) {
  ...
  switch (p1) {
    ...
    case (uint8_t)(NFC_PCSC_GET_DATA_TYPE4_CC): {
      ...
      if (!reader_tags_get_type4_info(&type4) || !type4.cc_valid ||
          type4.cc_len == 0u) {
        return reader_ccid_append_status(
            rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
            (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
...
uint16_t reader_ccid_handle_get_data_apdu(const uint8_t* apdu,
  ...
  if (nero_nfc_u8_at(apdu, (size_t)(apdu_len),
                     (size_t)(NFC_ISO7816_APDU_IDX_P2)) !=
      (uint8_t)(NFC_ISO7816_GET_DATA_P2_DEFAULT)) {
    return reader_ccid_append_status(
        rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
        (uint8_t)(NFC_ISO7816_SW2_FUNC_NOT_SUPPORTED));
  }

  const uint8_t p1 = nero_nfc_u8_at(apdu, (size_t)(apdu_len),
...
uint16_t reader_ccid_handle_escape_transparent_apdu(const uint8_t* apdu,
  ...
  rlen = reader_tags_type5_transceive(body, lc, rsp,
                                      (uint16_t)(rsp_cap - NFC_ISO7816_SW_LEN));
  if (rlen < 0) {
    return reader_ccid_append_status(
        rsp, 0u, rsp_cap, (uint8_t)(NFC_ISO7816_SW1_VERIFICATION_FAILED),
        (uint8_t)(NFC_ISO7816_SW2_SUCCESS));
  }
  return reader_ccid_append_success_status(rsp, (uint16_t)(rlen), rsp_cap);
}

static bool storage_read_binary_fields(const uint8_t* apdu, uint16_t apdu_len,
                                       uint16_t* offset_out, uint8_t* le_out) {
  ...
  if ((G_TAG_KIND == READER_TAG_KIND_TYPE5) &&
    ...
    *offset_out = nero_nfc_u8_at(apdu, (size_t)(apdu_len),
                                 (size_t)(NFC_ISO7816_APDU_IDX_P2));
    ...
    *offset_out =
        (uint16_t)(*offset_out / (uint16_t)(NFC_STORAGE_TYPE5_BLOCK_SIZE));
  }
  return true;
}
...
static bool storage_update_binary_offset(const uint8_t* apdu, uint16_t apdu_len,
  ...
  *offset_out = parsed_offset;
  ...
  if (apdu_len <= NFC_ISO7816_APDU_IDX_P2) {
    return true;
  }
  if ((nero_nfc_u8_at(apdu, (size_t)(apdu_len),
                      (size_t)(NFC_ISO7816_APDU_IDX_P1)) ==
       (uint8_t)(NFC_ISO7816_READ_BINARY_P1_BLOCK_IN_P2)) &&
      nfc_iso7816_apdu_min_len(apdu_len, ISO7816_UPDATE_BINARY_MIN_HDR_LEN)) {
    *offset_out = nero_nfc_u8_at(apdu, (size_t)(apdu_len),
                                 (size_t)(NFC_ISO7816_APDU_IDX_P2));
    return true;
  }
  ...
  *offset_out =
      (uint16_t)(parsed_offset / (uint16_t)(NFC_STORAGE_TYPE5_BLOCK_SIZE));
  return true;
}

static bool storage_type2_read_span_ok(const reader_tag_type2_info_t* type2,
...
static bool storage_type2_update_span_ok(const reader_tag_type2_info_t* type2,
                                         uint16_t page, uint8_t len) {
  if ((type2 == NERO_NFC_NULL) || !type2->cc_valid ||
      !type2->write_access_open) {
    return false;
  }
  return nfc_storage_type2_write_span_ok(type2->data_area_size_bytes, page,
                                         len);
}

static bool storage_type5_read_span_ok(const reader_tag_type5_info_t* type5,
                                       uint16_t block, uint16_t len) {
  uint16_t cc_bytes;
  uint8_t block_size;
  ...
  return nfc_storage_type5_read_span_ok(cc_bytes, type5->data_area_size_bytes,
                                        block_size, type5->block_count, block,
                                        len);
}

static bool storage_type5_update_span_ok(const reader_tag_type5_info_t* type5,
                                         uint16_t block, uint8_t len) {
  uint16_t cc_bytes;
  uint8_t block_size;

  if ((type5 == NERO_NFC_NULL) || !type5->cc_valid ||
      !type5->write_access_open) {
    return false;
  }
  block_size =
      (type5->block_size == 0u) ? ST25_T5_USER_BLOCK_SIZE : type5->block_size;
  cc_bytes = reader_tags_type5_cc_byte_len(type5);
  return nfc_storage_type5_write_span_ok(cc_bytes, type5->data_area_size_bytes,
                                         block_size, type5->block_count, block,
                                         len);
}
...
static bool raw_type5_uid_lsb_matches_active(const uint8_t* uid_lsb) {
  ...
  for (uint8_t i = 0u; i < (uint8_t)(ISO15693_UID_LEN); ++i) {
    if (uid_lsb[i] != G_UID15[ISO15693_UID_LEN - 1u - i]) {
      return false;
    }
  }
  return true;
...
static bool raw_type5_addressed_uid_allowed(const uint8_t* tx, uint16_t tx_len,
                                            uint16_t uid_offset) {
  return (tx != NERO_NFC_NULL) &&
         nero_nfc_span_ok(uid_offset, ISO15693_UID_LEN, tx_len) &&
         raw_type5_uid_lsb_matches_active(tx + uid_offset);
}

static bool raw_type5_fixed_command_allowed(const uint8_t* tx, uint16_t tx_len,
                                            uint16_t base_len) {
  if ((tx == NERO_NFC_NULL) || (tx_len < base_len)) {
    return false;
  }
  if ((nero_nfc_u8_at(tx, (size_t)(tx_len), (size_t)(0)) &
       ISO15693_ADDRESS_FLAG) != 0u) {
    return (tx_len == (uint16_t)(base_len + ISO15693_UID_LEN)) &&
...
static bool raw_type5_block_payload_offset(const uint8_t* tx, uint16_t tx_len,
                                           uint16_t* offset_out) {
  uint16_t offset = ISO15693_HEADER_LEN;

  if (offset_out != NERO_NFC_NULL) {
    *offset_out = 0u;
  }
  if ((tx == NERO_NFC_NULL) || (offset_out == NERO_NFC_NULL) ||
      (tx_len < ISO15693_HEADER_LEN)) {
    return false;
  }
  if ((nero_nfc_u8_at(tx, (size_t)(tx_len), (size_t)(0)) &
       ISO15693_ADDRESS_FLAG) != 0u) {
    ...
    offset = (uint16_t)(offset + ISO15693_UID_LEN);
  }
  *offset_out = offset;
  return true;
}
...
static bool raw_type5_read_span_allowed(uint16_t block, uint16_t byte_len) {
  ...
  if ((byte_len == 0u) || !reader_tags_get_type5_info(&type5) ||
      !type5.cc_valid || !type5.read_access_open) {
    return false;
  }
  return storage_type5_read_span_ok(&type5, block, byte_len);
}

static bool raw_type5_read_command_allowed(const uint8_t* tx, uint16_t tx_len,
                                           bool extended, bool multiple) {
  uint16_t offset = 0u;
  uint16_t block = 0u;
  uint16_t count = 1u;
  ...
  if (extended) {
    const uint16_t field_len =
        multiple ? (uint16_t)(ISO15693_EXT_MULTIPLE_BLOCK_FIELD_LEN)
                 : (uint16_t)(ISO15693_EXT_BLOCK_FIELD_LEN);
    uint8_t field[ISO15693_EXT_MULTIPLE_BLOCK_FIELD_LEN];
    if (!nero_nfc_span_ok(offset, field_len, tx_len) ||
        tx_len != (uint16_t)(offset + field_len) ||
        !nero_nfc_copy_bytes(field, sizeof(field), 0u, tx + offset,
                             field_len)) {
      return false;
    }
    block =
        (uint16_t)((uint16_t)(field[0]) |
                   ((uint16_t)(field[1]) << NFC_ISO7816_U16_HIGH_BYTE_SHIFT));
    if (multiple) {
      const uint16_t count_field =
          (uint16_t)((uint16_t)(field[ISO15693_EXT_COUNT_LSB_REL_OFFSET]) |
                     ((uint16_t)(field[ISO15693_EXT_COUNT_MSB_REL_OFFSET])
    ...
    const uint16_t field_len =
        multiple ? (uint16_t)(ISO15693_SHORT_MULTIPLE_BLOCK_FIELD_LEN)
                 : (uint16_t)(ISO15693_SHORT_BLOCK_FIELD_LEN);
    uint8_t field[ISO15693_SHORT_MULTIPLE_BLOCK_FIELD_LEN];
    if (!nero_nfc_span_ok(offset, field_len, tx_len) ||
        tx_len != (uint16_t)(offset + field_len) ||
        !nero_nfc_copy_bytes(field, sizeof(field), 0u, tx + offset,
                             field_len)) {
  ...
  if (multiple &&
      (count > (uint16_t)(NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX))) {
    return false;
  }
  if (count > (uint16_t)(UINT16_MAX / NFC_STORAGE_TYPE5_BLOCK_SIZE)) {
    return false;
  }
  return raw_type5_read_span_allowed(
      block, (uint16_t)(count * (uint16_t)(NFC_STORAGE_TYPE5_BLOCK_SIZE)));
}

bool reader_ccid_type2_raw_transceive_allowed(const uint8_t* tx,
                                              uint16_t tx_len) {
  reader_tag_type2_info_t type2;

  if ((tx == NERO_NFC_NULL) || (tx_len == 0u) ||
      (G_TAG_KIND != READER_TAG_KIND_TYPE2)) {
    return false;
  }
  switch (nero_nfc_u8_at(tx, (size_t)(tx_len), (size_t)(0))) {
    case CCID_RAW_T2_CMD_READ:
      if ((tx_len != (uint16_t)(NFC_TAG_T2T_READ_CMD_LEN)) ||
          !reader_tags_get_type2_storage_info(&type2) || !type2.cc_valid ||
          !type2.read_access_open) {
        return false;
      }
      return storage_type2_read_span_ok(&type2, tx[1],
                                        NFC_TAG_T2T_READ_RESP_BYTES);
    case CCID_RAW_T2_CMD_FAST_READ: {
      uint16_t len;
      const uint16_t first_page = tx[1];
      const uint16_t last_page = tx[TYPE2_RAW_FAST_READ_LAST_PAGE_OFFSET];
      if ((tx_len != (uint16_t)(NFC_TAG_T2T_FAST_READ_CMD_LEN)) ||
    ...
    case CCID_RAW_T2_CMD_READ_SIG:
      return tx_len == (uint16_t)(TYPE2_RAW_READ_SIG_CMD_LEN);
    default:
      return false;
  }
}

bool reader_ccid_type5_raw_transceive_allowed(const uint8_t* tx,
                                              uint16_t tx_len) {
  if ((tx == NERO_NFC_NULL) || (tx_len < ISO15693_HEADER_LEN) ||
      (G_TAG_KIND != READER_TAG_KIND_TYPE5)) {
    return false;
  }
  switch (nero_nfc_u8_at(tx, (size_t)(tx_len), (size_t)(1))) {
    case CCID_RAW_T5_CMD_INVENTORY:
      return tx_len >= ISO15693_HEADER_LEN;
    ...
    case NFC_TAG_T5T_ISO15693_CMD_GET_SYS_INFO:
      return raw_type5_fixed_command_allowed(tx, tx_len,
                                             (uint16_t)(ISO15693_HEADER_LEN));
    case NFC_TAG_T5T_ISO15693_CMD_EXT_GET_SYS_INFO:
      return raw_type5_fixed_command_allowed(
          tx, tx_len, (uint16_t)(ISO15693_HEADER_LEN + 1u));
    default:
      return false;
  }
}
...
static uint16_t storage_read_binary_remaining_bytes(uint16_t offset) {
  ...
  if (G_TAG_KIND == READER_TAG_KIND_TYPE2) {
    ...
    if (!nero_nfc_try_add_u16((uint16_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
                              type2.data_area_size_bytes, &total)) {
      return 0u;
    }
...
uint16_t reader_ccid_handle_read_binary_apdu(const uint8_t* apdu,
  ...
  if (le == 0u) {
    ...
    req_len =
        (remaining != 0u) ? remaining : (uint16_t)(ISO7816_SHORT_LE_ZERO_BYTES);
    req_len = NERO_NFC_MIN(req_len, (uint16_t)(ISO7816_SHORT_LE_ZERO_BYTES));
  } else {
    req_len = (uint16_t)(le);
  }
  if (!nero_nfc_try_add_size((size_t)(req_len), (size_t)(NFC_ISO7816_SW_LEN),
                             &rsp_need) ||
  ...
  if (G_TAG_KIND == READER_TAG_KIND_TYPE2) {
    ...
    if (!storage_type2_read_span_ok(&type2, offset, req_len)) {
      return reader_ccid_append_status(rsp, 0u, rsp_cap,
                                       (uint8_t)(NFC_ISO7816_SW1_WRONG_P1P2),
                                       (uint8_t)(NFC_ISO7816_SW2_SUCCESS));
    }
...
uint16_t reader_ccid_handle_update_binary_apdu(const uint8_t* apdu,
  ...
  if (G_TAG_KIND == READER_TAG_KIND_TYPE2) {
    ...
    ok = true;
    for (uint8_t off = 0u; off < lc;
         off = (uint8_t)(off + NFC_STORAGE_TYPE2_UNIT_SIZE)) {
      const uint16_t page =
          (uint16_t)(offset + (off / NFC_STORAGE_TYPE2_UNIT_SIZE));
      if (reader_ccid_abort_requested() || (page > NFC_BYTE_VALUE_MAX) ||
          !reader_tags_type2_write_page((uint8_t)(page), data + off)) {
        ok = false;
        break;
      }
    }
    ...
    if ((offset == 0u) && (lc == (uint8_t)(NFC_TAG_T5T_CC_LEN_SHORT))) {
      ...
      if (!new_cc.cc_valid) {
        return reader_ccid_append_status(rsp, 0u, rsp_cap,
                                         (uint8_t)(NFC_ISO7816_SW1_WRONG_DATA),
                                         (uint8_t)(NFC_ISO7816_SW2_SUCCESS));
      }
      ok = !reader_ccid_abort_requested() &&
           reader_tags_type5_write_block(0u, data, lc);
      return reader_ccid_append_status(
          rsp, 0u, rsp_cap,
    ...
    ok = true;
    for (uint8_t off = 0u; off < lc;
         off = (uint8_t)(off + NFC_STORAGE_TYPE5_BLOCK_SIZE)) {
      if (reader_ccid_abort_requested() ||
          !reader_tags_type5_write_block(
              (uint16_t)(offset + (off / NFC_STORAGE_TYPE5_BLOCK_SIZE)),
