...
/*
 * reader_tags_type4.c — ISO-DEP Type 4 tag read path.
 */
...
enum {
  READER_TAGS_TYPE4_MIN_CMD_LEN = NFC_ISO7816_SHORT_APDU_HDR_LEN,
  READER_TAGS_TYPE4_CC_PAYLOAD_MIN =
      (uint16_t)(NFC_PCSC_T4_CC_FILE_LEN + NFC_ISO7816_SW_STATUS_WORD_LEN),
  READER_TAGS_TYPE4_CC_LEN_FIELD_WIDTH = NFC_PCSC_T4_NLEN_LEN,
  READER_TAGS_TYPE4_NDEF_LEN_READ_MIN =
      NFC_ISO7816_MIN_RESPONSE_WITH_STATUS_LEN,
  /* [T4T-ISO14443-4] NDEF file layout: 2-byte NLEN field followed immediately
   * by the NDEF message, so the message data begins one NLEN field (2 bytes)
   * into the file. */
  READER_TAGS_TYPE4_NDEF_DATA_OFFSET = NFC_PCSC_T4_NLEN_LEN,
};

static int reader_tags_type4_iso_read_binary(const uint8_t* cmd,
                                             uint8_t cmd_len, uint8_t* resp,
                                             uint16_t resp_cap) {
  int rlen;
  uint8_t correct_le = 0u;

  if ((cmd == NERO_NFC_NULL) || (cmd_len < READER_TAGS_TYPE4_MIN_CMD_LEN) ||
      (resp == NERO_NFC_NULL) || (resp_cap < NFC_ISO7816_SW_STATUS_WORD_LEN)) {
    return -1;
  }
  ...
  if (nfc_iso7816_response_wrong_length(resp, rlen, &correct_le)) {
    ...
    const uint16_t offset = (uint16_t)(((uint16_t)(cmd[NFC_ISO7816_APDU_IDX_P1])
                                        << NFC_CCID_U32_SHIFT_BYTE1) |
                                       cmd[NFC_ISO7816_APDU_IDX_P2]);

    if (nfc_pcsc_build_t4_read_binary_apdu(
            offset, correct_le, retry, (uint8_t)(sizeof(retry)), &retry_len)) {
...
bool reader_tags_type4_load_info(reader_tag_type4_info_t* info,
  ...
  if (!nfc_pcsc_build_t4_select_file_apdu((uint16_t)(NFC_PCSC_T4_CC_FILE_ID),
                                          ...
                                          &sel_cc_len) ||
      !nfc_pcsc_build_t4_read_binary_apdu(
  ...
  if ((rlen < NFC_ISO7816_SW_LEN) || !nfc_iso7816_response_sw_ok(resp, rlen)) {
    if (log_errors) {
      uint8_t sw1 = 0u;
      uint8_t sw2 = 0u;
  ...
  rlen = reader_tags_type4_iso_read_binary(read_cc, read_cc_len, resp,
                                           sizeof(resp));
  if ((rlen < READER_TAGS_TYPE4_CC_PAYLOAD_MIN) ||
  ...
  if (cc_len > (uint16_t)(NFC_PCSC_T4_CC_FILE_LEN)) {
    ...
    rlen = reader_tags_type4_iso_read_binary(read_cc, read_cc_len, resp,
                                             sizeof(resp));
    if ((rlen < (cc_len + NFC_ISO7816_SW_STATUS_WORD_LEN)) ||
        !nfc_iso7816_response_sw_ok(resp, rlen)) {
      if (log_errors) {
...
bool reader_tags_read_type4_ndef(void) {
  ...
  if (!reader_tags_get_typea_info(&typea) ||
      !reader_tags_type4_load_info(&type4, &ndef_file_hi, &ndef_file_lo,
                                   true)) {
    return false;
  }
  ...
  nfc_tutorial_t4t_select_file(
      nero_nfc_log_putc, (uint8_t)(NFC_PCSC_T4_CC_FILE_ID >> NFC_BYTE_SHIFT_8),
      (uint8_t)(NFC_PCSC_T4_CC_FILE_ID & NFC_BYTE_VALUE_MAX));
  nfc_tutorial_t4t_read_binary(nero_nfc_log_putc, 0u,
                               (uint8_t)(sizeof(type4.cc)));
  nfc_tutorial_t4t_cc(nero_nfc_log_putc, type4.cc, (uint8_t)(sizeof(type4.cc)));
  if (!nfc_pcsc_type4_max_message_size(type4.max_ndef_size,
  ...
  if (!nfc_pcsc_build_t4_select_file_apdu(
          (uint16_t)(((uint16_t)(ndef_file_hi) << NFC_CCID_U32_SHIFT_BYTE1) |
                     ndef_file_lo),
          sel_ndef, (uint8_t)(sizeof(sel_ndef)), &sel_ndef_len) ||
      !nfc_pcsc_build_t4_read_binary_apdu(
          0u, (uint8_t)(NFC_PCSC_T4_NLEN_LEN), read_len_cmd,
  ...
  rlen = reader_tags_type4_iso_read_binary(read_len_cmd, read_len_cmd_len, resp,
                                           sizeof(resp));
  if ((rlen < READER_TAGS_TYPE4_NDEF_LEN_READ_MIN) ||
      !nfc_iso7816_response_sw_ok(resp, rlen)) {
    nero_nfc_log_line("  NDEF length read failed");
    return false;
  }
  ndef_msg_len = (uint16_t)(((uint16_t)(resp[NFC_TAG_CC_CCLEN_MSB_INDEX])
                             << NFC_BYTE_SHIFT_8) |
                            resp[NFC_TAG_CC_CCLEN_LSB_INDEX]);
  ...
  if (ndef_msg_len > READER_NDEF_BUF_MAX) {
    nero_nfc_log_write("  NDEF message too large (max ");
    {
      char ndc[NERO_NFC_FORMAT_SNPRINTF_DEC_CAP];
      (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u",
                              (unsigned)((uint32_t)(READER_NDEF_BUF_MAX)));
      nero_nfc_log_write(ndc);
    }
   ...
   * reader). */
  {
    ...
    uint16_t mle_cap =
        (type4.mle != 0u) ? type4.mle : (uint16_t)(NFC_PCSC_T4_CC_DEFAULT_MLE);
    uint16_t chunk_max =
        (mle_cap < (uint16_t)(NFC_PCSC_T4_READ_BINARY_DATA_MAX))
            ? mle_cap
            : (uint16_t)(NFC_PCSC_T4_READ_BINARY_DATA_MAX);
    while (assembled < to_read) {
      uint16_t offset =
          (uint16_t)(READER_TAGS_TYPE4_NDEF_DATA_OFFSET + assembled);
      uint16_t remaining = (uint16_t)(to_read - assembled);
      uint8_t chunk =
          (remaining > chunk_max) ? (uint8_t)(chunk_max) : (uint8_t)(remaining);
      uint8_t cmd[NFC_PCSC_T4_READ_BINARY_APDU_LEN];
      uint8_t cmd_len = 0u;
      if (!nfc_pcsc_build_t4_read_binary_apdu(
              offset, chunk, cmd, (uint8_t)(sizeof(cmd)), &cmd_len)) {
        return false;
      }
      nfc_tutorial_t4t_read_binary(nero_nfc_log_putc, offset, chunk);
      rlen =
          reader_tags_type4_iso_read_binary(cmd, cmd_len, resp, sizeof(resp));
      if ((rlen < (chunk + NFC_ISO7816_SW_STATUS_WORD_LEN)) ||
          !nfc_iso7816_response_sw_ok(resp, rlen)) {
        nero_nfc_log_line("  NDEF read failed");
        return false;
      }
      if (!nero_nfc_copy_bytes(READER_TAGS_NDEF_BUF,
                               sizeof(READER_TAGS_NDEF_BUF), assembled, resp,
                               chunk)) {
        nero_nfc_log_line("  NDEF read failed");
        return false;
      }
    ...
    nfc_tutorial_ndef_message(nero_nfc_log_putc, READER_TAGS_NDEF_BUF,
                              assembled);
    nero_nfc_log_write("  NDEF message: ");
    {
      char ndc[NERO_NFC_FORMAT_SNPRINTF_DEC_CAP];
      (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u",
                              (unsigned)((uint32_t)(ndef_msg_len)));
      nero_nfc_log_write(ndc);
    }
    nero_nfc_log_line(" bytes");
