...
/* CBOR nest depth cap — RFC 8949; prevents stack exhaustion on hostile CTAP CBOR. */
enum {
...
enum {
  ...
  /* Short-form NDEF URI record: 1-byte prefix code + suffix ≤ 255 payload bytes. */
...
/* Host PC/SC Type 5 storage read cap from block 0; TLV spans are uint16-bounded. */
enum { NERO_NFC_TYPE5_STORAGE_READ_MAX = 65535u };

/* [PCSC-P1] SCardStatus ATR buffer (typical contactless ATR ≤ 33 bytes; PC/SC allows larger). */
enum { NERO_NFC_PCSC_ATR_HOST_MAX = 64u };

/* [PCSC-P1] SCardTransmit response buffer. */
enum { NERO_NFC_PCSC_APDU_RX_MAX = 65536u };

/* [PCSC-P1] API string scratch (SCardStatus reader name, formatted error notes). */
enum {
...
/* NFC Forum RTD URI 1.0 — identifier codes 0x00..0x23 (36 prefix table entries). */
enum { NERO_NFC_URI_PREFIX_CODE_COUNT = 36u };

/* Arduino UNO R4 WiFi SRAM ceiling (32 KiB). Lint/CI link gates use this budget. */
enum {
...
/* CCID extended APDU response scratch for Type 4 relay responses on constrained SRAM. */
