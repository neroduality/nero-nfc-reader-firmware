...
enum {
  READER_ISO_DEP_IRQ_BYTE_MASK = 0xFFu,
  READER_ISO_DEP_IRQ_SHIFT_TIMER = 8u,
  READER_ISO_DEP_IRQ_SHIFT_ERR = 16u,
  READER_ISO_DEP_IRQ_SHIFT_TARGET = 24u,
};

void reader_iso_dep_debug_dump(const char* label, const uint8_t* buf,
                               uint16_t len, uint16_t max_show) {
  ...
  if ((g_iso_dep_trace == 0u) || (label == NERO_NFC_NULL) ||
      (buf == NERO_NFC_NULL)) {
    return;
  }
  if (show > max_show) {
  ...
  if (len > max_show) {
    nero_nfc_log_write(" ...(+");
    {
      ...
      (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u",
                              (unsigned)(uint32_t)((len - max_show)));
      nero_nfc_log_write(ndc);
    }
...
void reader_iso_dep_debug_dump_irq_u32(const char* label, uint32_t irqs) {
  ...
  nero_nfc_log_write(" main=0x");
  {
    ...
    (void)nero_nfc_snprintf(
        nhx, sizeof nhx, "%02X",
        (unsigned)((uint8_t)(irqs & READER_ISO_DEP_IRQ_BYTE_MASK)));
  ...
  nero_nfc_log_write(" timer=0x");
  {
    ...
    (void)nero_nfc_snprintf(
        nhx, sizeof nhx, "%02X",
        (unsigned)((uint8_t)((irqs >> READER_ISO_DEP_IRQ_SHIFT_TIMER) &
                             READER_ISO_DEP_IRQ_BYTE_MASK)));
    nero_nfc_log_write(nhx);
  }
  nero_nfc_log_write(" err=0x");
  {
    ...
    (void)nero_nfc_snprintf(
        nhx, sizeof nhx, "%02X",
        (unsigned)((uint8_t)((irqs >> READER_ISO_DEP_IRQ_SHIFT_ERR) &
                             READER_ISO_DEP_IRQ_BYTE_MASK)));
    nero_nfc_log_write(nhx);
  }
  nero_nfc_log_write(" target=0x");
  {
    ...
    (void)nero_nfc_snprintf(
        nhx, sizeof nhx, "%02X",
        (unsigned)((uint8_t)((irqs >> READER_ISO_DEP_IRQ_SHIFT_TARGET) &
                             READER_ISO_DEP_IRQ_BYTE_MASK)));
    nero_nfc_log_write(nhx);
  }
  nero_nfc_log_write("\r\n");
...
void reader_iso_dep_debug_dump_xcvr_diag(uint16_t tx_len, uint16_t timeout_ms,
                                         int rlen) {
