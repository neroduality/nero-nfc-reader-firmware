...
TEST(ReaderCbor, HasRemainingAndAtEnd) {
  ...
  cbor_reader_t c = Wrap(data, sizeof(data));
...
TEST(ReaderCbor, ReadUintAdditional24) {
  const uint8_t data[] = {0x18u, 0x18u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint32_t v = 0u;
  ASSERT_TRUE(cbor_read_uint(&c, &v));
  EXPECT_EQ(v, 24u);
...
TEST(ReaderCbor, ReadUintAdditional25) {
  const uint8_t data[] = {0x19u, 0x03u, 0xE8u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint32_t v = 0u;
  ASSERT_TRUE(cbor_read_uint(&c, &v));
  EXPECT_EQ(v, 1000u);
...
TEST(ReaderCbor, ReadUintAdditional26) {
  const uint8_t data[] = {0x1Au, 0x00u, 0x01u, 0xE2u, 0x40u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint32_t v = 0u;
  ASSERT_TRUE(cbor_read_uint(&c, &v));
  EXPECT_EQ(v, 123456u);
...
TEST(ReaderCbor, ReadUintRejectsWrongMajor) {
  const uint8_t data[] = {0x41u, 'x'}; /* byte string, not uint */
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint32_t v = 0u;
...
TEST(ReaderCbor, ReadHeadRejectsTruncated) {
  const uint8_t data[] = {0x19u, 0x03u}; /* need 2 bytes after 0x19 */
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t major = 0u;
  uint32_t value = 0u;
...
TEST(ReaderCbor, ReadTextRejectsWhenBufferTooSmallForPayload) {
  const uint8_t data[] = {0x66, 'h', 'e', 'l', 'l', 'o', '!'}; /* text len 6 */
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[4];
  EXPECT_FALSE(cbor_read_text(&c, buf, sizeof(buf)));
  EXPECT_EQ(c.pos, 0u);
}

TEST(ReaderCbor, ReadTextNoTruncationWhenBufferBigEnough) {
  const uint8_t data[] = {0x62u, 'y', 'o'};
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[32];
  ASSERT_TRUE(cbor_read_text(&c, buf, sizeof(buf)));
  EXPECT_STREQ(buf, "yo");
}

TEST(ReaderCbor, BytesRejectsWhenBufferTooSmallForPayload) {
  const uint8_t data[] = {0x42u, 0xAAu, 0xBBu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[1];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes(&c, buf, sizeof(buf), &out_len));
  EXPECT_EQ(c.pos, 0u);
}

TEST(ReaderCbor, BytesIndefiniteHappyPath) {
  const uint8_t data[] = {0x5Fu, 0x42u, 0xAAu, 0xBBu, 0x41u, 0xCCu, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[8];
  uint16_t out_len = 0u;
  ASSERT_TRUE(cbor_read_bytes_indefinite(&c, buf, sizeof(buf), &out_len));
...
TEST(ReaderCbor, BytesFlexibleFallsBackToIndefinite) {
  const uint8_t data[] = {0x5Fu, 0x42u, 0x01u, 0x02u, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  uint16_t out_len = 0u;
  ASSERT_TRUE(cbor_read_bytes_flexible(&c, buf, sizeof(buf), &out_len));
  EXPECT_EQ(out_len, 2u);
}

TEST(ReaderCbor, BytesMaybeTaggedSkipsTag) {
  /* tag(0) + h'0102' === C0 42 01 02 */
  const uint8_t data[] = {0xC0u, 0x42u, 0x01u, 0x02u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[8];
  uint16_t out_len = 0u;
  ASSERT_TRUE(cbor_read_bytes_maybe_tagged(&c, buf, sizeof(buf), &out_len));
  EXPECT_EQ(out_len, 2u);
...
TEST(ReaderCbor, LegacySignatureTailAliasThree) {
  const uint8_t data[] = {0x03u, 0x42u, 0xDEu, 0xADu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[8];
  uint16_t out_len = 0u;
  ASSERT_TRUE(cbor_read_legacy_signature_tail(&c, buf, sizeof(buf), &out_len));
  EXPECT_EQ(out_len, 2u);
...
TEST(ReaderCbor, SkipNestedArray) {
  const uint8_t data[] = {0x82u, 0x01u, 0x02u};
  cbor_reader_t c = Wrap(data, sizeof(data));
...
TEST(ReaderCbor, SkipTaggedValue) {
  const uint8_t data[] = {0xC1u, 0x1Au, 0x00u, 0x00u, 0x00u, 0x2Au};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipMapEntries) {
  const uint8_t data[] = {0xA2u, 0x01u, 0x02u, 0x03u, 0x04u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipIndefiniteByteString) {
  const uint8_t data[] = {0x5Fu, 0x41u, 0xAAu, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipFailsOnTruncatedMap) {
  const uint8_t data[] = {0xA1u, 0x01u}; /* missing value */
  cbor_reader_t c = Wrap(data, sizeof(data));
...
TEST(ReaderCbor, ReadBytesRejectsNullOutLen) {
  ...
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  EXPECT_FALSE(cbor_read_bytes(&c, buf, sizeof(buf), NERO_NFC_NULL));
}
...
TEST(ReaderCbor, ReadBytesRejectsUndersizedBuffer) {
  const uint8_t data[] = {0x43u, 0x01u, 0x02u, 0x03u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[2];
  uint16_t out_len = 99u;
  EXPECT_FALSE(cbor_read_bytes(&c, buf, sizeof(buf), &out_len));
  EXPECT_EQ(c.pos, 0u);
}

TEST(ReaderCbor, ReadTextRejectsUndersizedBuffer) {
  const uint8_t data[] = {0x63u, 'a', 'b', 'c'};
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[3];
  EXPECT_FALSE(cbor_read_text(&c, buf, sizeof(buf)));
  EXPECT_EQ(c.pos, 0u);
...
TEST(ReaderCbor, ReadHeadAi26Truncated) {
  const uint8_t data[] = {0x1Au, 0x00u, 0x00u, 0x00u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t major = 0u;
  uint32_t value = 0u;
  EXPECT_FALSE(cbor_read_head(&c, &major, &value));
...
TEST(ReaderCbor, ReadTextNullOrZeroBuffer) {
  const uint8_t data[] = {0x61u, 'x'};
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[4];
  EXPECT_FALSE(cbor_read_text(&c, NERO_NFC_NULL, sizeof(buf)));
  c = Wrap(data, sizeof(data));
  EXPECT_FALSE(cbor_read_text(&c, buf, 0u));
}

TEST(ReaderCbor, ReadTextWrongMajor) {
  const uint8_t data[] = {0x40u}; /* definite-length byte string, length 0 */
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[4];
  EXPECT_FALSE(cbor_read_text(&c, buf, sizeof(buf)));
  EXPECT_EQ(c.pos, 0u);
...
TEST(ReaderCbor, ReadTextTruncatedPayload) {
  const uint8_t data[] = {0x62u, 'a'}; /* text len 2, one content byte */
  cbor_reader_t c = Wrap(data, sizeof(data));
  char buf[4];
  EXPECT_FALSE(cbor_read_text(&c, buf, sizeof(buf)));
}
...
TEST(ReaderCbor, BytesIndefiniteWrongChunkMajor) {
  const uint8_t data[] = {0x5Fu, 0x63u, 'a',
                          'a',   'a',   0xFFu}; /* text chunk in byte stream */
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[8];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes_indefinite(&c, buf, sizeof(buf), &out_len));
}

TEST(ReaderCbor, BytesIndefiniteTruncatedChunk) {
  const uint8_t data[] = {0x5Fu, 0x42u, 0x00u,
...
TEST(ReaderCbor, BytesIndefiniteMissingTerminator) {
  const uint8_t data[] = {0x5Fu, 0x41u, 0xAAu}; /* no break byte */
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[8];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes_indefinite(&c, buf, sizeof(buf), &out_len));
...
TEST(ReaderCbor, BytesMaybeTaggedUintPayloadFails) {
  const uint8_t data[] = {0x01u, 0x00u}; /* unsigned(1), not bytes */
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes_maybe_tagged(&c, buf, sizeof(buf), &out_len));
}

TEST(ReaderCbor, BytesMaybeTaggedInnerNotBytes) {
  /* Tag + unsigned int: after consuming the tag, flexible cannot read bytes. */
  const uint8_t data[] = {0xC0u, 0x01u, 0x00u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes_maybe_tagged(&c, buf, sizeof(buf), &out_len));
...
TEST(ReaderCbor, BytesMaybeTaggedTruncatedTagFollowUp) {
   ...
   */
  const uint8_t data[] = {0xD9u, 0x01u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_bytes_maybe_tagged(&c, buf, sizeof(buf), &out_len));
...
TEST(ReaderCbor, LegacySignatureTailRejectsWrongAlias) {
  const uint8_t data[] = {0x04u, 0x42u, 0x01u, 0x02u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  uint8_t buf[4];
  uint16_t out_len = 0u;
  EXPECT_FALSE(cbor_read_legacy_signature_tail(&c, buf, sizeof(buf), &out_len));
}
...
TEST(ReaderCbor, SkipDefiniteByteString) {
  const uint8_t data[] = {0x42u, 0x01u, 0x02u};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipIndefiniteTextString) {
   ...
   */
  const uint8_t data[] = {0x7Fu, 0x61u, 0xAAu, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipIndefiniteArray) {
  const uint8_t data[] = {0x9Fu, 0x01u, 0x02u, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipIndefiniteMap) {
  const uint8_t data[] = {0xBFu, 0x01u, 0x02u, 0xFFu};
  cbor_reader_t c = Wrap(data, sizeof(data));
  ASSERT_TRUE(cbor_skip(&c));
  EXPECT_TRUE(cbor_at_end(&c));
}
...
TEST(ReaderCbor, SkipRejectsExcessiveNestDepth) {
  ...
  data.reserve(NERO_NFC_CBOR_MAX_NEST_DEPTH + 2u);
  for (unsigned i = 0u; i <= NERO_NFC_CBOR_MAX_NEST_DEPTH; ++i) {
    data.push_back(0x81u);
  }
