...
void nfc_ccid_u32_store_le(uint8_t *dst, uint32_t value) {
  ...
  dst[NFC_CCID_U32_BYTE2] = (uint8_t)((value >> NFC_CCID_U32_SHIFT_BYTE2) & NFC_CCID_BYTE_MASK);
  dst[NFC_CCID_U32_BYTE3] = (uint8_t)((value >> NFC_CCID_U32_SHIFT_BYTE3) & NFC_CCID_BYTE_MASK);
}

bool nfc_ccid_bulk_frame_validate(const uint8_t *frame, uint16_t nbytes, uint32_t *data_len_out) {
  ...
  if (!nero_nfc_try_add_size((size_t)NFC_CCID_BULK_HEADER_LEN, (size_t)datalen_bytes, &frame_len) ||
...
bool nfc_ccid_xfr_payload_needs_time_extension(const uint8_t *payload, uint16_t payload_len) {
...
bool nfc_ccid_xfr_frame_requests_response_continuation(const uint8_t *frame, uint16_t frame_len) {
  ...
  return frame[0] == NFC_CCID_MSG_PC_TO_RDR_XFR && data_len == 0u &&
         frame[NFC_CCID_BULK_LEVEL_PARAM2_OFFSET] == NFC_CCID_XFR_RESPONSE_CONTINUE &&
