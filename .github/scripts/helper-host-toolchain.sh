...
# Host GCC 15+ for userspace builds (firmware uses arduino-cli).
#
# Policy:
#   - If distro defaults (gcc, g++, cc, c++) already meet the minimum, use them.
#   - Otherwise on apt hosts install gcc-15/g++-15 (PPA on Ubuntu LTS) and symlink
#     all four names under /usr/local/bin so CMake picks GCC 15 instead of /usr/bin/c++.
#
# Sourceable API (install-linux-deps.sh, ci-lint.sh):
#   nero_nfc_host_toolchain_system_ok
#   nero_nfc_host_toolchain_effective_ok
#   nero_nfc_host_toolchain_ok
#   nero_nfc_host_toolchain_ensure_symlinks
#   nero_nfc_host_toolchain_activate
#   nero_nfc_host_toolchain_verify
#
# Subcommands:
#   bash helper-host-toolchain.sh verify
#   bash helper-host-toolchain.sh activate

[[ -n ${NERO_NFC_HOST_TOOLCHAIN_HELPERS_LOADED:-} ]] && return 0
NERO_NFC_HOST_TOOLCHAIN_HELPERS_LOADED=1

NERO_NFC_GXX_MIN_VERSION="${NERO_NFC_GXX_MIN_VERSION:-15.0}"
NERO_NFC_GXX_MIN_MAJOR="${NERO_NFC_GXX_MIN_MAJOR:-15}"
NERO_NFC_HOST_TOOLCHAIN_PREFIX="${NERO_NFC_HOST_TOOLCHAIN_PREFIX:-/usr/local/bin}"
NERO_NFC_HOST_TOOLCHAIN_NAMES=(gcc g++ cc c++)
...
nero_nfc_ht_priv() {
  if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
    ...
    printf 'error: need root or sudo for host toolchain install\n' >&2
    return 1
  fi
}
...
nero_nfc_ht_tools_ok_on_path() {
  local name v
  for name in "${NERO_NFC_HOST_TOOLCHAIN_NAMES[@]}"; do
...
nero_nfc_ht_path_without_prefix() {
  local p="${1:-${PATH:-}}"
  p="${p#"${NERO_NFC_HOST_TOOLCHAIN_PREFIX}:"}"
  printf '%s' "${p}"
}
...
nero_nfc_host_toolchain_effective_ok() {
  ...
  PATH="${NERO_NFC_HOST_TOOLCHAIN_PREFIX}:$(nero_nfc_ht_path_without_prefix "${saved_path}")"
  nero_nfc_ht_tools_ok_on_path
  rc=$?
  PATH="${saved_path}"
  return "${rc}"
...
nero_nfc_host_toolchain_ok() {
  nero_nfc_host_toolchain_system_ok || nero_nfc_host_toolchain_effective_ok
}

nero_nfc_host_toolchain_best_versioned_gxx() {
  local bin ver
...
nero_nfc_host_toolchain_ensure_symlinks() {
  local best_gxx gcc_bin name dest_dir="${NERO_NFC_HOST_TOOLCHAIN_PREFIX}"

  if nero_nfc_host_toolchain_system_ok; then
    return 0
  fi

  best_gxx="$(nero_nfc_host_toolchain_best_versioned_gxx || true)"
  if [[ -z ${best_gxx} ]]; then
...
nero_nfc_host_toolchain_activate() {
  ...
  if nero_nfc_host_toolchain_effective_ok; then
    local stripped
    stripped="$(nero_nfc_ht_path_without_prefix "${PATH:-}")"
    export PATH="${NERO_NFC_HOST_TOOLCHAIN_PREFIX}:${stripped}"
    export CC=gcc
    export CXX=g++
    return 0
  fi
  printf 'error: host toolchain below GCC %s (run INSTALL_DEPS=1 bash make/install-linux-deps.sh)\n' \
...
nero_nfc_host_toolchain_verify() {
  ...
  printf '── Host toolchain smoke (GCC %s+ for userspace; firmware uses arduino-cli) ──\n' \
    "${NERO_NFC_GXX_MIN_MAJOR}"
  for name in "${NERO_NFC_HOST_TOOLCHAIN_NAMES[@]}"; do
    path="$(command -v "${name}")"
    major="$(nero_nfc_ht_version_major "${name}")"
    if [[ ${path} == "${NERO_NFC_HOST_TOOLCHAIN_PREFIX}/"* && -L ${path} ]]; then
  ...
  printf '── Host toolchain: OK ──\n'
}

if [[ ${BASH_SOURCE[0]} == "${0}" ]]; then
  set -euo pipefail
  case "${1:-verify}" in
    verify) nero_nfc_host_toolchain_verify ;;
    activate) nero_nfc_host_toolchain_activate ;;
    *)
