...
static const uint8_t kNfcCtapPkCredTypeKey[] = {0x64u, 0x74u, 0x79u, 0x70u, 0x65u};
static const uint8_t kNfcCtapPkCredTypePublicKey[] = {
  0x6Au, 0x70u, 0x75u, 0x62u, 0x6Cu, 0x69u, 0x63u, 0x2Du, 0x6Bu, 0x65u, 0x79u,
};
static const uint8_t kNfcCtapOptionsUp[] = {0x62u, 0x75u, 0x70u, NFC_CBOR_BOOL_TRUE};
static const uint8_t kNfcCtapOptionsUv[] = {0x62u, 0x75u, 0x76u, NFC_CBOR_BOOL_FALSE};

NERO_NFC_STATIC_ASSERT(sizeof(kNfcCtapOptionsUp) + sizeof(kNfcCtapOptionsUv) ==
                         NFC_CTAP_GET_ASSERTION_OPTIONS_BODY_LEN,
                       "getAssertion options body must match emitted \"up\"/\"uv\" bytes");

static bool emit_pk_cred_descriptor(const uint8_t *cred_id, uint16_t cred_len, uint8_t *buf,
                                    uint16_t buf_max, uint16_t *p_io) {
  uint16_t p;
  uint8_t id_hdr_len;
  size_t required = 0u;

  if ((cred_len == 0u) || (cred_id == NERO_NFC_NULL) || (buf == NERO_NFC_NULL) ||
      (p_io == NERO_NFC_NULL)) {
    return false;
  }
  p = *p_io;
  ...
  if (!nero_nfc_try_add_size((size_t)p, NFC_CTAP_PK_CRED_DESCRIPTOR_EXTRA, &required) ||
  ...
  if (!nero_nfc_copy_bytes(buf, buf_max, p, kNfcCtapPkCredTypeKey,
                           (uint16_t)sizeof(kNfcCtapPkCredTypeKey))) {
    return false;
  }
  p = (uint16_t)(p + (uint16_t)sizeof(kNfcCtapPkCredTypeKey));
  if (!nero_nfc_copy_bytes(buf, buf_max, p, kNfcCtapPkCredTypePublicKey,
                           (uint16_t)sizeof(kNfcCtapPkCredTypePublicKey))) {
    return false;
  }
  p = (uint16_t)(p + (uint16_t)sizeof(kNfcCtapPkCredTypePublicKey));
  if (cred_len <= NFC_CBOR_AI_INLINE_MAX) {
    ...
    buf[p++] = (uint8_t)((cred_len >> NFC_BYTE_SHIFT_8) & NFC_ISO7816_BYTE_MASK);
...
bool nfc_ctap_encode_get_assertion(const uint8_t *client_hash32, const char *rp_id,
                                   const uint8_t *allow_cred, uint16_t allow_cred_len, uint8_t *out,
  ...
  if ((client_hash32 == NERO_NFC_NULL) || (rp_id == NERO_NFC_NULL) || (out == NERO_NFC_NULL) ||
      (out_len == NERO_NFC_NULL)) {
    return false;
  }
  has_allow_credential = (allow_cred != NERO_NFC_NULL) && (allow_cred_len > 0u);
  if (!nero_nfc_bounded_strlen(rp_id, (size_t)NFC_CTAP_RP_ID_MAX + 1u, &rplen) || (rplen == 0u) ||
      (rplen > NFC_CTAP_RP_ID_MAX)) {
    return false;
  }
  required = NFC_CTAP_GET_ASSERTION_MAP_HDR_LEN + NFC_CTAP_GET_ASSERTION_RP_KEY_LEN +
             ((rplen <= NFC_CBOR_AI_INLINE_MAX) ? NFC_CBOR_HDR_INLINE : NFC_CBOR_HDR_UINT8) +
             rplen + NFC_CTAP_GET_ASSERTION_CDH_PREFIX_LEN + NFC_CTAP_CLIENT_HASH_LEN +
             NFC_CTAP_GET_ASSERTION_OPTIONS_PREFIX_LEN + NFC_CTAP_GET_ASSERTION_OPTIONS_BODY_LEN;
  if (has_allow_credential) {
    ...
    if (!nero_nfc_try_add_size(required,
                               NFC_CBOR_HDR_UINT8 + NFC_CTAP_PK_CRED_DESCRIPTOR_HDR_LEN +
  ...
  if (!nero_nfc_copy_bytes(out, out_max, p, client_hash32, NFC_CTAP_CLIENT_HASH_LEN)) {
    return false;
  }
  ...
  if (has_allow_credential) {
    ...
    if (!emit_pk_cred_descriptor(allow_cred, allow_cred_len, out, out_max, &p)) {
      return false;
    }
  }
  ...
  if (!nero_nfc_copy_bytes(out, out_max, p, kNfcCtapOptionsUp,
                           (uint16_t)sizeof(kNfcCtapOptionsUp))) {
    return false;
  }
  p = (uint16_t)(p + (uint16_t)sizeof(kNfcCtapOptionsUp));
  if (!nero_nfc_copy_bytes(out, out_max, p, kNfcCtapOptionsUv,
                           (uint16_t)sizeof(kNfcCtapOptionsUv))) {
    return false;
  }
  p = (uint16_t)(p + (uint16_t)sizeof(kNfcCtapOptionsUv));

  *out_len = p;
  return true;
}

bool nfc_ctap_pack_cbor_apdu(uint8_t ctap_cmd, const uint8_t *cbor, uint16_t cbor_len,
                             bool force_extended, uint8_t *apdu, uint16_t apdu_cap,
                             uint16_t *apdu_len) {
  ...
  if (((cbor == NERO_NFC_NULL) && (cbor_len != 0u)) || (apdu == NERO_NFC_NULL) ||
      (apdu_len == NERO_NFC_NULL)) {
    return false;
  }
  ...
  if (!nero_nfc_try_add_size((size_t)payload_len,
  ...
  apdu[0] = NFC_CTAP_CLA;
  apdu[1] = NFC_CTAP_INS_MSG;
  apdu[NFC_ISO7816_APDU_IDX_P1] = NFC_CTAP_P1_GETRESPONSE_SUPPORTED; /* [CTAP2.3] section 11.3.7.1 —
  ...
  if (!force_extended && (payload_len <= NFC_ISO7816_SHORT_LC_MAX)) {
    ...
    if (!nero_nfc_copy_bytes(apdu, apdu_cap, NFC_CTAP_APDU_SHORT_HDR_LEN, cbor, cbor_len)) {
      return false;
    }
    cmd_len = (uint16_t)(NFC_CTAP_APDU_SHORT_HDR_LEN + cbor_len + NFC_CTAP_APDU_SHORT_TAIL_LEN);
    ...
    apdu[NFC_CTAP_APDU_EXTENDED_LC_LSB_OFFSET] = (uint8_t)(payload_len & NFC_ISO7816_BYTE_MASK);
    apdu[NFC_CTAP_APDU_EXTENDED_CMD_OFFSET] = ctap_cmd;
    if (!nero_nfc_copy_bytes(apdu, apdu_cap, NFC_CTAP_APDU_EXTENDED_HDR_LEN, cbor, cbor_len)) {
      return false;
    }
    cmd_len =
      (uint16_t)(NFC_CTAP_APDU_EXTENDED_HDR_LEN + cbor_len + NFC_CTAP_APDU_EXTENDED_TAIL_LEN);
...
bool nfc_ctap_pack_select_fido_apdu(bool add_le_00, uint8_t *apdu, uint16_t apdu_cap,
                                    uint16_t *apdu_len) {
  ...
  if (!nfc_ctap_fido_app_select_variant_match(NFC_ISO7816_SW2_SUCCESS, add_le_00, &variant)) {
    return false;
  }
  return nfc_pcsc_build_select_aid_apdu(NFC_CTAP_FIDO_AID, (uint8_t)NFC_CTAP_FIDO_AID_LEN,
                                        variant.p2, variant.add_le_00, apdu, apdu_cap, apdu_len);
}

bool nfc_ctap_fido_webauthn_select_step(uint8_t step, nfc_ctap_fido_webauthn_select_step_t *out) {
  static const nfc_ctap_fido_webauthn_select_step_t
    kSteps[NFC_CTAP_FIDO_WEBAUTHN_SELECT_STEP_COUNT] = {
      {1u, NFC_CTAP_FIDO_SELECT_PREP_NONE, NFC_CTAP_FIDO_SELECT_LOG_NONE},
      {1u, NFC_CTAP_FIDO_SELECT_PREP_RECOVER, NFC_CTAP_FIDO_SELECT_LOG_REOPEN},
      {0u, NFC_CTAP_FIDO_SELECT_PREP_SETTLE, NFC_CTAP_FIDO_SELECT_LOG_NONE},
      {1u, NFC_CTAP_FIDO_SELECT_PREP_RECOVER, NFC_CTAP_FIDO_SELECT_LOG_LAST_CHANCE},
    };
  ...
  if ((out == NERO_NFC_NULL) || (step >= (uint8_t)NFC_CTAP_FIDO_WEBAUTHN_SELECT_STEP_COUNT)) {
    return false;
  }
  *out = kSteps[step];
  return true;
...
bool nfc_ctap_apdu_is_select_fido_aid(const uint8_t *apdu, uint16_t apdu_len, bool *add_le_00_out) {
  uint8_t lc;
  const uint8_t *aid;
  ...
  if ((apdu[0] != NFC_ISO7816_CLA_ISO) || (apdu[1] != NFC_ISO7816_INS_SELECT) ||
      (apdu[NFC_ISO7816_APDU_IDX_P1] != NFC_ISO7816_P1_SELECT_BY_DF_NAME) ||
      !nfc_ctap_fido_app_select_variant_match(apdu[NFC_ISO7816_APDU_IDX_P2], false,
                                              NERO_NFC_NULL)) {
    return false;
  }
  if (!nfc_iso7816_apdu_lc(apdu, apdu_len, &lc) || (lc != (uint8_t)NFC_CTAP_FIDO_AID_LEN)) {
    return false;
  }
  ...
  if ((aid == NERO_NFC_NULL) || (memcmp(aid, NFC_CTAP_FIDO_AID, NFC_CTAP_FIDO_AID_LEN) != 0)) {
    return false;
  }
  if (nfc_iso7816_apdu_lc_body_with_le_ok(apdu_len, lc)) {
    if (!nero_nfc_span_ok((size_t)(NFC_ISO7816_SHORT_APDU_HDR_LEN + lc), NFC_CTAP_CBOR_HDR_INLINE,
                          apdu_len) ||
        (apdu[NFC_ISO7816_SHORT_APDU_HDR_LEN + lc] != NFC_ISO7816_SW2_SUCCESS)) {
      return false;
    }
    if (!nfc_ctap_fido_app_select_variant_match(apdu[NFC_ISO7816_APDU_IDX_P2], true,
                                                NERO_NFC_NULL)) {
      return false;
    }
    if (add_le_00_out != NERO_NFC_NULL) {
...
bool nfc_ctap_apdu_command(const uint8_t *apdu, uint16_t apdu_len, uint8_t *cmd_out) {
  uint16_t lc;
  uint8_t lc8;
  const uint8_t *data;
  ...
  if ((apdu[0] != NFC_CTAP_CLA) || (apdu[1] != NFC_CTAP_INS_MSG) ||
  ...
  lc = (uint16_t)(((uint16_t)apdu[NFC_CTAP_APDU_EXTENDED_LC_MSB_OFFSET] << NFC_BYTE_SHIFT_8) |
                  apdu[NFC_CTAP_APDU_EXTENDED_LC_LSB_OFFSET]);
  {
    size_t expected_no_le = 0u;
    size_t expected_with_le = 0u;
    if ((lc == 0u) ||
        !nero_nfc_try_add_size(NFC_CTAP_APDU_EXTENDED_LC_HDR_OFFSET, (size_t)lc, &expected_no_le) ||
        !nero_nfc_try_add_size(expected_no_le, NFC_CTAP_APDU_EXTENDED_TAIL_LEN,
                               &expected_with_le) ||
        (((size_t)apdu_len != expected_no_le) && ((size_t)apdu_len != expected_with_le))) {
      return false;
    }
  }
  if (cmd_out != NERO_NFC_NULL) {
...
bool nfc_ctap_response_more_data(const uint8_t *resp, int rlen) {
  ...
  return nfc_iso7816_response_sw(resp, rlen, &sw1, &sw2) &&
         (sw1 == NFC_ISO7816_SW1_MORE_DATA_VENDOR) && (sw2 == NFC_ISO7816_SW2_SUCCESS);
}
...
bool nfc_ctap_apdu_is_getresponse(const uint8_t *apdu, uint16_t apdu_len) {
  ...
  return (apdu[0] == NFC_CTAP_CLA) && (apdu[1] == NFC_CTAP_INS_GETRESPONSE) &&
         (apdu[NFC_ISO7816_APDU_IDX_P2] == NFC_ISO7816_GET_DATA_P2_DEFAULT) &&
...
bool nfc_ctap_apdu_is_control_end(const uint8_t *apdu, uint16_t apdu_len) {
  ...
  if ((apdu[0] != NFC_CTAP_CLA) || (apdu[1] != NFC_CTAP_INS_CONTROL) ||
...
bool nfc_ctap_apdu_needs_ccid_time_extension(const uint8_t *apdu, uint16_t apdu_len) {
  uint8_t ctap_cmd = 0u;
  ...
  return (ctap_cmd == NFC_CTAP_CMD_MAKE_CREDENTIAL) || (ctap_cmd == NFC_CTAP_CMD_GET_ASSERTION) ||
         (ctap_cmd == NFC_CTAP_CMD_CLIENT_PIN);
}

nfc_ctap_relay_disposition_t nfc_ctap_relay_response_disposition(const uint8_t *raw,
                                                                 uint16_t raw_len) {
  uint16_t sw;
  ...
  sw = (uint16_t)(((uint16_t)raw[raw_len - NFC_ISO7816_SW_STATUS_WORD_LEN] << NFC_BYTE_SHIFT_8) |
  ...
  if (((sw & NFC_ISO7816_SW1_HIGH_BYTE_MASK) == NFC_ISO7816_SW1_MORE_DATA_PATTERN) ||
      ((sw & NFC_ISO7816_SW1_HIGH_BYTE_MASK) == NFC_ISO7816_SW1_MORE_DATA_VENDOR_PATTERN)) {
    return NFC_CTAP_RELAY_PASSTHROUGH;
  }
  /* ISO7816 warning/error status words (62xx..6Fxx). */
  if ((sw >= NFC_ISO7816_SW_WARNING_MIN) && (sw <= NFC_ISO7816_SW_WARNING_MAX)) {
    ...
    if ((raw_len >= NFC_CTAP_MIN_RESPONSE_BODY_LEN) && (raw[0] == NFC_CTAP_STATUS_SUCCESS)) {
      return NFC_CTAP_RELAY_FAIL_CLOSED;
    }
    return NFC_CTAP_RELAY_PASSTHROUGH;
  }
  /* Any other trailing word is not a valid ISO7816 status word (e.g. a CBOR body
   * byte misread as a status word): the response is malformed — fail closed. */
  return NFC_CTAP_RELAY_FAIL_CLOSED;
}

bool nfc_ctap_dissect_response(const uint8_t *raw, uint16_t raw_len, uint8_t *inner_out,
  ...
  if (((uint16_t)(raw[raw_len - NFC_ISO7816_SW_STATUS_WORD_LEN] << NFC_BYTE_SHIFT_8) |
  ...
  if (inner_out != NERO_NFC_NULL) {
    ...
    if ((inner_len_local > 0u) &&
        !nero_nfc_copy_bytes(inner_out, inner_cap, 0u, raw + NFC_CTAP_CBOR_HDR_INLINE,
                             inner_len_local)) {
      return false;
    }
  }
