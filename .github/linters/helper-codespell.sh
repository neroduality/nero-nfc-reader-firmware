...
#   nero_nfc_ensure_codespell
set -euo pipefail

NERO_NFC_CODESPELL_MIN_VERSION=2.4.2

nero_nfc_codespell_hint() {
  printf '%s\n' \
    'hint: INSTALL_DEPS=1 bash make/install-linux-deps.sh' \
    "      or: python3 -m pip install --user --break-system-packages 'codespell==${NERO_NFC_CODESPELL_MIN_VERSION}'" >&2
}
...
nero_nfc_ensure_codespell() {
  ...
  if nero_nfc_codespell_supports_multiline_regex; then
    return 0
  fi
  ...
  pip_args=(
    ...
    "codespell==${NERO_NFC_CODESPELL_MIN_VERSION}"
  )
  if [[ ${EUID} -ne 0 ]]; then
...
EOF
  ...
  default_targets() {
    local -a out=(README.md INSTALLATION.md)
    shopt -s nullglob
