...
/* CTAP NFC transport command bytes and buffer sizes: nfc_tag_geometry_limits.h ([CTAP2.3];
 * normative values in docs/spec-traceability.yaml). */
enum {
...
NERO_NFC_STATIC_ASSERT(NFC_CTAP_RP_ID_MAX <= 253u, "RP ID cap fits CTAP text-string limits");
NERO_NFC_STATIC_ASSERT(NFC_ISO_DEP_IBLOCK_TX_BUF_LEN == 832u,
                       "ISO-DEP I-block TX buffer fits CTAP APDU + framing overhead");

NERO_NFC_NODISCARD bool nfc_ctap_encode_get_assertion(const uint8_t *client_hash32,
                                                      const char *rp_id, const uint8_t *allow_cred,
                                                      uint16_t allow_cred_len, uint8_t *out,
                                                      uint16_t out_max, uint16_t *out_len);

NERO_NFC_NODISCARD bool nfc_ctap_pack_cbor_apdu(uint8_t ctap_cmd, const uint8_t *cbor,
                                                uint16_t cbor_len, bool force_extended,
                                                uint8_t *apdu, uint16_t apdu_cap,
                                                uint16_t *apdu_len);

NERO_NFC_NODISCARD bool nfc_ctap_pack_select_fido_apdu(bool add_le_00, uint8_t *apdu,
...
nfc_ctap_fido_app_select_variant(uint8_t index, nfc_ctap_fido_app_select_variant_t *out) {
  static const struct {
    uint8_t p2;
    bool add_le_00;
  } kVariants[NFC_CTAP_FIDO_APP_SELECT_VARIANT_COUNT] = {
    {0x00u, false},
    {0x00u, true},
    {0x0Cu, false},
    {0x0Cu, true},
  ...
  if ((out == NERO_NFC_NULL) || (index >= (uint8_t)NFC_CTAP_FIDO_APP_SELECT_VARIANT_COUNT)) {
    return false;
  }
  out->aid = NFC_CTAP_FIDO_AID;
  out->aid_len = (uint8_t)NFC_CTAP_FIDO_AID_LEN;
  out->p2 = kVariants[index].p2;
  out->add_le_00 = kVariants[index].add_le_00;
  return true;
}

NERO_NFC_NODISCARD static inline bool
nfc_ctap_fido_app_select_variant_match(uint8_t p2, bool add_le_00,
                                       nfc_ctap_fido_app_select_variant_t *out) {
  if (out != NERO_NFC_NULL) {
    out->aid = NERO_NFC_NULL;
    out->aid_len = 0u;
    out->p2 = 0u;
...
nfc_ctap_fido_webauthn_select_step(uint8_t step, nfc_ctap_fido_webauthn_select_step_t *out);

NERO_NFC_NODISCARD bool nfc_ctap_apdu_is_select_fido_aid(const uint8_t *apdu, uint16_t apdu_len,
                                                         bool *add_le_00_out);

NERO_NFC_NODISCARD bool nfc_ctap_apdu_command(const uint8_t *apdu, uint16_t apdu_len,
                                              uint8_t *cmd_out);

NERO_NFC_NODISCARD bool nfc_ctap_response_more_data(const uint8_t *resp, int rlen);

NERO_NFC_NODISCARD bool nfc_ctap_apdu_is_getresponse(const uint8_t *apdu, uint16_t apdu_len);

NERO_NFC_NODISCARD bool nfc_ctap_apdu_is_control_end(const uint8_t *apdu, uint16_t apdu_len);

NERO_NFC_NODISCARD bool nfc_ctap_apdu_needs_ccid_time_extension(const uint8_t *apdu,
                                                                uint16_t apdu_len);
...
typedef enum {
  ...
  NFC_CTAP_RELAY_FAIL_CLOSED = 1, /* malformed/contradictory/error: return ISO7816 error */
#ifdef __cplusplus
};
#else
...
NERO_NFC_NODISCARD bool nfc_ctap_dissect_response(const uint8_t *raw, uint16_t raw_len,
