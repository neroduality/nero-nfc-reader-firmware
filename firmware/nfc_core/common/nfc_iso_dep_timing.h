...
/*
 * [ISO14443-4] section 7.2 — the PCD must wait at least FWT for the PICC response.
 * FWT reaches ~4949 ms at FWI=14, so the link-response timeout ceiling must cover
 * the full FWT range (aligned with the WTX wait ceiling) instead of truncating a
 * compliant high-FWI token's allowed response window.
 */
#define ISO_DEP_LINK_RESPONSE_TIMEOUT_MAX_MS ISO_DEP_WTX_TIMEOUT_MAX_MS
#define ISO_DEP_APDU_CHUNK_SLACK 28u
#define ISO_DEP_APDU_CHUNK_HARD_CAP 48u
#define ISO_DEP_APDU_CHUNK_MIN 24u
#define ISO_DEP_APDU_CHUNK_FALLBACK_FSC 256u
#define ISO_DEP_APDU_CHUNK_FALLBACK_MIN_FSC 48u
...
static inline uint32_t reader_iso_dep_fwt_us_from_fwi(uint8_t fwi) {
  ...
  return (uint32_t)((((uint64_t)ISO_DEP_FWT_SCALE_NUM) * (1ull << fwi) * ISO_DEP_US_PER_SECOND +
...
static inline uint16_t reader_iso_dep_pre_first_iblock_delay_ms(uint32_t fwt_us) {
  uint32_t ms = (fwt_us + ISO_DEP_US_TO_MS_ROUND_UP) / (ISO_DEP_US_TO_MS_ROUND_UP + 1u);
...
static inline uint16_t reader_iso_dep_pre_first_iblock_delay_default_ms(void) {
  return reader_iso_dep_pre_first_iblock_delay_ms(reader_iso_dep_fwt_us_default());
}
...
static inline uint16_t reader_iso_dep_link_response_timeout_ms(uint32_t fwt_us) {
  uint32_t ms = (fwt_us + ISO_DEP_US_TO_MS_ROUND_UP) / (ISO_DEP_US_TO_MS_ROUND_UP + 1u);
...
static inline uint16_t reader_iso_dep_apdu_chunk_budget(uint16_t pic_frame_max,
                                                        uint8_t tx_hdr_len) {
  uint16_t f = pic_frame_max;
  uint16_t cap;

  if (f < ISO_DEP_APDU_CHUNK_FALLBACK_MIN_FSC) {
    f = ISO_DEP_APDU_CHUNK_FALLBACK_FSC;
  }
  if ((uint16_t)(f - ISO_DEP_APDU_CHUNK_SLACK) <= (uint16_t)tx_hdr_len) {
    cap = ISO_DEP_APDU_CHUNK_MIN;
  } else {
    cap = (uint16_t)((uint16_t)(f - ISO_DEP_APDU_CHUNK_SLACK) - (uint16_t)tx_hdr_len);
  }
  if (cap > ISO_DEP_APDU_CHUNK_HARD_CAP) {
    cap = ISO_DEP_APDU_CHUNK_HARD_CAP;
  }
  if (cap < ISO_DEP_APDU_CHUNK_MIN) {
    cap = ISO_DEP_APDU_CHUNK_MIN;
  }
  return cap;
}
