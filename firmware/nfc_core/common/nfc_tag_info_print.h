...
static inline void nfc_tag_print_hex_bytes(nero_nfc_emit_fn_t emit, const uint8_t *data,
                                           uint8_t len, char separator) {
...
static inline void nfc_tag_print_type2_debug(nero_nfc_emit_fn_t emit,
                                             const nfc_tag_typea_info_t *typea,
                                             const nfc_tag_type2_info_t *type2) {
  if ((emit == NERO_NFC_NULL) || (typea == NERO_NFC_NULL) || (type2 == NERO_NFC_NULL)) {
    return;
  }
  nfc_tag_print_line_prefix(emit);
  ...
  if (type2->version_valid) {
    ...
    if (type2->version_len >= NFC_TAG_NTAG_VER_REPLY_LEN) {
      ...
      nero_nfc_emit_dec_u32(emit, type2->version[NFC_TAG_NTAG_VER_PRODUCT_MAJOR_INDEX]);
      emit('.');
      nero_nfc_emit_dec_u32(emit, type2->version[NFC_TAG_NTAG_VER_PRODUCT_MINOR_INDEX]);
      nero_nfc_emit_write(emit, "\r\n");
    }
  }
  if (type2->product_name != NERO_NFC_NULL) {
  ...
  if (type2->cc_valid) {
    if (nero_nfc_span_ok(NFC_TAG_T2T_CC_ACCESS_INDEX, 1u, NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
      ...
      nero_nfc_emit_write(emit, "\r\n");
      nfc_tag_print_line_prefix(emit);
      nero_nfc_emit_write(emit, "Data format: NFC Forum Type 2 Tag / NDEF TLV\r\n");
      nfc_tag_print_line_prefix(emit);
      ...
      nero_nfc_emit_write(emit, type2->read_access_open ? "open" : "restricted");
      nero_nfc_emit_write(emit, "  Write access: ");
      nero_nfc_emit_write(emit, type2->write_access_open ? "open" : "restricted");
      nero_nfc_emit_write(emit, "\r\n");
      nfc_tag_print_line_prefix(emit);
      ...
      nero_nfc_emit_write(emit, type2->write_access_open ? "yes" : "already restricted/unknown");
      nero_nfc_emit_write(emit, "\r\n");
    }
  }
  ...
  if (type2->signature_valid) {
    ...
    nfc_tag_print_hex_bytes(emit, type2->signature, (uint8_t)sizeof(type2->signature), ' ');
    nero_nfc_emit_write(emit, "\r\n");
  }
}

static inline void nfc_tag_print_type4_debug(nero_nfc_emit_fn_t emit,
                                             const nfc_tag_typea_info_t *typea,
                                             const nfc_tag_type4_info_t *type4) {
  if ((emit == NERO_NFC_NULL) || (typea == NERO_NFC_NULL) || (type4 == NERO_NFC_NULL)) {
    return;
  }
  nfc_tag_print_line_prefix(emit);
  ...
  nero_nfc_emit_write(emit, "\r\n");
  nfc_tag_print_line_prefix(emit);
  nero_nfc_emit_write(emit, "Data format: NFC Forum Type 4 Tag / NDEF file\r\n");
  nfc_tag_print_line_prefix(emit);
  nero_nfc_emit_write(emit, "NDEF mapping: ");
  ...
  nero_nfc_emit_write(emit, "  Can be made read-only: ");
  nero_nfc_emit_write(emit, type4->write_access_open ? "yes (if tag supports locking)" : "no");
  nero_nfc_emit_write(emit, "\r\n");
}

static inline void nfc_tag_print_type5_debug(nero_nfc_emit_fn_t emit, const uint8_t *uid,
                                             uint8_t uid_len, const nfc_tag_type5_info_t *type5) {
  const char *vendor_name;

  if ((emit == NERO_NFC_NULL) || (uid == NERO_NFC_NULL) || (type5 == NERO_NFC_NULL)) {
    return;
  }
  nfc_tag_print_line_prefix(emit);
  ...
  if (type5->cc_valid) {
    ...
    nero_nfc_emit_write(emit, "\r\n");
    nfc_tag_print_line_prefix(emit);
    nero_nfc_emit_write(emit, "Data format: NFC Forum Type 5 Tag / NDEF TLV\r\n");
    nfc_tag_print_line_prefix(emit);
    nero_nfc_emit_write(emit, "NDEF mapping: ");
    ...
    nero_nfc_emit_write(emit, "  Can be made read-only: ");
    nero_nfc_emit_write(emit, type5->write_access_open ? "yes (if tag supports locking)" : "no");
    nero_nfc_emit_write(emit, "\r\n");
  }
}
