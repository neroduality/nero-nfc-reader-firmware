...
/*
 * reader_app.cpp — Unified NFC reader for the X-NUCLEO-NFC08A1 (ST25R3916B)
...
/* Pin map: port/include/nfc_board_defaults.h (UNO footprint + NFC08 shield) */
...
/* Reuse the existing field-load threshold for host-owned CCID Type 4 sessions.
 * Full WUPA/anticollision probes can disturb an active ISO-DEP exchange. */
#define CCID_ACTIVE_TYPE4_FIELD_LOAD_PRESENT 3

enum {
  kReaderAppSerialReadyWaitMs = 250u,
  kReaderAppSerialSettleMs = 40u,
  kReaderAppFieldBarWidth = 20u,
  kReaderAppFieldAmpMax = 255u,
  kReaderAppFieldAmpPadHundreds = 100u,
  kReaderAppFieldAmpPadTens = 10u,
  kReaderAppCcidPollIntervalMs = 100u,
  kReaderAppNearPollIntervalMs = 50u,
  kReaderAppFieldPrintNearMs = 200u,
  kReaderAppFieldPrintIdleMs = 1500u,
  kReaderAppTagRemoveWaitMs = 5000u,
  kReaderAppTagRemoveSettleMs = 50u,
  kReaderAppTagRemoveFinalMs = 20u,
  kReaderAppCouplingTagOn = 10u,
  kReaderAppCouplingApproach = 3u,
  kReaderAppFieldLoadNear = 3u,
  kReaderAppFatalBlinkMs = 200u,
  kReaderAppRfForceMs = 10u,
};

bool reader_ccid_prepare_tag_for_power_on(reader_tag_kind_t tag_kind) {
#if defined(NERO_CCID_USB_BUILD)
  reader_frontend_quiesce();
  if ((tag_kind == READER_TAG_KIND_TYPE2) || (tag_kind == READER_TAG_KIND_TYPE4)) {
    reader_protocol_configure_iso14443a();
    reader_frontend_ensure_tx_rx();
    if (!reader_protocol_activate_iso14443a()) {
      return false;
    }
    return (((g_sak & NFC_TAG_T4T_SAK_ISO14443_4_BIT) != 0u) ? READER_TAG_KIND_TYPE4
...
/* Rate-limits periodic field-strength gauge lines shared by setup + loop. */
static uint32_t g_last_field_print_ms;
#if defined(NERO_CCID_ONLY_BUILD)
static bool g_nfc_frontend_ready = false;
#endif

static uint8_t measure_amplitude(void) {
  return reader_frontend_measure_amplitude();
}
...
static void print_field_strength(uint8_t amp, uint8_t baseline) {
  ...
  (void)baseline;
#else
  uint8_t bar_len = (uint8_t)(((uint16_t)amp * kReaderAppFieldBarWidth) / kReaderAppFieldAmpMax);
  nero_nfc_log_write("[NFC reader] Field ");
  if (amp < kReaderAppFieldAmpPadHundreds) {
    nero_nfc_log_putc(' ');
  }
  if (amp < kReaderAppFieldAmpPadTens) {
    nero_nfc_log_putc(' ');
  }
  do {
  ...
  for (uint8_t i = 0u; i < kReaderAppFieldBarWidth; i++) {
  ...
  if (baseline > 0u) {
    ...
    nero_nfc_log_write(" | ");
    if (coupling > kReaderAppCouplingTagOn) {
      nero_nfc_log_write("tag on antenna");
    } else if (coupling > kReaderAppCouplingApproach) {
...
#endif
  s = reader_frontend_init(&chip_id, &vdd_mv);
#if !defined(NERO_CCID_USB_BUILD)
...
static void open_in_browser(void) {
  if ((!g_url_detected || (g_detected_url[0] == '\0')) && (g_detected_url_count == 0u)) {
    return;
  }
  if (g_detected_url_count != 0u) {
    ...
    if (!reader_is_browser_safe_url(g_detected_url)) {
      nero_nfc_log_line("  URL scheme blocked; only http(s) auto-open is allowed.");
...
static void reader_read_type4_ndef_tag(void) {
  ...
  if (!reader_security_key_iso_dep_session_open_quiet(SECURITY_KEY_RATS_TIMEOUT_MS)) {
...
static bool reader_poll_handle_iso14443a(void) {
  bool same = (g_uid14_len == g_last_uid_len) && (memcmp(g_uid14, g_last_uid, g_uid14_len) == 0);
  if (same) {
#if defined(NERO_CCID_USB_BUILD)
  ...
  if (!nero_nfc_copy_bytes(g_last_uid, sizeof(g_last_uid), 0u, g_uid14, g_uid14_len)) {
    return false;
  }
...
static bool ccid_probe_present_tag(reader_tag_kind_t *tag_kind_out, bool *same_uid_out) {
  ...
  reader_frontend_ensure_tx_rx();
  if (reader_protocol_activate_iso14443a()) {
    *tag_kind_out = ((g_sak & NFC_TAG_T4T_SAK_ISO14443_4_BIT) != 0u) ? READER_TAG_KIND_TYPE4
                                                                     : READER_TAG_KIND_TYPE2;
  ...
  if (reader_tags_iso15693_inventory_step()) {
    ...
    *same_uid_out = ccid_last_uid_matches(g_uid15, NFC_FRONTEND_ISO15693_UID_LEN);
    return true;
  }
...
static bool ccid_active_type4_field_present(void) {
  ...
  const int16_t load = (int16_t)g_baseline_amp - (int16_t)amp;
...
void reader_setup(void) {
...
#if !defined(NERO_CCID_ONLY_BUILD)
  reader_hal_serial_begin(NFC_HOST_SERIAL_BAUD);
#endif
#if !defined(NERO_CCID_USB_BUILD)
  ...
  while (!reader_hal_serial_ready() && ((reader_hal_millis() - t0) < kReaderAppSerialReadyWaitMs)) {
  }
  if (reader_hal_serial_ready()) {
    reader_hal_delay_ms(kReaderAppSerialSettleMs);
  }
  ...
  nero_nfc_log_write("    Tap tags for NDEF; http(s) lines emit BROWSER_OPEN for the "
                     "desktop opener\r\n");
  nero_nfc_log_line("  UART (combined firmware):");
  nero_nfc_log_write("    mode writer  · tag payloads (help for URL/text/…);  mode "
                     "reader  · this "
                     "reader\r\n");
#endif
...
void reader_setup_nfc_hardware(void) {
  ...
  if (!init_chip()) {
    ...
    while (true) {
      reader_hal_digital_write(LED_PIN, (uint8_t)!reader_hal_digital_read(LED_PIN));
      reader_hal_delay_ms(kReaderAppFatalBlinkMs);
    }
#endif
  }
...
#endif
    reader_frontend_enable_tx_rx();
#if defined(NERO_CCID_ONLY_BUILD)
    ccid_service_delay_ms(kReaderAppRfForceMs);
#else
    reader_hal_delay_ms(kReaderAppRfForceMs);
#endif
  }

#if defined(NERO_CCID_ONLY_BUILD)
  ccid_service_delay_ms(kReaderAppTagRemoveSettleMs);
#else
  reader_hal_delay_ms(kReaderAppTagRemoveSettleMs);
#endif
  g_baseline_amp = measure_amplitude();
#if !defined(NERO_CCID_USB_BUILD)
...
#endif
  g_last_field_print_ms = reader_hal_millis();
#if defined(NERO_CCID_ONLY_BUILD)
  g_nfc_frontend_ready = true;
#endif
}

#if defined(NERO_CCID_USB_BUILD)
static void reader_ccid_loop_step(void) {
  static uint32_t last_ccid_poll = 0u;
  static uint8_t ccid_remove_misses = 0u;
  static uint8_t last_ccid_icc_status = (uint8_t)NFC_CCID_ICC_NO_ICC;
  uint32_t now_ccid = reader_hal_millis();
  uint8_t icc_status = reader_ccid_icc_status();

  if (icc_status != last_ccid_icc_status) {
    last_ccid_icc_status = icc_status;
    ccid_remove_misses = 0u;
  }

  if (icc_status != (uint8_t)NFC_CCID_ICC_NO_ICC) {
    static uint32_t last_ccid_remove_probe = 0u;
    reader_tag_kind_t probed_kind = READER_TAG_KIND_NONE;
    bool same_uid = false;
    const bool active_type4_session = (icc_status == (uint8_t)NFC_CCID_ICC_ACTIVE) &&
                                      (reader_ccid_tag_kind() == READER_TAG_KIND_TYPE4);
    if ((now_ccid - last_ccid_remove_probe) < TAG_REMOVE_MISS_RETRY_MS) {
      return;
    }
    last_ccid_remove_probe = now_ccid;
    bool still_present;
    if (active_type4_session) {
      still_present = ccid_active_type4_field_present();
      same_uid = true;
      probed_kind = READER_TAG_KIND_TYPE4;
    } else {
    ...
    if (still_present) {
      ccid_remove_misses = 0u;
      if (!same_uid) {
        const uint8_t *uid = (probed_kind == READER_TAG_KIND_TYPE5) ? g_uid15 : g_uid14;
        const uint8_t uid_len =
          (probed_kind == READER_TAG_KIND_TYPE5) ? NFC_FRONTEND_ISO15693_UID_LEN : g_uid14_len;
    ...
    ccid_remove_misses++;
    if (ccid_remove_misses >= TAG_REMOVE_CONSECUTIVE_MISSES) {
      ccid_remove_misses = 0u;
      g_last_uid_len = 0u;
      reader_ccid_on_tag_removed_from_field();
    }
  ...
  ccid_remove_misses = 0u;
  if ((now_ccid - last_ccid_poll) < kReaderAppCcidPollIntervalMs) {
    return;
  }
  last_ccid_poll = now_ccid;
  (void)poll_one_cycle();
}
#endif
...
void reader_loop(void) {
  ...
  static uint32_t poll_interval = kReaderAppCcidPollIntervalMs;
  ...
  if (load > kReaderAppFieldLoadNear) {
    poll_interval = kReaderAppNearPollIntervalMs;
    if (!was_near) {
#if !defined(NERO_CCID_USB_BUILD)
    ...
    poll_interval = kReaderAppCcidPollIntervalMs;
    if (was_near) {
#if !defined(NERO_CCID_USB_BUILD)
   ...
   * path is faster. */
  if ((now - g_last_field_print_ms) >=
      (was_near ? kReaderAppFieldPrintNearMs : kReaderAppFieldPrintIdleMs)) {
    print_field_strength(amp, g_baseline_amp);
    g_last_field_print_ms = now;
  }
  ...
  while ((reader_hal_millis() - remove_start) < kReaderAppTagRemoveWaitMs) {
#if defined(NERO_CCID_USB_BUILD)
    reader_ccid_poll();
#endif
    ...
    reader_ccid_on_tag_removed_from_field();
#endif
    reader_hal_delay_ms(kReaderAppTagRemoveFinalMs);
    g_baseline_amp = measure_amplitude();
#if !defined(NERO_CCID_USB_BUILD)
