...
enum {
  kWriterCliHexNibbleShift = 4u,
  kWriterCliLongOptPrefixLen = 2u,
};
...
static char g_line_storage[WRITER_CLI_LINE_CAP];
static char *g_line = g_line_storage;
static uint16_t g_line_len;
static bool g_line_overflow;
static uint8_t g_raw_ndef[WRITER_NDEF_MAX_BYTES];
...
template <size_t N>
// NOLINTNEXTLINE(cert-dcl50-cpp) — bounded printf wrapper for embedded CLI
static bool nero_nfc_snprintf_buf(char (&dst)[N], const char *fmt, ...) CLI_PRINTF_LIKE;

template <size_t N>
static bool nero_nfc_snprintf_buf(char (&dst)[N], const char *fmt, ...) { // NOLINT(cert-dcl50-cpp)
  va_list args;
  va_start(args, fmt);
  const int n = nero_nfc_vsnprintf(dst, N, fmt, args);
  va_end(args);
...
static void trim(char *s, size_t cap) {
  ...
  if ((s == NERO_NFC_NULL) || (cap == 0u) || !nero_nfc_bounded_strlen(s, cap, &n)) {
    return;
  }
  ...
  if (p != s) {
    auto consumed = static_cast<size_t>(p - s);
    (void)nero_nfc_move_bytes(s, cap, 0u, p, n - consumed + 1u);
    n -= consumed;
  }
  while ((n > 0u) && ((unsigned char)s[n - 1u] <= ' ')) {
    n--;
    s[n] = '\0';
  }
}
...
static void strip_https_prefix(char *s) {
  ...
  constexpr size_t kHttpsPrefixLen = sizeof(kHttpsPrefix) - 1u;
  if (starts_ci(s, kHttpsPrefix)) {
    size_t n = 0u;
    if (nero_nfc_bounded_strlen(s, WRITER_STR1_MAX, &n) && (n >= kHttpsPrefixLen)) {
      (void)nero_nfc_move_bytes(s, WRITER_STR1_MAX, 0u, s + kHttpsPrefixLen,
                                n - kHttpsPrefixLen + 1u);
    }
  }
}
...
static void print_show(const writer_payload_config_t *cfg) {
  ...
  if ((cfg->kind == WRITER_PAYLOAD_RAW_NDEF) && (cfg->raw_ndef != NERO_NFC_NULL)) {
    nero_nfc_log_write("\r\n  NDEF bytes: ");
    do {
      char ndc[NERO_NFC_FORMAT_SNPRINTF_DEC_CAP];
      (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u", (unsigned)(uint32_t)(cfg->raw_ndef_len));
...
static bool parse_hex_bytes(const char *text, uint8_t *out, uint16_t out_cap, uint16_t *out_len) {
  uint16_t n = 0u;
  uint8_t hi = 0u;
  bool have_hi = false;
  ...
  if ((text == NERO_NFC_NULL) || (out_len == NERO_NFC_NULL) || (out_cap == 0u)) {
    return false;
  }
  for (const char *p = text; *p != '\0'; ++p) {
    ...
    if (out != NERO_NFC_NULL) {
      out[n] = (uint8_t)((hi << kWriterCliHexNibbleShift) | v);
    }
    n++;
    have_hi = false;
  }
...
static bool writer_cli_apply_command(writer_payload_config_t *cfg, const char *cmd, char *rest) {
  bool updated = true;
  ...
  if (starts_ci(cmd, "url")) {
    ...
    if (rest[0] == '\0' || !nero_nfc_snprintf_buf(uri, "tel:%s", rest) ||
    ...
    if (!copy_field(cfg->str1, sizeof(cfg->str1), a)) {
      ...
      (void)copy_field(cfg->str2, sizeof(cfg->str2), (b != NERO_NFC_NULL) ? b : "");
    ...
    if (!copy_field(cfg->str1, sizeof(cfg->str1), addr)) {
      ...
      if ((subj != NERO_NFC_NULL) && (body != NERO_NFC_NULL)) {
        (void)nero_nfc_snprintf_buf(combined, "%s|%s", subj, body);
      } else if (subj != NERO_NFC_NULL) {
        (void)nero_nfc_snprintf_buf(combined, "%s", subj);
      }
    ...
    if (!copy_field(cfg->str1, sizeof(cfg->str1), fn)) {
      ...
      if ((tel != NERO_NFC_NULL) && (email != NERO_NFC_NULL)) {
        (void)nero_nfc_snprintf_buf(combo, "%s|%s", tel, email);
      } else if (tel != NERO_NFC_NULL) {
        (void)nero_nfc_snprintf_buf(combo, "%s", tel);
      }
    ...
    char uri[WRITER_STR1_MAX];
    if (rest[0] == '\0' ||
        !nero_nfc_snprintf_buf(uri, "https://www.google.com/search?q=%s", rest) ||
        !copy_field(cfg->str1, sizeof(cfg->str1), uri)) {
  ...
  } else if (starts_ci(cmd, "address") || starts_ci(cmd, "destination-address")) {
    cfg->uri_id = 0x00u;
    char uri[WRITER_STR1_MAX];
    const char *pfx = starts_ci(cmd, "destination-address")
                        ? "https://www.google.com/maps/dir/?api=1&destination="
                        : "https://www.google.com/maps/search/?api=1&query=";
    if (rest[0] == '\0' || !nero_nfc_snprintf_buf(uri, "%s%s", pfx, rest) ||
        !copy_field(cfg->str1, sizeof(cfg->str1), uri)) {
  ...
  } else if (starts_ci(cmd, "launch") || starts_ci(cmd, "uri") || starts_ci(cmd, "link") ||
             starts_ci(cmd, "unit-link") || starts_ci(cmd, "social") || starts_ci(cmd, "video") ||
    ...
    uint16_t raw_len = 0u;
    if (!parse_hex_bytes(rest, NERO_NFC_NULL, (uint16_t)sizeof(g_raw_ndef), &raw_len) ||
        !parse_hex_bytes(rest, g_raw_ndef, (uint16_t)sizeof(g_raw_ndef), &raw_len)) {
      nero_nfc_log_write("  ndef-hex: invalid or oversized hex NDEF message\r\n");
      updated = false;
    } else {
      cfg->raw_ndef = g_raw_ndef;
...
static bool handle_line(writer_payload_config_t *cfg, char *line) {
  ...
  if ((line[0] == '-') && (line[1] == '-')) {
    line += kWriterCliLongOptPrefixLen;
    char *eq = strchr(line, '=');
    if (eq != NERO_NFC_NULL) {
      *eq = ' ';
    }
...
bool writer_serial_cli_poll(writer_payload_config_t *cfg) {
...
#if defined(NERO_RAM_CONSTRAINED)
  if ((cfg == NERO_NFC_NULL) || (line == NERO_NFC_NULL)) {
    return false;
  }
#else
  if (cfg == NERO_NFC_NULL) {
  ...
  if (g_line_len < (WRITER_CLI_LINE_CAP - 1u)) {
    line[g_line_len++] = ch;
  } else {
