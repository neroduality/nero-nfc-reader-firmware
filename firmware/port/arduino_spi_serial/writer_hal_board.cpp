...
static SPISettings g_writer_spi_settings(NFC_BOARD_SPI_CLOCK_HZ, MSBFIRST, SPI_MODE1);
...
extern "C" void nero_nfc_log_putc(char c) {
  writer_hal_serial_write_char(c);
}
...
void writer_hal_spi_begin_transaction(void) {
  SPI.beginTransaction(g_writer_spi_settings);
}
