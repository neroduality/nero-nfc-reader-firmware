...
#define NERO_NFC_HEX_DIGITS "0123456789ABCDEF"
...
static inline void nero_nfc_emit_hex_u8(nero_nfc_emit_fn_t emit,
                                        uint8_t value) {
  if (emit == NERO_NFC_NULL) {
    return;
  }
  emit(NERO_NFC_HEX_DIGITS[value >> NERO_NFC_EMIT_HEX_HIGH_NIBBLE_SHIFT]);
  emit(NERO_NFC_HEX_DIGITS[value & NERO_NFC_EMIT_HEX_NIBBLE_MASK]);
}

static inline void nero_nfc_emit_dec_u32(nero_nfc_emit_fn_t emit,
                                         uint32_t value) {
...
static inline void nero_nfc_emit_dec_i32(nero_nfc_emit_fn_t emit,
                                         int32_t value) {
  if (emit == NERO_NFC_NULL) {
    return;
  }
