...
/*
 * nfc_byte_tutorial.h — byte-level annotated dumps for the serial (CDC)
 * console.
 *
 ...
 *   [T5T-ISO15693]   NFC Forum Type 5 Tag Technical Specification 1.0 over
 * ISO/IEC 15693-3 [T4T-ISO14443-4]   NFC Forum Type 4 Tag Technical
 * Specification 1.0 [ISO7816-4] ISO/IEC 7816-4 (SELECT / READ BINARY / UPDATE
 * BINARY) [T2T-ISO14443-A-NTAG21x]  NXP NTAG213/215/216 data sheet Rev. 3.2
...
static inline void nfc_tutorial_byte_line(nero_nfc_emit_fn_t emit,
                                          uint8_t value, const char* meaning) {
...
static inline void nfc_tutorial_hex_row(nero_nfc_emit_fn_t emit,
...
static inline void nfc_tutorial_print_area_bytes(nero_nfc_emit_fn_t emit,
                                                 uint16_t mlen_units) {
  nero_nfc_emit_write(emit, "NDEF data area = ");
  nero_nfc_emit_dec_u32(
      emit, (uint32_t)mlen_units * NFC_TAG_T2T_AREA_SIZE_UNIT_BYTES);
...
/* CC at page NFC_TAG_T2T_CC_PAGE_INDEX. [T2T-ISO14443-A] Type 2 Tag Op Spec
 * 1.1 §6.1 — E1h magic, MLEN, RWA. */
static inline void nfc_tutorial_t2t_cc(nero_nfc_emit_fn_t emit,
                                       const uint8_t* cc) {
  if ((emit == NERO_NFC_NULL) || (cc == NERO_NFC_NULL)) {
    return;
  }
  nfc_tutorial_hex_row(emit, "[T2T CC @ page 3]", cc,
                       NFC_TAG_T2T_PAGE_SIZE_BYTES);
  nfc_tutorial_byte_line(
      emit,
      nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES), (size_t)(0)),
      (nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES), (size_t)(0)) ==
       (uint8_t)NFC_FORUM_CC_MAGIC)
          ? "magic E1h: NFC Forum T2T, NDEF present "
            "([T2T-ISO14443-A] Type 2 Tag Op Spec 1.1 §6.1)"
          : "magic: NOT E1h -> not an NDEF T2T");
  nero_nfc_emit_write(emit, "      ");
  nero_nfc_emit_hex_u8(
      emit,
      nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES), (size_t)(1)));
  nero_nfc_emit_write(emit, "  mapping version ");
  nero_nfc_emit_dec_u32(
      emit, (uint32_t)(nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
  ...
  nero_nfc_emit_write(emit, "\r\n");
  nero_nfc_emit_write(emit, "      ");
  nero_nfc_emit_hex_u8(emit,
                       nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
  ...
  nero_nfc_emit_hex_u8(emit,
                       nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
                                      (size_t)(NFC_TAG_T2T_CC_MLEN_INDEX)));
  nero_nfc_emit_write(emit, " -> ");
  nfc_tutorial_print_area_bytes(
      emit, nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
                           (size_t)(NFC_TAG_T2T_CC_MLEN_INDEX)));
  nero_nfc_emit_write(emit, "\r\n");
  nero_nfc_emit_write(emit, "      ");
  nero_nfc_emit_hex_u8(emit,
  ...
  nero_nfc_emit_write(
      emit, ((nero_nfc_u8_at(cc, (size_t)(NFC_TAG_T2T_PAGE_SIZE_BYTES),
                             (size_t)(NFC_TAG_T2T_CC_ACCESS_INDEX)) &
              NFC_TAG_T2T_ACK_NIBBLE_MASK) == 0u)
                ? "open"
                : "restricted");
  nero_nfc_emit_write(emit, "  (T2T Op Spec 1.1 §6.1 RWA)\r\n");
}

/* [T2T-ISO14443-A-NTAG21x] section 10.2 / [T2T-ISO14443-A] section 5.1 — READ
 * (30h) returns NFC_TAG_T2T_READ_RESP_BYTES.
 */
static inline void nfc_tutorial_t2t_read_cmd(nero_nfc_emit_fn_t emit,
                                             uint8_t page) {
  const uint8_t frame[NFC_TAG_T2T_READ_CMD_LEN] = {NFC_FRONTEND_NTAG_CMD_READ,
                                                   page};
  if (emit == NERO_NFC_NULL) {
    return;
  }
  nfc_tutorial_hex_row(emit, "[T2T READ cmd]", frame, sizeof(frame));
  nfc_tutorial_byte_line(emit, frame[0],
                         "30h READ opcode (returns 16 bytes = 4 pages)");
  nfc_tutorial_byte_line(emit, frame[1], "start page address");
}

static inline void nfc_tutorial_t2t_fast_read_cmd(nero_nfc_emit_fn_t emit,
                                                  uint8_t first_page,
                                                  uint8_t last_page) {
  const uint8_t frame[NFC_TAG_T2T_FAST_READ_CMD_LEN] = {
      NFC_FRONTEND_NTAG_CMD_FAST_READ, first_page, last_page};
  if (emit == NERO_NFC_NULL) {
    return;
  }
  nfc_tutorial_hex_row(emit, "[T2T FAST_READ cmd]", frame, sizeof(frame));
  nfc_tutorial_byte_line(
      emit, frame[0], "3Ah FAST_READ opcode (returns pages in one response)");
  nfc_tutorial_byte_line(emit, frame[1], "first page address");
  nfc_tutorial_byte_line(emit,
                         frame[NFC_TUTORIAL_T2T_FAST_READ_LAST_PAGE_INDEX],
                         "last page address");
}

/* [T2T-ISO14443-A-NTAG21x] section 10.4 / [T2T-ISO14443-A] section 5.2 — WRITE
 * (A2h) one NFC_TAG_T2T_PAGE_SIZE_BYTES page. */
static inline void nfc_tutorial_t2t_write_cmd(nero_nfc_emit_fn_t emit,
                                              uint8_t page,
                                              const uint8_t* data) {
  ...
  if (!nero_nfc_copy_bytes(frame, sizeof(frame),
                           NFC_TUTORIAL_T2T_WRITE_DATA_BASE, data,
                           NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
    return;
  }
  ...
  nfc_tutorial_hex_row(emit, "  payload (4 data bytes):",
                       &frame[NFC_TUTORIAL_T2T_WRITE_DATA_BASE],
                       NFC_TAG_T2T_PAGE_SIZE_BYTES);
}

/* --- NFC Forum Type 5 (T5T) / ISO 15693 --- */

/* CC at block 0. [T5T-ISO15693] section 4.3.1 — magic E1h/E2h, version+access,
 * MLEN. */
static inline void nfc_tutorial_t5t_cc(nero_nfc_emit_fn_t emit,
                                       const uint8_t* cc_block,
                                       uint8_t cc_len) {
  uint16_t mlen_units;
  ...
  nfc_tutorial_byte_line(
      emit, nero_nfc_u8_at(cc_block, (size_t)(cc_len), (size_t)(0)),
      (nero_nfc_u8_at(cc_block, (size_t)(cc_len), (size_t)(0)) ==
       (uint8_t)NFC_T5T_CC_MAGIC_8BYTE)
          ? "magic E2h: T5T, 2-byte block address mode "
            "([T5T-ISO15693] section 4.3.1)"
          : "magic E1h: T5T, 1-byte block address mode "
            "([T5T-ISO15693] section 4.3.1)");
  {
    nero_nfc_emit_write(emit, "      ");
    nero_nfc_emit_hex_u8(
        emit, nero_nfc_u8_at(cc_block, (size_t)(cc_len), (size_t)(1)));
    nero_nfc_emit_write(emit, "  ver ");
    nero_nfc_emit_dec_u32(
        emit, (uint32_t)(nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                                        (size_t)(NFC_TAG_T2T_CC_VER_INDEX)) >>
  ...
  if ((nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                      (size_t)(NFC_TAG_T5T_CC_MLEN_BYTE_INDEX)) != 0u) ||
      (cc_len < NFC_TAG_T5T_CC_LEN_EXTENDED)) {
    mlen_units = nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                                (size_t)(NFC_TAG_T5T_CC_MLEN_BYTE_INDEX));
    nero_nfc_emit_write(emit, "      ");
    nero_nfc_emit_hex_u8(
        emit, nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                             (size_t)(NFC_TAG_T5T_CC_MLEN_BYTE_INDEX)));
    ...
    nfc_tutorial_byte_line(
        emit,
        nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                       (size_t)(NFC_TAG_T5T_CC_FLAGS_BYTE_INDEX)),
        "feature flags (special frame / lock block / overflow)");
  } else {
    mlen_units =
        (uint16_t)(((uint16_t)nero_nfc_u8_at(
                        cc_block, (size_t)(cc_len),
                        (size_t)(NFC_TUTORIAL_T5T_CC_MLEN_U16_OFF))
                    << NFC_TUTORIAL_UINT8_SHIFT) |
                   nero_nfc_u8_at(
                       cc_block, (size_t)(cc_len),
                       (size_t)(NFC_TUTORIAL_T5T_CC_MLEN_U16_OFF + 1u)));
    nfc_tutorial_byte_line(
        emit,
        nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                       (size_t)(NFC_TAG_T5T_CC_MLEN_BYTE_INDEX)),
        "MLEN=00h -> extended CC: size in bytes 6..7 "
        "([T5T-ISO15693] section 4.3.1.17)");
    nfc_tutorial_byte_line(
        emit,
        nero_nfc_u8_at(cc_block, (size_t)(cc_len),
                       (size_t)(NFC_TAG_T5T_CC_FLAGS_BYTE_INDEX)),
        "feature flags");
    nfc_tutorial_hex_row(emit, "  bytes 6..7 (16-bit MLEN):",
                         (cc_block + (NFC_TUTORIAL_T5T_CC_MLEN_U16_OFF)),
...
static inline void nfc_tutorial_t5t_read_cmd(nero_nfc_emit_fn_t emit,
  ...
  nfc_tutorial_byte_line(
      emit, NFC_TAG_T5T_ISO15693_FLAGS_ADDRESSED,
      "request flags: Addressed + high data rate ([T5T-ISO15693] section 7.3)");
  nfc_tutorial_byte_line(
      emit, NFC_TAG_T5T_ISO15693_CMD_READ_SINGLE,
  ...
  nfc_tutorial_byte_line(emit, block,
                         "block number to read (4 bytes returned)");
}
...
static inline void nfc_tutorial_t5t_write_cmd(nero_nfc_emit_fn_t emit,
                                              const uint8_t* uid_lsb,
                                              uint8_t block,
                                              const uint8_t* data,
                                              uint8_t data_len) {
  ...
  nfc_tutorial_byte_line(
      emit, NFC_TAG_T5T_ISO15693_CMD_WRITE_SINGLE,
 ...
 */
static inline void nfc_tutorial_t4t_select_app(nero_nfc_emit_fn_t emit,
                                               const uint8_t* aid,
                                               uint8_t aid_len) {
  if (emit == NERO_NFC_NULL) {
    return;
  ...
  nfc_tutorial_byte_line(emit, NFC_ISO7816_INS_SELECT,
                         "INS = A4h SELECT (ISO 7816-4)");
  nfc_tutorial_byte_line(emit, NFC_ISO7816_P1_SELECT_BY_DF_NAME,
                         "P1 = 04h select by name (DF/AID)");
  nfc_tutorial_byte_line(emit, NFC_ISO7816_P2_SELECT_FIRST,
                         "P2 = 00h first/only occurrence");
  if (aid != NERO_NFC_NULL) {
    nfc_tutorial_byte_line(emit, aid_len, "Lc = AID length");
    nfc_tutorial_hex_row(
        emit, "  AID (NDEF Tag Application D2760000850101):", aid, aid_len);
  }
}

/* SELECT by file ID. [ISO7816-4] SELECT / [T4T-ISO14443-4] section 5.4.2. */
static inline void nfc_tutorial_t4t_select_file(nero_nfc_emit_fn_t emit,
                                                uint8_t fid_hi,
                                                uint8_t fid_lo) {
  ...
  nfc_tutorial_byte_line(emit, NFC_ISO7816_P2_SELECT_NO_FCI,
                         "P2 = 0Ch no response data");
  nfc_tutorial_byte_line(emit, NFC_ISO7816_LC_SELECT_FILE_ID,
                         "Lc = 2 (file ID length)");
  nfc_tutorial_hex_row(emit, "  file ID (E103=CC, NDEF FID from CC):", fid,
                       NFC_TUTORIAL_U16_FIELD_LEN);
}

/* [T4T-ISO14443-4] section 7.5.1 — NFC_TAG_T4T_CC_MIN_LEN-byte Capability
 * Container file. */
static inline void nfc_tutorial_t4t_cc(nero_nfc_emit_fn_t emit,
  ...
  cclen =
      (uint16_t)(((uint16_t)nero_nfc_u8_at(cc, (size_t)(cc_len), (size_t)(0))
                  << NFC_TUTORIAL_UINT8_SHIFT) |
                 nero_nfc_u8_at(cc, (size_t)(cc_len), (size_t)(1)));
  ...
  nfc_tutorial_hex_row(emit, "  bytes 0..1  CCLEN =", (cc + (0)),
                       NFC_TUTORIAL_U16_FIELD_LEN);
  nero_nfc_emit_write(emit, "      (");
  ...
  nfc_tutorial_hex_row(
      emit, "  bytes 3..4  MLe =", (cc + (NFC_TAG_T4T_CC_MLE_MSB_INDEX)),
      NFC_TUTORIAL_U16_FIELD_LEN);
  nero_nfc_emit_write(emit, "      (max bytes per READ BINARY response = ");
  nero_nfc_emit_dec_u32(emit, mle);
  nero_nfc_emit_write(emit, ")\r\n");
  nfc_tutorial_hex_row(
      emit, "  bytes 5..6  MLc =", (cc + (NFC_TAG_T4T_CC_MLC_MSB_INDEX)),
      NFC_TUTORIAL_U16_FIELD_LEN);
  ...
  nfc_tutorial_hex_row(emit, "  bytes 9..10 NDEF file ID =",
                       (cc + (NFC_TAG_T4T_CC_NDEF_FILE_ID_MSB_INDEX)),
                       NFC_TUTORIAL_U16_FIELD_LEN);
  nfc_tutorial_hex_row(emit, "  bytes 11..12 max NDEF file size =",
                       (cc + (NFC_TAG_T4T_CC_NDEF_SIZE_MSB_INDEX)),
                       NFC_TUTORIAL_U16_FIELD_LEN);
  nero_nfc_emit_write(emit, "      (");
  ...
  nfc_tutorial_byte_line(
      emit,
      nero_nfc_u8_at(cc, (size_t)(cc_len),
                     (size_t)(NFC_TAG_T4T_CC_READ_ACCESS_INDEX)),
      (nero_nfc_u8_at(cc, (size_t)(cc_len),
                      (size_t)(NFC_TAG_T4T_CC_READ_ACCESS_INDEX)) ==
       NFC_ISO7816_CLA_ISO)
  ...
  nfc_tutorial_byte_line(
      emit,
      nero_nfc_u8_at(cc, (size_t)(cc_len),
                     (size_t)(NFC_TAG_T4T_CC_WRITE_ACCESS_INDEX)),
      (nero_nfc_u8_at(cc, (size_t)(cc_len),
                      (size_t)(NFC_TAG_T4T_CC_WRITE_ACCESS_INDEX)) ==
       NFC_ISO7816_CLA_ISO)
...
static inline void nfc_tutorial_t4t_read_binary(nero_nfc_emit_fn_t emit,
  ...
  nfc_tutorial_byte_line(emit, NFC_ISO7816_CLA_ISO, "CLA = 00h");
  nfc_tutorial_byte_line(emit, NFC_ISO7816_INS_READ_BINARY,
                         "INS = B0h READ BINARY");
  nfc_tutorial_byte_line(emit, (uint8_t)(offset >> NFC_TUTORIAL_UINT8_SHIFT),
                         "P1 = offset high byte");
  nfc_tutorial_byte_line(emit, (uint8_t)(offset & NFC_BYTE_VALUE_MAX),
                         "P2 = offset low byte");
  nfc_tutorial_byte_line(emit, le,
                         "Le = bytes to read (response ends with 90 00)");
}

/* UPDATE BINARY: [ISO7816-4] / [T4T-ISO14443-4] section 5.4.5. */
static inline void nfc_tutorial_t4t_update_binary(nero_nfc_emit_fn_t emit,
                                                  uint16_t offset,
                                                  const uint8_t* data,
                                                  uint8_t data_len) {
  if (emit == NERO_NFC_NULL) {
  ...
  nfc_tutorial_byte_line(emit, NFC_ISO7816_CLA_ISO, "CLA = 00h");
  nfc_tutorial_byte_line(emit, NFC_ISO7816_INS_UPDATE_BINARY,
                         "INS = D6h UPDATE BINARY");
  nfc_tutorial_byte_line(emit, (uint8_t)(offset >> NFC_TUTORIAL_UINT8_SHIFT),
                         "P1 = offset high byte");
  nfc_tutorial_byte_line(emit, (uint8_t)(offset & NFC_BYTE_VALUE_MAX),
                         "P2 = offset low byte");
...
static inline void nfc_tutorial_t4t_nlen(nero_nfc_emit_fn_t emit,
                                         uint16_t nlen) {
  const uint8_t bytes[NFC_TUTORIAL_U16_FIELD_LEN] = {
      (uint8_t)(nlen >> NFC_TUTORIAL_UINT8_SHIFT),
      (uint8_t)(nlen & NFC_BYTE_VALUE_MAX)};
  if (emit == NERO_NFC_NULL) {
    return;
  }
  nfc_tutorial_hex_row(emit, "[T4T NLEN @ NDEF file offset 0]", bytes,
                       NFC_TUTORIAL_U16_FIELD_LEN);
...
static inline void nfc_tutorial_ndef_message(nero_nfc_emit_fn_t emit,
                                             const uint8_t* data,
                                             uint16_t len) {
...
/* --- Shared NDEF Message TLV ([T2T-ISO14443-A] section 5.4 / [T5T-ISO15693]
 * section 4.5) --- */
 ...
 */
static inline void nfc_tutorial_ndef_tlv(nero_nfc_emit_fn_t emit,
                                         const uint8_t* data, uint16_t len) {
  uint16_t value_len;
  uint16_t value_off;
  if ((emit == NERO_NFC_NULL) || (data == NERO_NFC_NULL) ||
      (len < NFC_TUTORIAL_NDEF_TLV_MIN_LEN)) {
    return;
  }
  nero_nfc_emit_write(emit, "    [NDEF Message TLV]\r\n");
  nfc_tutorial_byte_line(emit, nero_nfc_u8_at(data, len, 0u),
                         (nero_nfc_u8_at(data, len, 0u) == NFC_NDEF_TLV_MESSAGE)
                             ? "T = 03h NDEF Message TLV"
                             : "T (not an NDEF Message TLV)");
  if (nero_nfc_u8_at(data, len, 0u) != NFC_NDEF_TLV_MESSAGE) {
    return;
  }
  if (data[1] == NFC_NDEF_TLV_EXTENDED_LEN) {
    ...
    value_len = (uint16_t)(((uint16_t)data[NFC_TUTORIAL_NDEF_TLV_EXT_LEN_MSB]
                            << NFC_TUTORIAL_UINT8_SHIFT) |
                           data[NFC_TUTORIAL_NDEF_TLV_EXT_LEN_LSB]);
    value_off = NFC_TUTORIAL_NDEF_TLV_VALUE_OFF_EXT;
    nfc_tutorial_byte_line(emit, data[1],
                           "L = FFh -> 3-byte length form follows");
    nfc_tutorial_hex_row(
        emit, "  L (16-bit):", &data[NFC_TUTORIAL_NDEF_TLV_EXT_LEN_MSB],
  ...
  if (nero_nfc_span_ok((size_t)value_off, (size_t)value_len, len)) {
    nfc_tutorial_hex_row(emit, "  V (NDEF message):", &data[value_off],
                         value_len);
    if (nero_nfc_span_ok((size_t)value_off + (size_t)value_len, 1u, len)) {
      ...
      if (terminator == NFC_NDEF_TLV_TERMINATOR) {
        nfc_tutorial_byte_line(emit, NFC_NDEF_TLV_TERMINATOR,
                               "Terminator TLV (end of data area)");
      }
    }
  }
}
