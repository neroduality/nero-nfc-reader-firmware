...
enum {
  kReaderSecurityKeyRelayMinResponseLen = NFC_ISO7816_MIN_RESPONSE_WITH_SW_LEN,
};

static constexpr uint16_t kReaderSecurityKeyRelayExchangeTimeoutMs = 45000u;
static constexpr uint16_t kReaderSecurityKeyRelayPostFidoSelectMs = 12u;
...
bool reader_security_key_ccid_open_iso_session(void) {
  bool reopen_after_deselect = (reader_security_key_iso_dep_last_deselect_ms() != 0u);
  ...
  if (reopen_after_deselect) {
    ...
    if (!reader_security_key_iso_dep_open_main_from_active(SECURITY_KEY_RATS_TIMEOUT_MS)) {
      return false;
    }
    reader_security_key_iso_dep_post_recover_rf_settle();
  } else {
    if (!reader_security_key_iso_dep_session_open(SECURITY_KEY_RATS_TIMEOUT_MS)) {
      reader_protocol_configure_iso14443a();
      reader_frontend_ensure_tx_rx();
      if (!reader_protocol_activate_iso14443a() ||
          !reader_security_key_iso_dep_session_open(SECURITY_KEY_RATS_TIMEOUT_MS)) {
        return false;
      }
    }
...
bool reader_security_key_pcsc_contactless_copy_atr(uint8_t *dst, uint16_t dst_cap,
                                                   uint16_t *alen_io) {
  return reader_security_key_copy_ats_as_pcsc_atr(g_ats_data, g_ats_len, dst, dst_cap, alen_io);
}

static uint16_t security_key_relay_failure_response(uint8_t *rsp, uint16_t rsp_cap) {
  return reader_security_key_relay_failure_response(rsp, rsp_cap);
}

static bool relay_select_with_recovery(const uint8_t *apdu, uint16_t apdu_len, uint8_t *rsp,
  ...
  *rsp_len_out = 0u;
  ...
  reader_security_key_iso_dep_pre_first_iblock_delay();
  /* [CTAP2.3] section 11.3.7.2 — host relays GETRESPONSE; do not merge SW61xx here. */
  total = reader_security_key_send_apdu_timeout_ex(apdu, apdu_len, rsp, rsp_cap,
                                                   SECURITY_KEY_SHORT_FRAME_MS, false);
  if (reader_security_key_apdu_response_has_status_word(rsp, (total >= 0) ? (uint16_t)total : 0u)) {
  ...
  if (reader_security_key_iso_dep_recover_session()) {
    ...
    total = reader_security_key_send_apdu_timeout_ex(apdu, apdu_len, rsp, rsp_cap,
                                                     SECURITY_KEY_SHORT_FRAME_MS, false);
    if (reader_security_key_apdu_response_has_status_word(rsp,
                                                          (total >= 0) ? (uint16_t)total : 0u)) {
      *rsp_len_out = (uint16_t)total;
      return true;
    }
  }
  ...
  reader_security_key_iso_dep_protocol_settle();
  reader_security_key_iso_dep_pre_first_iblock_delay();
  for (uint8_t i = 0u; i < (uint8_t)NFC_CTAP_FIDO_APP_SELECT_VARIANT_COUNT; ++i) {
    ...
    if (reader_security_key_select_app_ex(variant.aid, variant.aid_len, "FIDO/FIDO2", variant.p2,
                                          variant.add_le_00, false, false, rsp, rsp_cap,
                                          rsp_len_out)) {
      return true;
    }
  }
  return *rsp_len_out >= kReaderSecurityKeyRelayMinResponseLen;
}
...
uint16_t reader_security_key_apdu_exchange(const uint8_t *apdu, uint16_t apdu_len, uint8_t *rsp,
  ...
  if (reader_security_key_apdu_is_select_fido_aid(apdu, apdu_len, NERO_NFC_NULL)) {
  ...
  is_ctap = reader_security_key_apdu_fido_ctap_command(apdu, apdu_len, &ctap_cmd);
  exchange_timeout_ms =
    is_ctap ? reader_security_key_ctap_timeout_for_command(
                ctap_cmd, kReaderSecurityKeyRelayExchangeTimeoutMs, SECURITY_KEY_SHORT_FRAME_MS)
            : kReaderSecurityKeyRelayExchangeTimeoutMs;
#if defined(NERO_CCID_USB_BUILD)
  reader_security_key_iso_dep_set_last_error(0u);
#endif
  /* Transparent APDU relay — host owns GET RESPONSE / NFCCTAP_GETRESPONSE polling. */
  total = reader_security_key_send_apdu_timeout_ex(apdu, apdu_len, rsp, rsp_cap,
                                                   exchange_timeout_ms, false);
  if ((total < (int)NFC_ISO7816_SW_STATUS_WORD_LEN) && is_ctap) {
    uint16_t select_len = 0u;

    if (reader_security_key_iso_dep_recover_session()) {
      reader_security_key_iso_dep_post_recover_rf_settle();
      if (reader_security_key_select_fido_for_host_webauthn(rsp, rsp_cap, &select_len, false)) {
        reader_hal_delay_ms(kReaderSecurityKeyRelayPostFidoSelectMs);
        total = reader_security_key_send_apdu_timeout_ex(apdu, apdu_len, rsp, rsp_cap,
                                                         exchange_timeout_ms, false);
      }
    }
  }
  /* [CTAP2.3] §11.3.5.2 — relay the token's response (including its ISO7816 status
   * word) verbatim; never synthesize or rewrite a status word. Fail closed with
   * a clean ISO7816 error only when the token response is malformed or carries a
   * contradictory/error status word. 61xx/91xx are forwarded so the host owns
   * GET RESPONSE / NFCCTAP_GETRESPONSE. */
  if (is_ctap) {
    if (nfc_ctap_relay_response_disposition(rsp, (total >= 0) ? (uint16_t)total : 0u) ==
        NFC_CTAP_RELAY_FAIL_CLOSED) {
      return security_key_relay_failure_response(rsp, rsp_cap);
    }
    return (uint16_t)total;
  }
  if (total < (int)NFC_ISO7816_SW_STATUS_WORD_LEN) {
    return security_key_relay_failure_response(rsp, rsp_cap);
  }
  return (uint16_t)total;
}
