...
/*
 * reader_tags_type2.c — NFC Forum Type 2 tag read path.
 */
...
enum { READER_TAGS_TYPE2_FAST_READ_RESP_BYTES_MAX = 192u };
...
int reader_tags_ntag_read_page(uint8_t page, uint8_t* buffer) {
  ...
  return reader_protocol_transceive14(
      cmd, NFC_TAG_T2T_READ_CMD_LEN, buffer, NFC_TAG_T2T_READ_RESP_BYTES, true,
      NFC_TAG_T2T_READ_TIMEOUT_MS, false, false);
}

int reader_tags_ntag_fast_read(uint8_t first_page, uint8_t last_page,
  ...
  const uint16_t chunk_page_limit =
      (uint16_t)(READER_TAGS_TYPE2_FAST_READ_RESP_BYTES_MAX /
                 NFC_TAG_T2T_PAGE_SIZE_BYTES);

  if ((buffer == NERO_NFC_NULL) || (last_page < first_page)) {
    return -1;
  }
  page_count = (uint16_t)((unsigned)last_page - (unsigned)first_page + 1u);
  expected =
      (uint16_t)((unsigned)page_count * (unsigned)NFC_TAG_T2T_PAGE_SIZE_BYTES);
  if ((expected == 0u) || (expected > buffer_len)) {
    return -1;
  }
  while (page <= (uint16_t)(last_page)) {
    uint16_t remaining_pages =
        (uint16_t)((unsigned)last_page - (unsigned)page + 1u);
    uint16_t chunk_pages = (remaining_pages > chunk_page_limit)
                               ? chunk_page_limit
                               : remaining_pages;
    uint8_t chunk_last = (uint8_t)((unsigned)page + (unsigned)chunk_pages - 1u);
    uint16_t chunk_len = (uint16_t)((unsigned)chunk_pages *
                                    (unsigned)NFC_TAG_T2T_PAGE_SIZE_BYTES);
    const uint8_t fast_read_request[NFC_TAG_T2T_FAST_READ_CMD_LEN] = {
        NFC_FRONTEND_NTAG_CMD_FAST_READ, (uint8_t)(page), chunk_last};
    int rlen = reader_protocol_transceive14(
        fast_read_request, NFC_TAG_T2T_FAST_READ_CMD_LEN, &buffer[copied],
        chunk_len, true, NFC_TAG_T2T_READ_TIMEOUT_MS, false, false);
...
bool reader_tags_ntag_write_page(uint8_t page, const uint8_t* data) {
  ...
  /*
   * [T2T-ISO14443-A] section 5.2 — WRITE is acknowledged only by the 4-bit ACK
   * 1010b (0Ah).
   */
  return (rlen >= 1) &&
  ...
  if (info->cc_valid && (info->data_area_size_bytes != 0u)) {
    const uint16_t pages_from_cc =
        (uint16_t)(NFC_TAG_T2T_CC_PAGE_INDEX +
                   ((info->data_area_size_bytes + NFC_TAG_T2T_PAGE_SIZE_BYTES -
                     1u) /
                    NFC_TAG_T2T_PAGE_SIZE_BYTES));

    return (pages_from_cc > NFC_TAG_NTAG216_LAST_PAGE)
               ? NFC_TAG_NTAG216_LAST_PAGE
               : pages_from_cc;
  }
...
static void reader_tags_parse_type2_tlv(const uint8_t* data, uint16_t len) {
  ...
  if (nero_nfc_u8_at(data, (size_t)(len), (size_t)(0)) != NFC_FORUM_CC_MAGIC) {
    return;
  }
...
static bool reader_tags_type2_tlv_complete(const uint8_t* data, uint16_t len) {
  ...
  if ((data == NERO_NFC_NULL) || !nero_nfc_span_ok(0u, 1u, len) ||
      (len <= READER_TYPE2_CC_PREFIX_BYTES) ||
      (nero_nfc_u8_at(data, (size_t)(len), (size_t)(0)) !=
       NFC_FORUM_CC_MAGIC)) {
    return false;
  }
  if (nfc_ndef_find_message_tlv(data, len, READER_TYPE2_CC_PREFIX_BYTES,
                                &tlv) == NFC_NDEF_TLV_OK) {
    return true;
  }
...
void reader_tags_read_ntag_tag(void) {
  ...
  if (!reader_tags_get_typea_info(&typea) ||
      !reader_tags_get_type2_info(&type2)) {
  ...
  if (!type2.cc_valid || (type2.data_area_size_bytes == 0u)) {
    nero_nfc_log_line(
        "  Type 2 CC invalid or empty data area; cannot parse NDEF");
    return;
  }
  ...
  if ((type2.family == NFC_TAG_TYPE2_FAMILY_NTAG21X) &&
    ...
    if (reader_tags_ntag_read_page(NFC_TAG_T2T_CC_PAGE_INDEX, first_read) >=
      ...
      if (nero_nfc_copy_bytes(tag_data, sizeof(tag_data), 0u, first_read,
                              sizeof(first_read))) {
        total = NFC_TAG_T2T_READ_RESP_BYTES;
      }
      const uint16_t needed_pages =
          reader_tags_type2_needed_pages(tag_data, total, max_page);
      if (needed_pages > NFC_TAG_T2T_READ_RESP_PAGES) {
        const uint16_t fast_len =
            (uint16_t)(needed_pages * NFC_TAG_T2T_PAGE_SIZE_BYTES);
        if ((fast_len <= sizeof(tag_data)) &&
            (reader_tags_ntag_fast_read(
                 NFC_TAG_T2T_CC_PAGE_INDEX,
      ...
      if (rlen < (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
        nero_nfc_log_write("  Read failed at page ");
        {
          ...
          (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u",
                                  (unsigned)((uint32_t)(page)));
          nero_nfc_log_write(ndc);
        }
        nero_nfc_log_write("\r\n");
        break;
      }
      if ((uint16_t)((unsigned)page + (unsigned)NFC_TAG_T2T_READ_RESP_PAGES -
                     1u) > max_page) {
        pages_to_copy = (uint8_t)((unsigned)max_page - (unsigned)page + 1u);
      }
  ...
  if (total >= NFC_TAG_T2T_MIN_NDEF_DUMP_BYTES) {
    ...
    if (total > NFC_TAG_T2T_READ_RESP_BYTES) {
      nfc_tutorial_t2t_fast_read_cmd(
          ...
          (uint8_t)(NFC_TAG_T2T_CC_PAGE_INDEX +
                    ((total / NFC_TAG_T2T_PAGE_SIZE_BYTES) - 1u)));
    }
    nfc_tutorial_t2t_cc(nero_nfc_log_putc, tag_data);
    nfc_tutorial_ndef_tlv(nero_nfc_log_putc,
                          &tag_data[READER_TYPE2_CC_PREFIX_BYTES],
