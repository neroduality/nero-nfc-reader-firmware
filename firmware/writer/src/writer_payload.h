 ...
 */
enum writer_payload_kind_t : int {
...
NERO_NFC_STATIC_ASSERT(WRITER_CLI_LINE_CAP == ((WRITER_NDEF_MAX_BYTES * 2u) + 16u),
...
enum {
  ...
  WRITER_VCARD_STR2_FIELDS = 2u,
};

/* vCard MIME text assembly (FN + TEL + EMAIL + fixed lines). */
#define WRITER_VCARD_TEMPLATE_OVERHEAD 66u
#define WRITER_VCARD_TEXT_MAX                                                                      \
  (WRITER_STR1_MAX + (WRITER_STR2_MAX * WRITER_VCARD_STR2_FIELDS) + WRITER_VCARD_TEMPLATE_OVERHEAD)

/* Wi-Fi WSC credential blob scratch (see build_wifi_wsc in writer_payload.cpp). */
enum {
...
struct writer_payload_config_t {
  writer_payload_kind_t kind;
  /* str1/str2 interpretation depends on `kind` (see writer_serial_cli.cpp). */
...
NERO_NFC_NODISCARD bool writer_payload_configured(const writer_payload_config_t *cfg);
