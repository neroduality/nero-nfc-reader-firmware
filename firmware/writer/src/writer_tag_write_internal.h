...
extern uint8_t writer_tag_t4_blk;
extern bool writer_tag_t4_pcb_has_cid;
extern uint8_t writer_tag_t4_cid;
extern bool writer_tag_t4_need_first_iblock_delay;

NERO_NFC_NODISCARD bool writer_collect_type2_info(writer_type2_family_t fam,
...
typedef bool (*writer_tag_storage_write_unit_fn_t)(uint16_t unit, const uint8_t *data,
                                                   uint8_t data_len);

NERO_NFC_NODISCARD static inline bool
writer_tag_write_storage_tlv_units(const uint8_t *tlv, uint16_t tlv_len, uint16_t first_unit,
  ...
  for (uint16_t unit_index = 0u; unit_index < units_needed; unit_index++) {
    ...
    if ((chunk != 0u) && !nero_nfc_copy_bytes(unit_data, sizeof(unit_data), 0u, tlv + off, chunk)) {
      return false;
    }
    if (!write_unit(unit, unit_data, unit_size)) {
      ...
      do {
        ...
        (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u", (unsigned)(uint32_t)(unit));
