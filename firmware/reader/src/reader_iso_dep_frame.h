...
static inline uint16_t reader_iso_dep_crc_a(const uint8_t *buf, uint16_t len) {
  ...
  for (uint16_t i = 0u; i < len; i++) {
    ...
    crc = (uint16_t)((crc >> ISO_DEP_BYTE_SHIFT_8) ^ ((uint16_t)b << ISO_DEP_BYTE_SHIFT_8) ^
                     ((uint16_t)b << ISO_DEP_CRC_MUL_SHL) ^ ((uint16_t)b >> ISO_DEP_BYTE_SHIFT_4));
  }
  return crc;
}

NERO_NFC_NODISCARD static inline bool reader_iso_dep_has_crc_a_suffix(const uint8_t *frame,
                                                                      uint16_t len) {
  uint16_t crc;
  ...
  return (frame[len - ISO_DEP_CRC_LEN] == (uint8_t)(crc & ISO_DEP_BYTE_MASK_LOW)) &&
         (frame[len - ISO_DEP_HDR_BASE_LEN] == (uint8_t)(crc >> ISO_DEP_BYTE_SHIFT_8));
}

static inline uint8_t reader_iso_dep_chained_crc_tail_len(uint8_t pcb, const uint8_t *frame,
                                                          uint16_t len) {
  if (frame == NERO_NFC_NULL) {
    return 0u;
  }
  if (((pcb & ISO_DEP_PCB_CHAIN_BIT) == 0u) || !reader_iso_dep_has_crc_a_suffix(frame, len)) {
    return 0u;
  }
...
static inline uint8_t reader_iso_dep_sw16_confidence(uint8_t s1, uint8_t s2) {
  ...
  if ((s1 == (uint8_t)NFC_ISO7816_SW1_SUCCESS) && (s2 == (uint8_t)NFC_ISO7816_SW2_SUCCESS)) {
...
static inline uint16_t reader_iso_dep_trim_crc_suffix(const uint8_t *buf, uint16_t len) {
  ...
  tail_confidence = reader_iso_dep_sw16_confidence(buf[len - ISO_DEP_INF_STATUS_TAIL_OFFSET],
                                                   buf[len - ISO_DEP_HDR_BASE_LEN]);
  before_tail_confidence = reader_iso_dep_sw16_confidence(
    buf[len - ISO_DEP_INF_STATUS_BEFORE_TAIL_OFFSET], buf[len - ISO_DEP_INF_STATUS_BEFORE_OFFSET]);
  ...
  if (((buf[len - ISO_DEP_INF_STATUS_BEFORE_TAIL_OFFSET] == (uint8_t)NFC_ISO7816_SW1_MORE_DATA) ||
       ((buf[len - ISO_DEP_INF_STATUS_BEFORE_TAIL_OFFSET] == (uint8_t)NFC_ISO7816_SW1_SUCCESS) &&
        (buf[len - ISO_DEP_INF_STATUS_BEFORE_OFFSET] == (uint8_t)NFC_ISO7816_SW2_SUCCESS))) &&
      (buf[len - ISO_DEP_INF_STATUS_TAIL_OFFSET] != (uint8_t)NFC_ISO7816_SW1_MORE_DATA)) {
...
NERO_NFC_NODISCARD static inline bool reader_iso_dep_inf_first_plausible_select_resp(uint8_t b) {
  if ((b == (uint8_t)NFC_ISO7816_SW1_MORE_DATA) || (b == (uint8_t)NFC_ISO7816_SW1_WARNING_MIN) ||
      (b == ISO_DEP_SELECT_RESP_TAG_D) || (b == (uint8_t)NFC_ISO7816_SW1_WRONG_LENGTH_ALT) ||
      (b == (uint8_t)NFC_ISO7816_SW1_GENERAL_ERROR) || (b == ISO_DEP_SELECT_RESP_TAG_S) ||
      (b == NFC_CBOR_HEADER_ONE_BYTE_TEXT) || (b == (uint8_t)NFC_ISO7816_SW1_SUCCESS)) {
    return true;
  }
  if ((b >= ISO_DEP_SELECT_RESP_SW1_MIN) && (b <= ISO_DEP_SELECT_RESP_SW1_MAX)) {
    return true;
  }
  return false;
}

static inline uint8_t reader_iso_dep_pick_inf_offset(const uint8_t *rx, int rlen, bool pcb_has_cid,
                                                     uint8_t cid, bool have_tc, uint8_t tc_byte) {
  uint8_t pcb;
  uint8_t nstd;
  ...
  if (((pcb & ISO_DEP_PCB_CLASS_MASK) != ISO_DEP_PCB_CLASS_I_BLOCK) || !pcb_has_cid) {
    return nstd;
  }

  if ((cid != 0u) && (rlen >= ISO_DEP_MIN_TRIM_LEN) && ((pcb & ISO_DEP_PCB_CID_BIT) == 0u) &&
      (rx[ISO_DEP_HDR_BASE_LEN] == cid)) {
    return ISO_DEP_CID_HDR_OFFSET;
  }

  if (have_tc && ((tc_byte & ISO_DEP_BLOCK_NUM_MASK) != 0u) &&
      ((pcb & ISO_DEP_PCB_CID_BIT) != 0u) && ((pcb & ISO_DEP_PCB_NAD_BIT) == 0u) &&
      ((pcb & ISO_DEP_PCB_CHAIN_BIT) == 0u) && (nstd >= ISO_DEP_CID_HDR_OFFSET) &&
      (rlen >= (int)nstd + ISO_DEP_INF_MIN_FRAME_LEN) &&
      (rlen <= (int)ISO_DEP_SELECT_RESP_PROBE_MAX_LEN)) {
    uint8_t fa = rx[nstd];
    uint8_t fb = rx[(uint8_t)(nstd + ISO_DEP_HDR_BASE_LEN)];

    if ((fa <= ISO_DEP_SELECT_RESP_LEN_MAX) && reader_iso_dep_inf_first_plausible_select_resp(fb)) {
...
static inline uint8_t reader_iso_dep_rx_nad_byte(const uint8_t *rx, int rlen, uint8_t pcb,
                                                 uint8_t default_nad) {
  ...
  if (((pcb & ISO_DEP_PCB_NAD_BIT) != 0u) && (rx != NERO_NFC_NULL) && (rlen > (int)nad_idx)) {
    return rx[nad_idx];
  }
  return default_nad;
}

static inline uint16_t reader_iso_dep_wtx_response_timeout_ms(uint32_t fwt_us, const uint8_t *rx,
                                                              int rlen, uint8_t hdr_skip) {
  uint32_t base_ms = (fwt_us + ISO_DEP_US_TO_MS_ROUND_UP) / (ISO_DEP_US_TO_MS_ROUND_UP + 1u);
  uint32_t wait_ms;
...
NERO_NFC_NODISCARD static inline bool reader_iso_dep_i_block_inf_len(const uint8_t *rx, int rlen,
                                                                     uint8_t inf_off, uint8_t pcb,
                                                                     uint16_t *inf_len_out,
                                                                     uint8_t *crc_tail_len_out) {
...
NERO_NFC_NODISCARD static inline bool reader_iso_dep_rx_rblock_block(const uint8_t *rx, int rlen,
                                                                     bool expect_nak,
                                                                     bool pcb_has_cid, uint8_t cid,
                                                                     uint8_t *block_out) {
  uint8_t pcb;
  uint8_t need_hdr;
...
reader_iso_dep_rx_is_chain_ack_for_block(const uint8_t *rx, int rlen, uint8_t blk_sent,
                                         bool pcb_has_cid, uint8_t cid, uint8_t *ack_block_out) {
  uint8_t block = 0u;

  if (!reader_iso_dep_rx_rblock_block(rx, rlen, false, pcb_has_cid, cid, &block)) {
    return false;
  }
  /* [ISO14443-4] §7.5.4 Rule 8 — during PCD chaining the PICC acknowledges a chained
   * I-block with an R(ACK) whose block number equals the received I-block's block
   * number (== blk_sent). Require exactly that; do not tolerate the toggled value. */
...
NERO_NFC_NODISCARD static inline bool
reader_iso_dep_rx_is_chain_nak_for_block(const uint8_t *rx, int rlen, uint8_t blk_sent,
                                         bool pcb_has_cid, uint8_t cid) {
  uint8_t block = 0u;

  if (!reader_iso_dep_rx_rblock_block(rx, rlen, true, pcb_has_cid, cid, &block)) {
    return false;
  }
  return (block == (blk_sent & ISO_DEP_BLOCK_NUM_MASK)) ||
         (block == ((blk_sent ^ ISO_DEP_BLOCK_NUM_MASK) & ISO_DEP_BLOCK_NUM_MASK));
}

NERO_NFC_NODISCARD static inline bool
reader_iso_dep_build_chained_ack(uint8_t *ack_out, uint8_t ack_cap, uint8_t block_num,
                                 bool pcb_has_cid, uint8_t cid, bool include_nad, uint8_t nad,
                                 uint8_t *ack_len_out) {
  uint8_t ack_pcb = (uint8_t)(ISO_DEP_PCB_R_BLOCK_BASE | (block_num & ISO_DEP_BLOCK_NUM_MASK));
  ...
  *ack_len_out = 0u;
  ...
  if (include_nad) {
    ack_pcb = (uint8_t)(ack_pcb | ISO_DEP_PCB_NAD_BIT);
  }
...
NERO_NFC_NODISCARD static inline bool
reader_iso_dep_apdu_response_has_status_word(const uint8_t *rsp, uint16_t rsp_len) {
  if ((rsp == NERO_NFC_NULL) || (rsp_len < NFC_ISO7816_SW_STATUS_WORD_LEN)) {
    return false;
  }
  return (rsp[rsp_len - ISO_DEP_INF_STATUS_TAIL_OFFSET] == NFC_ISO7816_SW1_SUCCESS) ||
         ((rsp[rsp_len - ISO_DEP_INF_STATUS_TAIL_OFFSET] == NFC_ISO7816_SW1_MORE_DATA_VENDOR) &&
          (rsp[rsp_len - ISO_DEP_HDR_BASE_LEN] == NFC_ISO7816_SW2_SUCCESS)) ||
         ((rsp[rsp_len - ISO_DEP_INF_STATUS_TAIL_OFFSET] >= NFC_ISO7816_SW1_MORE_DATA) &&
          (rsp[rsp_len - ISO_DEP_INF_STATUS_TAIL_OFFSET] <= NFC_ISO7816_SW1_GENERAL_ERROR));
}
