...
static const uint8_t K_CC54[54] = {
    54,   0x21, 0x10, 0x01, 0x00, 0x07, 0x02, 0x00, 0x00, 0x00, 0xF8,
    0x34, 0x00, 0x00, 0xF8, 0x34, 0x00, 0x00, 0x00, 0x10, 0x9E, 0x01,
    0x00, 0x80, 0xF0, 0x0C, 0x00, 0x00, 0xFE, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xBA, 0x04, 0x04, 0x00,
...
size_t nero_ccid_copy_interface_descriptor_blob_with_endpoints_cap(
  ...
  /* Standard interface — class 11 smart card CCID — three endpoints. Bulk
   * IN/OUT share endpoint number 2 (distinct directions); the interrupt IN uses
   * a separate address. */
  *p++ = NERO_USB_DESC_LEN_INTERFACE;
  *p++ = NERO_USB_DESC_TYPE_INTERFACE;
  *p++ = (uint8_t)(interface_number);
  *p++ = 0;
  *p++ = NERO_USB_CCID_INTERFACE_NUM_ENDPOINTS;
  *p++ = NERO_USB_CLASS_SMART_CARD;
  *p++ = 0;
  *p++ = 0;
  *p++ = (uint8_t)(ccid_string_index & NERO_USB_LOW_BYTE_MASK);

  if (!nero_nfc_copy_bytes(dst, dst_cap, (size_t)(p - dst), K_CC54,
                           sizeof(K_CC54))) {
    return 0u;
  }
  p += sizeof(K_CC54);
  ...
  *p++ = (uint8_t)(NERO_CCID_BULK_EPSIZE & NERO_USB_LOW_BYTE_MASK);
  *p++ = (uint8_t)((NERO_CCID_BULK_EPSIZE >> NFC_CCID_U32_SHIFT_BYTE1) &
  ...
  *p++ = (uint8_t)(NERO_CCID_INT_EPSIZE & NERO_USB_LOW_BYTE_MASK);
  *p++ = (uint8_t)((NERO_CCID_INT_EPSIZE >> NFC_CCID_U32_SHIFT_BYTE1) &
                   NERO_USB_LOW_BYTE_MASK);
...
size_t nero_ccid_copy_interface_descriptor_blob(uint8_t* dst,
  ...
  return nero_ccid_copy_interface_descriptor_blob_with_endpoints_cap(
      dst, NERO_CCID_CFG_DESC_TOTAL, interface_number, ccid_string_index,
      NERO_CCID_EP_INT, NERO_CCID_EP_OUT, NERO_CCID_EP_IN);
}
