...
nfc_mode_scan_result_t nfc_mode_scan_feed(nfc_mode_scan_state_t *st, uint8_t ch, uint8_t *pushback,
  ...
  static const char kModePrefix[] = "mode ";

  if ((st == NERO_NFC_NULL) || (pushback == NERO_NFC_NULL) || (push_len == NERO_NFC_NULL)) {
  ...
  *push_len = 0u;
  ...
  if (ch == '\n') {
    const bool is_reader = (st->len == NFC_MODE_SCAN_CMD_LEN) &&
                           (memcmp(st->buf, "mode reader", NFC_MODE_SCAN_CMD_LEN) == 0);
    const bool is_writer = (st->len == NFC_MODE_SCAN_CMD_LEN) &&
  ...
  if (st->len < NFC_MODE_SCAN_PREFIX_LEN) {
    if (ch != (uint8_t)kModePrefix[st->len]) {
      st->len++;
      if (st->len > pushback_cap) {
        st->len = 0u;
        return NFC_MODE_SCAN_PUSHBACK_STOP;
