...
/* [T5T-ISO15693] section 4.2 — ISO/IEC 15693 user block size is
 ...
 * spans the NTAG216 user area (NTAG21x datasheet: 888 user bytes) plus
 * headroom; parsed CC MLEN and per-IC page caps still bound real tag access.
 */
...
#define READER_TYPE2_RAW_READ_MAX \
  (READER_TYPE2_CC_PREFIX_BYTES + READER_TYPE2_DATA_AREA_READ_MAX)
/* Page-3..max_page dump buffer for interactive NTAG reads (see
 * type2_max_read_page). */
...
reader_tags_type2_write_attempt_needs_activation(uint8_t attempt) {
  /* Attempt zero continues the already-active transaction. Reactivation is a
   * recovery step performed only before a retry, never after each write. */
  return attempt != 0u;
}

static inline uint16_t reader_tags_type5_cc_byte_len(
  ...
  return (type5->cc[NFC_TAG_T5T_CC_MLEN_BYTE_INDEX] == 0u)
             ? NFC_TAG_T5T_CC_LEN_EXTENDED
             : NFC_TAG_T5T_CC_LEN_SHORT;
}

static inline uint16_t reader_tags_type5_tlv_start_block(
    const nfc_tag_type5_info_t* type5) {
...
#define READER_TAGS_NDEF_BUF (reader_context_active()->ndef_buf)
...
NERO_NFC_NODISCARD bool reader_tags_ntag_write_page(uint8_t page,
                                                    const uint8_t* data);
int reader_tags_iso15693_user_read_multiple(uint16_t first_block,
                                            uint16_t block_count, uint8_t* buf,
                                            uint16_t buf_cap);
NERO_NFC_NODISCARD bool reader_tags_type4_load_info(nfc_tag_type4_info_t* info,
                                                    uint8_t* ndef_file_hi_out,
                                                    uint8_t* ndef_file_lo_out,
                                                    bool log_errors);
...
int reader_tags_iso15693_user_read(uint16_t block, uint8_t* buf,
                                   uint8_t buf_len);
NERO_NFC_NODISCARD bool reader_tags_iso15693_user_write(uint16_t block,
                                                        const uint8_t* data,
                                                        uint8_t data_len);
