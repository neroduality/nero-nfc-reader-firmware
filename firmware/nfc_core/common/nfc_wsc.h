 ...
 * Reference: Wi-Fi Alliance "Wi-Fi Simple Configuration Technical Specification"
 * (data element / attribute IDs, WSC section 12).
 *
 * Single source of truth shared by the firmware writer (writer_payload.cpp) and
 * the host writer (nero_nfc_writer_payload.cpp) so both binaries emit byte-for-byte
 * identical credentials and cannot drift apart.
 */
enum {
