...
enum {
  READER_OUTPUT_URL_HTTPS_PREFIX_LEN = 8u,
  READER_OUTPUT_URL_HTTP_PREFIX_LEN = 7u,
};
...
bool reader_is_browser_safe_url(const char* url) {
  ...
  return (strncmp(url, "https://", READER_OUTPUT_URL_HTTPS_PREFIX_LEN) == 0) ||
         (strncmp(url, "http://", READER_OUTPUT_URL_HTTP_PREFIX_LEN) == 0);
}
