...
void reader_ccid_reply_slot_stat(uint8_t *buf10, uint8_t seq8, uint8_t icclvl, uint8_t err_code) {
  nfc_ccid_encode_slot_status(buf10, RDR_PC_SLOTSTATUS, seq8, icclvl, err_code);
}

void reader_ccid_reply_data_block_error(uint8_t *buf10, uint8_t seq8, uint8_t icclvl,
                                        uint8_t err_code) {
  /* [CCID1.10] Rev 1.10 section 6.2.1 — the response to PC_to_RDR_IccPowerOn (and
   * other DataBlock-bearing commands) is always RDR_to_PC_DataBlock, even on
   * failure: dwLength=0, bStatus carries bmCommandStatus=failed, bError is set. */
  nfc_ccid_encode_slot_status(buf10, RDR_PC_DATABLOCK, seq8, icclvl, err_code);
}

static void reply_data_preface(uint8_t *buf10, uint8_t seq8, uint32_t databytes, uint8_t chain) {
  nfc_ccid_encode_data_block_header(buf10, RDR_PC_DATABLOCK, seq8, databytes, CCICC_ACTIVE, chain);
}

void reader_ccid_reply_data_preface(uint8_t *buf10, uint8_t seq8, uint32_t databytes,
                                    uint8_t chain) {
  reply_data_preface(buf10, seq8, databytes, chain);
}

static void reply_data_time_extension(uint8_t *buf10, uint8_t seq8) {
  nfc_ccid_encode_data_block_header(buf10, RDR_PC_DATABLOCK, seq8, 0u,
                                    (uint8_t)(CCICC_CMD_TIME_EXTENSION | CCICC_ACTIVE), 0u);
  buf10[NFC_CCID_BULK_LEVEL_PARAM2_OFFSET] = CCICC_TIME_EXTENSION_BWT_MULTIPLIER;
}
...
static void begin_ccid_time_extension(uint8_t seq8, bool send_initial) {
  ...
  reader_security_key_set_ccid_time_extension_callback(send_ccid_time_extension, NERO_NFC_NULL);
  if (send_initial) {
...
void reader_ccid_clear_xfr_response_chain() {
  g_ccid_chain_active = false;
  g_ccid_chain_len = 0u;
  g_ccid_chain_off = 0u;
}

void reader_ccid_clear_xfr_command_chain() {
  g_ccid_cmd_chain_active = false;
  g_ccid_cmd_chain_len = 0u;
}

bool reader_ccid_append_xfr_command_chain(const uint8_t *data, uint16_t len, bool begin_chain) {
  size_t next_len = 0u;
  ...
  if (!nero_nfc_try_add_size((size_t)g_ccid_cmd_chain_len, (size_t)len, &next_len) ||
      next_len > sizeof(g_ccid_cmd_chain_buf) ||
      !nero_nfc_copy_bytes(g_ccid_cmd_chain_buf, sizeof(g_ccid_cmd_chain_buf), g_ccid_cmd_chain_len,
                           data, len)) {
    reader_ccid_clear_xfr_command_chain();
    return false;
  }
...
uint8_t reader_ccid_current_icc_level() {
  if (!g_card_present) {
    return CCICC_NO_ICC;
  }
...
__attribute__((weak)) bool reader_ccid_prepare_tag_for_power_on(reader_tag_kind_t tag_kind) {
  (void)tag_kind;
  return true;
}

void reader_ccid_reply_command_not_supported(uint8_t *buf10, uint8_t msg_type, uint8_t seq8) {
  nfc_ccid_encode_slot_status(buf10, msg_type, seq8,
...
void reader_ccid_send_xfr_chain_chunk(uint8_t *work, uint8_t seq) {
  ...
  if ((work == NERO_NFC_NULL) || !g_ccid_chain_active || g_ccid_chain_off >= g_ccid_chain_len) {
  ...
  chunk =
    (remaining > (uint16_t)NFC_CCID_RSP_DATA_CAP) ? (uint16_t)NFC_CCID_RSP_DATA_CAP : remaining;
  ...
  (void)reader_hal_ccid_send(work, (uint16_t)(NFC_CCID_BULK_HEADER_LEN + chunk),
                             (uint32_t)NFC_CCID_HAL_SEND_TIMEOUT_MS);
...
void reader_ccid_send_xfr_data_response(uint8_t *work, uint8_t seq, const uint8_t *data,
                                        uint16_t len) {
  ...
  if (len <= NFC_CCID_RSP_DATA_CAP) {
    if ((len != 0u) &&
        !nero_nfc_copy_bytes(work, sizeof(g_ccid_rsp), NFC_CCID_BULK_HEADER_LEN, data, len)) {
      const uint8_t err[] = {(uint8_t)NFC_ISO7816_SW1_GENERAL_ERROR,
                             (uint8_t)NFC_ISO7816_SW2_SUCCESS};
      if (!nero_nfc_copy_bytes(work, sizeof(g_ccid_rsp), NFC_CCID_BULK_HEADER_LEN, err,
                               (uint16_t)sizeof(err))) {
        return;
      }
      reply_data_preface(work, seq, (uint32_t)sizeof(err), NFC_CCID_XFR_LEVEL_SINGLE);
      (void)reader_hal_ccid_send(work, (uint16_t)(NFC_CCID_BULK_HEADER_LEN + sizeof(err)),
                                 (uint32_t)NFC_CCID_HAL_SEND_TIMEOUT_MS);
      return;
    }
  ...
  if (len > (uint16_t)sizeof(g_ccid_chain_buf) ||
      !nero_nfc_copy_bytes(g_ccid_chain_buf, sizeof(g_ccid_chain_buf), 0u, data, len)) {
    uint8_t err[] = {(uint8_t)NFC_ISO7816_SW1_GENERAL_ERROR, (uint8_t)NFC_ISO7816_SW2_SUCCESS};
    if (!nero_nfc_copy_bytes(work, sizeof(g_ccid_rsp), NFC_CCID_BULK_HEADER_LEN, err,
                             (uint16_t)sizeof(err))) {
      return;
    }
...
uint8_t reader_ccid_reply_param_status() {
  if (!g_card_present) {
    return CCICC_NO_ICC;
  }
  return g_sess ? CCICC_ACTIVE : CCICC_INACTIVE;
...
void reader_ccid_teardown_session() {
  reader_ccid_clear_xfr_response_chain();
  reader_ccid_clear_xfr_command_chain();
  reader_security_key_set_ccid_time_extension_callback(NERO_NFC_NULL, NERO_NFC_NULL);
...
void reader_ccid_complete_card_removal() {
...
static void send_ccid_time_extension(const void *ctx) {
  ...
  (void)ctx;
  reply_data_time_extension(buf, g_ccid_time_extension_seq);
  (void)reader_hal_ccid_send(buf, (uint16_t)sizeof(buf), (uint32_t)NFC_CCID_HAL_SEND_TIMEOUT_MS);
}
