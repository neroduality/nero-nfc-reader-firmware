...
typedef struct reader_iso_dep_ats_profile_t {
  uint8_t fsci;
...
static inline void reader_iso_dep_ats_profile_reset(
...
static inline uint16_t reader_iso_dep_fsc_from_fsci(uint8_t fsci) {
  ...
  static const uint16_t FSC_TABLE[] = {16u, 24u, 32u,  40u, 48u,
                                       64u, 96u, 128u, 256u};

  return (fsci < READER_ISO_DEP_FSCI_TABLE_LEN)
             ? FSC_TABLE[fsci]
             : (uint16_t)NFC_ISO14443_FSC_MAX;
}

NERO_NFC_NODISCARD static inline bool reader_iso_dep_parse_ats_profile(
  ...
  t0 = nero_nfc_u8_at(ats, ats_len, READER_ISO_DEP_ATS_T0_OFFSET);
  /* [ISO14443-4] section 5.2.5 — ATS T0 low nibble (b4..b1) carries FSCI; the
   * high bits b7/b6/b5 are the TC1/TB1/TA1 presence flags handled below. */
  ...
  if (((t0 & READER_ISO_DEP_ATS_T0_TB_MASK) != 0u) && (pos < ats_len)) {
    profile->has_tb = true;
    profile->tb = ats[pos++];
    profile->fwi = (uint8_t)((profile->tb >> READER_ISO_DEP_ATS_TB_FWI_SHIFT) &
                             READER_ISO_DEP_ATS_TB_FWI_MASK);
  ...
  if (((t0 & READER_ISO_DEP_ATS_T0_TC_MASK) != 0u) && (pos < ats_len)) {
    profile->has_tc = true;
    profile->tc = ats[pos++];
    profile->supports_cid =
        ((profile->tc & READER_ISO_DEP_ATS_TC_CID_BIT) != 0u);
    profile->supports_nad =
        ((profile->tc & READER_ISO_DEP_ATS_TC_NAD_BIT) != 0u);
  }
  if (pos < ats_len) {
