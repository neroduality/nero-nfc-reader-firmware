...
static void reader_tags_type2_refresh_fast_read_cache(void) {
  reader_tag_caps_cache_t* caps = &G_READER.tag_caps;

  if ((caps->type2_fast_read_uid_len == G_UID14_LEN) &&
      ((G_UID14_LEN == 0u) ||
       (memcmp(caps->type2_fast_read_uid, G_UID14, G_UID14_LEN) == 0))) {
    return;
  }
  caps->type2_fast_read_uid_len = G_UID14_LEN;
  caps->type2_fast_read_disabled = false;
  if (G_UID14_LEN != 0u) {
    (void)nero_nfc_copy_bytes(caps->type2_fast_read_uid,
                              sizeof(caps->type2_fast_read_uid), 0u, G_UID14,
                              G_UID14_LEN);
  }
}
...
static bool reader_tags_type2_fast_read_allowed(void) {
  ...
  return !G_READER.tag_caps.type2_fast_read_disabled && (G_UID14_LEN != 0u) &&
         (G_UID14[0] == NFC_TAG_MFR_CODE_NXP);
}

static void reader_tags_type5_refresh_read_multiple_cache(void) {
  reader_tag_caps_cache_t* caps = &G_READER.tag_caps;

  if (memcmp(caps->type5_read_multiple_uid, G_UID15,
             NFC_FRONTEND_ISO15693_UID_LEN) == 0) {
    return;
  }
  caps->type5_read_multiple_disabled = false;
  (void)nero_nfc_copy_bytes(caps->type5_read_multiple_uid,
                            sizeof(caps->type5_read_multiple_uid), 0u, G_UID15,
...
static bool reader_tags_type5_read_multiple_allowed(void) {
  ...
  return !G_READER.tag_caps.type5_read_multiple_disabled;
}
...
static int reader_tags_type2_get_version(uint8_t* out, uint8_t out_cap) {
  ...
  int ver_len = reader_protocol_transceive14(
      cmd, NFC_TAG_T2T_GET_VERSION_CMD_LEN, dst, cap, true,
...
static int reader_tags_type2_read_signature(uint8_t* out, uint8_t out_cap) {
  const uint8_t cmd[NFC_TAG_T2T_READ_CMD_LEN] = {NFC_FRONTEND_NTAG_CMD_READ_SIG,
                                                 0x00u};
  int sig_len = reader_protocol_transceive14(
      cmd, sizeof(cmd), out, out_cap, true, NFC_TAG_T2T_READ_SIG_TIMEOUT_MS,
      false, false);
 ...
 */
static int reader_tags_iso15693_get_system_info_raw(uint8_t* buf,
                                                    uint8_t buf_cap) {
  ...
  if ((buf == NERO_NFC_NULL) || (buf_cap == 0u) ||
      !nero_nfc_span_ok(0u,
                        NFC_TAG_T5T_ISO15693_SYS_INFO_CMD_HEADER_LEN +
                            NFC_FRONTEND_ISO15693_UID_LEN,
                        sizeof(cmd))) {
    return -1;
  }
  ...
  return reader_protocol_iso15693_transceive(
      cmd, n, buf, buf_cap, NFC_TAG_T5T_ISO15693_TRANSCEIVE_TIMEOUT_MS);
}

/*
 * [T5T-ISO15693] section 10.3.4 / [T5T-ISO15693-ST25DV] section 7.6.23 —
 * Extended Get System Info (command 0x3B). Used when [T5T-ISO15693]
 * section 4.3.1.17 MLEN-overflow is signaled in the CC, because standard Get
 * System Info cannot report more than 256 blocks.
 */
static int reader_tags_iso15693_get_system_info_ext_raw(uint8_t* buf,
                                                        uint8_t buf_cap) {
  uint8_t cmd[NFC_TAG_T5T_ISO15693_CMD_BUF_MAX];
  uint8_t n = 0u;
  ...
  if ((buf == NERO_NFC_NULL) || (buf_cap == 0u) ||
      !nero_nfc_span_ok(0u,
                        NFC_TAG_ST25DV_EXT_SYS_INFO_CMD_HEADER_LEN +
                            NFC_FRONTEND_ISO15693_UID_LEN,
                        sizeof(cmd))) {
    return -1;
  }
...
static bool reader_tags_type2_last_page(const reader_tag_type2_info_t* type2,
  ...
  if ((type2 == NERO_NFC_NULL) || (last_page_out == NERO_NFC_NULL) ||
      !type2->cc_valid || (type2->data_area_size_bytes == 0u)) {
    return false;
  }
  return nfc_storage_type2_last_page(type2->data_area_size_bytes,
                                     last_page_out);
}

static bool reader_tags_type2_read_cap(const reader_tag_type2_info_t* type2,
                                       uint16_t* cap_out) {
  uint16_t last_page;
  uint16_t pages;
  ...
  if ((cap_out == NERO_NFC_NULL) ||
      !reader_tags_type2_last_page(type2, &last_page) ||
...
bool reader_tags_get_typea_info(reader_tag_typea_info_t* info) {
  ...
  if (!nero_nfc_copy_bytes(info->uid, sizeof(info->uid), 0u, G_UID14,
                           G_UID14_LEN)) {
    return false;
  }
  ...
  if ((G_ATS_LEN != 0u) && !nero_nfc_copy_bytes(info->ats, sizeof(info->ats),
                                                0u, G_ATS_DATA, G_ATS_LEN)) {
    return false;
  }
...
bool reader_tags_get_type2_info(reader_tag_type2_info_t* info) {
  ...
  nero_nfc_zero_bytes(info, sizeof(*info));
  version_len = reader_tags_type2_get_version(info->version,
                                              (uint8_t)sizeof(info->version));
  if (version_len > 0) {
    info->version_len =
        (uint8_t)((version_len > (int)NFC_TAG_NTAG_VER_REPLY_LEN)
                      ? (int)NFC_TAG_NTAG_VER_REPLY_LEN
                      : version_len);
  ...
  if (info->family == NFC_TAG_TYPE2_FAMILY_NTAG21X) {
    ...
    int sig_len =
        reader_tags_type2_read_signature(signature, sizeof(signature));

    if (sig_len >= (int)sizeof(signature)) {
      nfc_tag_type2_apply_signature(info, signature,
                                    (uint8_t)sizeof(signature));
    }
    if (info->max_user_page != 0u) {
      uint8_t cfg[NFC_TAG_T2T_READ_RESP_BYTES];
      uint8_t auth0_page =
          (uint8_t)(info->max_user_page + NFC_TAG_NTAG_AUTH0_PAGE_OFFSET);
...
bool reader_tags_get_type5_info(reader_tag_type5_info_t* info) {
  ...
  if (page_len >= (int)NFC_TAG_T5T_CC_LEN_SHORT) {
    ...
    const bool ndef_magic = (page0[0] == NFC_FORUM_CC_MAGIC) ||
                            (page0[0] == NFC_T5T_CC_MAGIC_8BYTE);
    const uint16_t declared_cc_len =
        nfc_storage_type5_declared_cc_len_from_first_block(
            page0, (uint16_t)sizeof(page0));
    if (!nero_nfc_copy_bytes(cc, sizeof(cc), 0u, page0, sizeof(page0))) {
      return false;
    }
    if (ndef_magic &&
        declared_cc_len == (uint16_t)NFC_TAG_T5T_CC_LEN_EXTENDED) {
      ...
      if (page1_len >= (int)NFC_TAG_T5T_CC_LEN_SHORT &&
          nero_nfc_copy_bytes(cc, sizeof(cc), NFC_TAG_T5T_CC_LEN_SHORT, page1,
                              sizeof(page1))) {
  ...
  sys_len =
      reader_tags_iso15693_get_system_info_raw(sys_info, sizeof(sys_info));
  ...
  nfc_tag_type5_apply_system_info(
      info, sys_info,
      (uint8_t)((sys_len > (int)sizeof(info->raw)) ? (int)sizeof(info->raw)
                                                   : sys_len));

  if (nfc_tag_type5_cc_signals_mlen_overflow(info)) {
    uint8_t ext_info[NFC_TAG_T5T_ISO15693_SYS_INFO_RAW_BUF_MAX];
    int ext_len = reader_tags_iso15693_get_system_info_ext_raw(
    ...
    if ((ext_len >= (int)NFC_TAG_ST25DV_EXT_SYS_INFO_MIN_REPLY_LEN) &&
      ...
      nfc_tag_type5_apply_system_info_ext(
          info, ext_info,
          (uint8_t)((ext_len > (int)sizeof(info->raw)) ? (int)sizeof(info->raw)
                                                       : ext_len));
    }
  }
...
bool reader_tags_get_tag_info(reader_tag_info_t* info) {
  if (info == NERO_NFC_NULL) {
    return false;
  }
  nero_nfc_zero_bytes(info, sizeof(*info));
  ...
  if (G_UID15[0] != 0u) {
    ...
    return nero_nfc_copy_bytes(info->common.type5_uid,
                               sizeof(info->common.type5_uid), 0u, G_UID15,
                               NFC_FRONTEND_ISO15693_UID_LEN) &&
...
typedef bool (*reader_tags_storage_write_unit_fn_t)(uint16_t unit,
                                                    const uint8_t* data,
                                                    uint8_t data_len);

static bool reader_tags_write_storage_tlv_units(
    const uint8_t* tlv, uint16_t tlv_len, uint16_t first_unit,
    uint8_t unit_size, reader_tags_storage_write_unit_fn_t write_unit) {
...
static bool reader_tags_type2_write_unit(uint16_t unit, const uint8_t* data,
                                         uint8_t data_len) {
  if ((data == NERO_NFC_NULL) || (data_len != NFC_STORAGE_TYPE2_UNIT_SIZE) ||
      (unit > UINT8_MAX)) {
    return false;
  }
...
static bool reader_tags_type5_write_unit(uint16_t unit, const uint8_t* data,
                                         uint8_t data_len) {
...
int reader_tags_load_type2_ndef_message(uint8_t* dst, uint16_t dst_cap) {
  ...
  if ((dst == NERO_NFC_NULL) || (dst_cap == 0u) ||
      !reader_tags_get_type2_storage_info(&type2) || !type2.cc_valid ||
      !type2.read_access_open || (type2.data_area_size_bytes == 0u)) {
    return -1;
  }
  if (!reader_tags_type2_read_cap(&type2, &raw_cap) ||
      !nero_nfc_try_add_u16(READER_TYPE2_CC_PREFIX_BYTES,
                            type2.data_area_size_bytes, &raw_len)) {
    return -1;
  }
  if (raw_len > sizeof(raw)) {
    raw_len = (uint16_t)sizeof(raw);
  }
  raw_len = ((raw_len) < (raw_cap) ? (raw_len) : (raw_cap));
  got = reader_tags_type2_read_binary(NFC_TAG_T2T_CC_PAGE_INDEX, raw, raw_len);
  if (got < (int)NFC_TAG_T2T_PAGE_SIZE_BYTES) {
    return -1;
  }
  if (nfc_ndef_find_message_tlv(raw, (uint16_t)got,
                                READER_TYPE2_CC_PREFIX_BYTES,
                                &tlv) != NFC_NDEF_TLV_OK) {
    return -1;
  }
  ...
  if ((tlv.value_len != 0u) &&
      !nero_nfc_copy_bytes(dst, dst_cap, 0u, raw + tlv.value_offset,
                           tlv.value_len)) {
    return -1;
  }
...
int reader_tags_load_type5_ndef_message(uint8_t* dst, uint16_t dst_cap) {
  ...
  if ((dst == NERO_NFC_NULL) || (dst_cap == 0u) ||
      !reader_tags_get_type5_info(&type5) || !type5.cc_valid ||
  ...
  if (nfc_ndef_find_message_tlv(raw, (uint16_t)got, cc_len, &tlv) !=
      NFC_NDEF_TLV_OK) {
    return -1;
  }
  if (tlv.value_len > dst_cap) {
...
bool reader_tags_write_type2_ndef_message(const uint8_t* ndef,
  ...
  if (((ndef_len != 0u) && (ndef == NERO_NFC_NULL)) ||
      !reader_tags_get_type2_storage_info(&type2) || !type2.cc_valid ||
      !type2.write_access_open || (type2.data_area_size_bytes == 0u)) {
    return false;
  }
  if (!nfc_ndef_build_message_tlv(
          ndef, ndef_len, tlv,
  ...
  if (!nfc_storage_ceil_units_u16(tlv_len, NFC_STORAGE_TYPE2_UNIT_SIZE,
                                  &pages_needed) ||
      (pages_needed == 0u) ||
      !reader_tags_type2_last_page(&type2, &last_page) ||
      !nero_nfc_try_add_size(READER_TYPE2_CC_PREFIX_BYTES,
                             (size_t)pages_needed - 1u, &last_page_needed) ||
      (last_page_needed > last_page)) {
    return false;
  }
  return reader_tags_write_storage_tlv_units(
      tlv, tlv_len, NFC_STORAGE_TYPE2_FIRST_DATA_PAGE,
      NFC_STORAGE_TYPE2_UNIT_SIZE, reader_tags_type2_write_unit);
}
...
bool reader_tags_write_type5_ndef_message(const uint8_t* ndef,
  ...
  if (((ndef_len != 0u) && (ndef == NERO_NFC_NULL)) ||
      !reader_tags_get_type5_info(&type5) || !type5.cc_valid ||
      !type5.write_access_open || (type5.data_area_size_bytes == 0u)) {
    return false;
  }
  block_size =
      (type5.block_size == 0u) ? ST25_T5_USER_BLOCK_SIZE : type5.block_size;
  ...
  cc_len = reader_tags_type5_cc_byte_len(&type5);
  if (!nfc_storage_type5_data_blocks(cc_len, type5.data_area_size_bytes,
                                     block_size, type5.block_count,
                                     &first_data_block, &last_data_block) ||
      !nfc_storage_ceil_units_u16(tlv_len, NFC_STORAGE_TYPE5_BLOCK_SIZE,
                                  &blocks_needed) ||
      (blocks_needed == 0u)) {
    return false;
  }
  start_block = first_data_block;
  if (((uint32_t)start_block + (uint32_t)blocks_needed - 1u) >
      last_data_block) {
    return false;
  }
  return reader_tags_write_storage_tlv_units(tlv, tlv_len, start_block,
                                             NFC_STORAGE_TYPE5_BLOCK_SIZE,
                                             reader_tags_type5_write_unit);
}
...
int reader_tags_type2_transceive(const uint8_t* tx, uint16_t tx_len,
  ...
  rlen = reader_protocol_transceive14(
      tx, tx_len, rx, rx_max, true, NFC_TAG_T2T_READ_TIMEOUT_MS, false, false);
  if ((nero_nfc_u8_at(tx, (size_t)(tx_len), (size_t)(0)) ==
       NFC_FRONTEND_NTAG_CMD_GET_VERSION) &&
      (rlen < (int)NFC_TAG_NTAG_VER_REPLY_LEN)) {
    (void)reader_protocol_activate_iso14443a();
  }
  return rlen;
}
...
bool reader_tags_type2_write_page(uint8_t page, const uint8_t* data) {
  ...
  for (uint8_t attempt = 0u; attempt < NFC_TAG_READER_TYPE2_WRITE_ATTEMPTS;
       ++attempt) {
    uint8_t verify[NFC_TAG_T2T_READ_RESP_BYTES];
    bool write_ack;

    if (reader_tags_type2_write_attempt_needs_activation(attempt) &&
        !reader_protocol_activate_iso14443a()) {
      continue;
    }
    ...
    if ((reader_tags_ntag_read_page(page, verify) >=
         (int)NFC_TAG_T2T_PAGE_SIZE_BYTES) &&
...
int reader_tags_type2_read_binary(uint16_t page, uint8_t* buf,
  ...
  if ((buf_cap >= (uint16_t)NFC_TAG_T2T_READ_RESP_BYTES) &&
      (page <= (uint16_t)UINT8_MAX) && reader_tags_type2_fast_read_allowed()) {
    const uint16_t pages =
        (uint16_t)(buf_cap / (uint16_t)NFC_TAG_T2T_PAGE_SIZE_BYTES);
    const uint16_t fast_len =
        (uint16_t)(pages * (uint16_t)NFC_TAG_T2T_PAGE_SIZE_BYTES);
    const uint16_t last_page =
        (uint16_t)((unsigned)page + (unsigned)pages - 1u);

    if ((pages != 0u) && (last_page <= (uint16_t)UINT8_MAX)) {
      if (reader_tags_ntag_fast_read((uint8_t)page, (uint8_t)last_page, buf,
                                     fast_len) >= (int)fast_len) {
        copied = fast_len;
        page = (uint16_t)(page + pages);
      } else {
        G_READER.tag_caps.type2_fast_read_disabled = true;
      }
    }
  }

  while (copied < buf_cap) {
    uint8_t page_buf[NFC_TAG_T2T_READ_RESP_BYTES];
    uint16_t chunk = (uint16_t)((buf_cap - copied) > (uint16_t)sizeof(page_buf)
                                    ? (uint16_t)sizeof(page_buf)
                                    : (buf_cap - copied));
    int rlen;

    /* [T2T-ISO14443-A] page address is a single byte on the RF wire; refuse to
     * read past the one-byte page space instead of silently wrapping to a low
     * (config) page. */
    ...
    if (!nero_nfc_copy_bytes(buf, buf_cap, copied, page_buf, chunk)) {
      return (copied == 0u) ? -1 : (int)copied;
    }
...
int reader_tags_type5_transceive(const uint8_t* tx, uint16_t tx_len,
  ...
  return reader_protocol_iso15693_transceive(
      tx, tx_len, rx, rx_max, NFC_TAG_T5T_ISO15693_TRANSCEIVE_TIMEOUT_MS);
}
...
bool reader_tags_type5_write_block(uint16_t block, const uint8_t* data,
  ...
  if ((data == NERO_NFC_NULL) || (data_len == 0u) ||
      (data_len > sizeof(verify))) {
    return false;
  }
...
int reader_tags_type5_read_binary(uint16_t block, uint8_t* buf,
  ...
  while (copied < buf_cap) {
    ...
    const uint16_t full_blocks =
        (uint16_t)((buf_cap - copied) / (uint16_t)NFC_STORAGE_TYPE5_BLOCK_SIZE);
    if ((full_blocks > 1u) && reader_tags_type5_read_multiple_allowed()) {
      ...
      const uint16_t chunk_len =
          (uint16_t)(block_count * (uint16_t)NFC_STORAGE_TYPE5_BLOCK_SIZE);

      const uint16_t first_block = block;
      const uint16_t block_count_arg = block_count;
      if (nero_nfc_span_ok(copied, chunk_len, buf_cap) &&
          reader_tags_iso15693_user_read_multiple(first_block, block_count_arg,
                                                  buf + copied, chunk_len) >=
              (int)chunk_len) {
        copied = (uint16_t)(copied + chunk_len);
        block = (uint16_t)(block + block_count);
        continue;
      }
      G_READER.tag_caps.type5_read_multiple_disabled = true;
    }
    uint16_t chunk =
        (uint16_t)((buf_cap - copied) > (uint16_t)ST25_T5_USER_BLOCK_SIZE
                       ? (uint16_t)ST25_T5_USER_BLOCK_SIZE
                       : (buf_cap - copied));
    int rlen =
        reader_tags_iso15693_user_read(block, block_buf, sizeof(block_buf));

    if (rlen < (int)chunk) {
      return (copied == 0u) ? -1 : (int)copied;
    }
    if (!nero_nfc_copy_bytes(buf, buf_cap, copied, block_buf, chunk)) {
      return (copied == 0u) ? -1 : (int)copied;
    }
    copied = (uint16_t)(copied + chunk);
    block++;
