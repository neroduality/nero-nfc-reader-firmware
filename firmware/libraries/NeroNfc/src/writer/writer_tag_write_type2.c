...
/*
 * writer_tag_write_type2.c — NFC Forum Type 2 NDEF write path.
 */
...
enum { WRITER_TYPE2_POST_WRITE_SETTLE_MS = 2u };

static int ntag_read_page(uint8_t page, uint8_t* buf) {
  const uint8_t cmd[NFC_TAG_T2T_READ_CMD_LEN] = {NFC_FRONTEND_NTAG_CMD_READ,
                                                 page};
  if (buf == NERO_NFC_NULL) {
    return -1;
  }
  return writer_rf_transceive14(cmd, NFC_TAG_T2T_READ_CMD_LEN, buf,
...
static int type2_get_version(uint8_t* out, uint8_t out_cap) {
  ...
  int ver_len =
      writer_rf_transceive14(cmd, NFC_TAG_T2T_GET_VERSION_CMD_LEN, dst, cap,
...
static int type2_read_signature(uint8_t* out, uint8_t out_cap) {
  const uint8_t cmd[NFC_TAG_T2T_READ_CMD_LEN] = {NFC_FRONTEND_NTAG_CMD_READ_SIG,
                                                 0x00u};
  int sig_len =
      writer_rf_transceive14(cmd, sizeof(cmd), out, out_cap, true,
...
bool writer_collect_type2_info(writer_type2_family_t fam,
                               nfc_tag_type2_info_t* type2_out) {
  ...
  if (WRITER_APP_TAG_VERSION_LEN != 0u) {
    nfc_tag_type2_apply_version(type2_out, WRITER_APP_TAG_VERSION,
                                WRITER_APP_TAG_VERSION_LEN);
  } else if (fam == WRITER_TYPE2_FAMILY_NTAG21X) {
    int version_len = type2_get_version(type2_out->version,
                                        (uint8_t)(sizeof(type2_out->version)));
    if (version_len > 0) {
      type2_out->version_len =
          (uint8_t)((version_len > (int)(NFC_TAG_NTAG_VER_REPLY_LEN))
                        ? (int)(NFC_TAG_NTAG_VER_REPLY_LEN)
                        : version_len);
      nfc_tag_type2_apply_version(type2_out, type2_out->version,
                                  type2_out->version_len);
      (void)nero_nfc_copy_bytes(WRITER_APP_TAG_VERSION,
                                sizeof(WRITER_APP_TAG_VERSION), 0u,
  ...
  if (page_len >= (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
    ...
    (void)nero_nfc_copy_bytes(WRITER_APP_T2_CC_PAGE3,
                              sizeof(WRITER_APP_T2_CC_PAGE3), 0u, page3,
                              sizeof(WRITER_APP_T2_CC_PAGE3));
    WRITER_APP_T2_CC_FROM_TAG_VALID = true;
  } else if (WRITER_APP_T2_CC_FROM_TAG_VALID) {
    nfc_tag_type2_apply_cc(type2_out, WRITER_APP_T2_CC_PAGE3,
                           sizeof(WRITER_APP_T2_CC_PAGE3));
  }

  if ((fam == WRITER_TYPE2_FAMILY_ST25TN) ||
      ((type2_out->family == NFC_TAG_TYPE2_FAMILY_UNKNOWN) &&
       (WRITER_APP_UID14_LEN != 0u) &&
  ...
  if ((fam == WRITER_TYPE2_FAMILY_NTAG21X) ||
      (type2_out->family == NFC_TAG_TYPE2_FAMILY_NTAG21X)) {
    ...
    if (sig_len >= (int)(sizeof(signature))) {
      nfc_tag_type2_apply_signature(type2_out, signature,
                                    (uint8_t)(sizeof(signature)));
    }
    if (type2_out->max_user_page != 0u) {
      uint8_t cfg[NFC_TAG_T2T_READ_RESP_BYTES];
      uint8_t auth0_page =
          (uint8_t)(type2_out->max_user_page + NFC_TAG_NTAG_AUTH0_PAGE_OFFSET);
      int cfg_len = ntag_read_page(auth0_page, cfg);

      if (cfg_len >= (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
        nfc_tag_type2_apply_auth0(type2_out,
                                  cfg[NFC_TAG_NTAG_CFG_AUTH0_BYTE_INDEX]);
      }
    }
  }
  return true;
...
static bool ntag_write_page(uint8_t page, const uint8_t* data) {
  if ((data == NERO_NFC_NULL) ||
      !nero_nfc_span_ok(NFC_TAG_T2T_PAGE_BYTE3, 1u,
  ...
  cmd[1] = page;
  if (!nero_nfc_copy_bytes(cmd, sizeof(cmd), NFC_TAG_T2T_WRITE_CMD_DATA_OFFSET,
                           data, NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
    return false;
  }
  uint8_t rx[NFC_TAG_T2T_PAGE_SIZE_BYTES];
  int rlen = writer_rf_transceive14(
      cmd, NFC_TAG_T2T_WRITE_CMD_LEN, rx, sizeof(rx), true,
      NFC_TAG_T2T_WRITE_ACK_TIMEOUT_MS, false, true);
  /*
   * [T2T-ISO14443-A-NTAG21x] section 10.4 Table 34 — TTimeOut for WRITE ACK/NAK
   * (10 ms). [T2T-ISO14443-A] section 5.2 — WRITE is confirmed only by the
   * 4-bit ACK 1010b (0Ah).
   */
  return (rlen >= 1) &&
         ((rx[0] & NFC_TAG_T2T_ACK_NIBBLE_MASK) == NFC_TAG_T2T_ACK_NIBBLE);
}
...
static bool writer_type2_write_unit(uint16_t unit, const uint8_t* data,
                                    uint8_t data_len) {
 ...
 * Tries command 0x60; ST25TN-class ICs ignore it (see [T2T-ISO14443-A-ST25TN] —
 * no GET_VERSION).
 */
static bool ntag_try_get_version(void) {
  ...
  int len = writer_rf_transceive14(&cmd, NFC_TAG_T2T_GET_VERSION_CMD_LEN, rx,
                                   sizeof(rx), true,
                                   NFC_TAG_T2T_READ_TIMEOUT_MS, false, false);
  ...
  if (version_info.vendor_id != NFC_TAG_NTAG_VER_VENDOR_NXP) {
    nero_nfc_log_write("  NXP NTAG21x (GET_VERSION vendor byte 0x");
    {
      ...
      (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X",
                              (unsigned)(version_info.vendor_id));
  ...
  switch (version_info.size_id) {
    case NFC_TAG_NTAG213_SIZE_ID:
      nero_nfc_log_write(
          "    GET_VERSION byte 6 = 0x0F (common on NTAG213; not "
          "authoritative alone)\r\n");
      break;
  ...
  nero_nfc_log_write(
      "    Geometry follows CC page 3 MLEN when readable (may disagree on "
      "non-NXP tags)\r\n");
  return true;
}
...
static void serial_ntag21x_tier_hint_from_cc_mlen(uint8_t mlen) {
  ...
  {
    char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
    (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X", (unsigned)(mlen));
    nero_nfc_log_write(nhx);
  }
...
static void serial_ntag_version_cc_mismatch_hint(uint8_t cc_mlen) {
  if ((WRITER_APP_TAG_VERSION_LEN < NFC_TAG_NTAG_VER_REPLY_LEN) ||
      !writer_type2_is_ntag21x_version_reply(
          WRITER_APP_TAG_VERSION, (int)(WRITER_APP_TAG_VERSION_LEN))) {
    return;
  }
  uint8_t b6 = writer_type2_ntag21x_size_id(WRITER_APP_TAG_VERSION,
  ...
  if (ver_cap != cc_cap) {
    nero_nfc_log_write(
        "  Note: GET_VERSION byte 6 disagrees with CC MLEN — using CC for "
        "limits\r\n");
  }
}
...
static uint8_t t2_cc_size(writer_type2_family_t fam) {
  return writer_type2_cc_size(
      fam, WRITER_APP_T2_CC_FROM_TAG_VALID, WRITER_APP_T2_CC_PAGE3,
      WRITER_APP_TAG_VERSION_LEN, WRITER_APP_TAG_VERSION);
}

static uint16_t t2_max_page(writer_type2_family_t fam) {
  return writer_type2_max_page(
      fam, WRITER_APP_T2_CC_FROM_TAG_VALID, WRITER_APP_T2_CC_PAGE3,
      WRITER_APP_TAG_VERSION_LEN, WRITER_APP_TAG_VERSION);
}
...
bool writer_tag_write_type2_impl(writer_type2_family_t fam) {
  ...
  uint16_t total =
      writer_payload_build_tlv(&WRITER_APP_PAYLOAD, tlv, sizeof(tlv));
  ...
  if (type2_info.password_protected) {
    nero_nfc_log_line(
        "  ERROR: tag reports Type 2 password protection enabled");
    return false;
  }

  cc_size_byte = t2_cc_size(fam);
  need_mlen = (uint8_t)((total + NFC_TAG_T2T_AREA_SIZE_UNIT_BYTES - 1u) /
                        NFC_TAG_T2T_AREA_SIZE_UNIT_BYTES);
  cc_size_byte = NERO_NFC_MAX(need_mlen, cc_size_byte);

  max_pg = t2_max_page(fam);
  pages_needed = (uint16_t)((total + NFC_TAG_T2T_PAGE_SIZE_BYTES - 1u) /
                            NFC_TAG_T2T_PAGE_SIZE_BYTES);
  if (!nero_nfc_try_add_size(NFC_STORAGE_TYPE2_FIRST_DATA_PAGE,
  ...
  {
    const uint8_t cc_bytes[NFC_TAG_T2T_PAGE_SIZE_BYTES] = {
        NFC_FORUM_CC_MAGIC, NFC_T2T_CC_VERSION, cc_size_byte, 0x00u};
    nero_nfc_log_line("\r\n  ── Byte-level explainer (write) ──");
    nfc_tutorial_t2t_cc(nero_nfc_log_putc, cc_bytes);
    nfc_tutorial_t2t_write_cmd(nero_nfc_log_putc,
                               NFC_STORAGE_TYPE2_FIRST_DATA_PAGE, tlv);
   ...
   */
  {
    ...
    bool needs_cc = (r < (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) ||
                    (rb[0] != NFC_FORUM_CC_MAGIC) ||
                    (rb[NFC_TAG_T2T_CC_MLEN_INDEX] < cc_size_byte);
    if (needs_cc) {
      const uint8_t cc_page[NFC_TAG_T2T_PAGE_SIZE_BYTES] = {
          NFC_FORUM_CC_MAGIC, NFC_T2T_CC_VERSION, cc_size_byte, 0x00u};
      nero_nfc_log_line("  Writing CC...");
      if (!ntag_write_page(NFC_TAG_T2T_CC_PAGE_INDEX, cc_page)) {
        nero_nfc_log_line(
            "  CC write FAILED (CC bits are OTP — already locked?)");
        return false;
      }
  ...
  if (!writer_tag_write_storage_tlv_units(
          tlv, total, NFC_STORAGE_TYPE2_FIRST_DATA_PAGE,
          NFC_STORAGE_TYPE2_UNIT_SIZE, "Page", writer_type2_write_unit)) {
    return false;
  }
...
writer_type2_family_t writer_tag_write_identify_type2_family(void) {
  ...
  if (ntag_try_get_version()) {
    ...
    if ((cr >= (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) &&
        (rb[0] == NFC_FORUM_CC_MAGIC)) {
      WRITER_APP_T2_CC_PAGE3[0] = rb[0];
      WRITER_APP_T2_CC_PAGE3[1] = rb[1];
      WRITER_APP_T2_CC_PAGE3[NFC_TAG_T2T_CC_MLEN_INDEX] =
          rb[NFC_TAG_T2T_CC_MLEN_INDEX];
      WRITER_APP_T2_CC_PAGE3[NFC_TAG_T2T_CC_ACCESS_INDEX] =
          rb[NFC_TAG_T2T_CC_ACCESS_INDEX];
      WRITER_APP_T2_CC_FROM_TAG_VALID = true;
      nero_nfc_log_write("  CC page 3: ");
      {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X", (unsigned)(rb[0]));
        nero_nfc_log_write(nhx);
      }
      nero_nfc_log_putc(' ');
      {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X", (unsigned)(rb[1]));
        nero_nfc_log_write(nhx);
      }
      nero_nfc_log_putc(' ');
      {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X",
                                (unsigned)(rb[NFC_TAG_T2T_CC_MLEN_INDEX]));
        nero_nfc_log_write(nhx);
      }
      nero_nfc_log_putc(' ');
      {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X",
                                (unsigned)(rb[NFC_TAG_T2T_CC_ACCESS_INDEX]));
        nero_nfc_log_write(nhx);
      }
      nero_nfc_log_write("\r\n");
      if ((WRITER_APP_TAG_VERSION_LEN >= NFC_TAG_NTAG_VER_REPLY_LEN) &&
  ...
  writer_hal_delay_ms(WRITER_TYPE2_POST_WRITE_SETTLE_MS);
  ...
  if ((WRITER_APP_UID14_LEN >= 1u) &&
      (WRITER_APP_UID14[0] == NFC_TAG_MFR_CODE_ST)) {
    ...
    if (rlen >= (int)(NFC_TAG_T2T_PAGE_SIZE_BYTES)) {
      if (cc[0] == NFC_FORUM_CC_MAGIC) {
        nero_nfc_log_write(
            "  ST25TN-class tag (CC magic 0xE1, STMicroelectronics "
            "manufacturer code "
            "0x02)\r\n");
      ...
      {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X", (unsigned)(cc[0]));
        nero_nfc_log_write(nhx);
      }
