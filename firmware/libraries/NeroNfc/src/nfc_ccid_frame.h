...
enum {
  ...
  NFC_CCID_RSP_DATA_CAP =
      510u, /* 520-byte response chunk minus 10-byte CCID header */
  ...
  NFC_CCID_BULK_LEVEL_PARAM_TAIL_LEN = 2u,
  ...
  /* [DERIVED] Little-endian byte helpers; intentionally not standalone SPEC
     entries. */
...
NERO_NFC_STATIC_ASSERT(NFC_CCID_BULK_HEADER_LEN == 10u,
                       "CCID bulk header is 10 bytes");
...
/* Interrupt-IN NotifySlotChange bmSlotICCState (slot 0) per CCID 1.1 §6.3.1:
 * bit0 = current state (0=absent, 1=present); bit1 = changed (0=no, 1=yes). */
enum {
  NFC_CCID_NOTIFY_SLOT_PRESENT_CHANGED = 0x03u, /* present + changed */
  NFC_CCID_NOTIFY_SLOT_ABSENT_CHANGED = 0x02u,  /* absent + changed */
};

uint32_t nfc_ccid_u32_load_le(const uint8_t* src);
void nfc_ccid_u32_store_le(uint8_t* dst, uint32_t value);

NERO_NFC_NODISCARD bool nfc_ccid_bulk_frame_validate(const uint8_t* frame,
                                                     uint16_t nbytes,
                                                     uint32_t* data_len_out);

NERO_NFC_NODISCARD bool nfc_ccid_xfr_payload_needs_time_extension(
    const uint8_t* payload, uint16_t payload_len);
NERO_NFC_NODISCARD bool nfc_ccid_xfr_frame_requests_response_continuation(
...
NERO_NFC_NODISCARD static inline bool nfc_ccid_control_abort_request_matches(
    ...
    uint16_t w_index, uint16_t w_length, uint8_t interface_number,
    uint8_t* slot_out, uint8_t* seq_out) {
  ...
  if (bm_request_type != NFC_CCID_CONTROL_BMREQ_ABORT ||
      b_request != NFC_CCID_CONTROL_ABORT ||
  ...
  if (seq_out != NERO_NFC_NULL) {
    *seq_out =
        (uint8_t)((w_value >> NFC_CCID_U32_SHIFT_BYTE1) & NFC_CCID_BYTE_MASK);
  }
  return true;
}

NERO_NFC_NODISCARD static inline bool
nfc_ccid_control_abort_request_matches_slot(
    uint8_t bm_request_type, uint8_t b_request, uint16_t w_value,
    uint16_t w_index, uint16_t w_length, uint8_t interface_number,
    uint8_t max_slot_index, uint8_t* slot_out, uint8_t* seq_out) {
  uint8_t slot = 0u;
  uint8_t seq = 0u;
  ...
  if (!nfc_ccid_control_abort_request_matches(bm_request_type, b_request,
                                              w_value, w_index, w_length,
                                              interface_number, &slot, &seq) ||
      (slot > max_slot_index)) {
    return false;
  }
  if (slot_out != NERO_NFC_NULL) {
