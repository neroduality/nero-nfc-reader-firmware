...
/*
 * reader_tags_type5.cpp — ISO15693 Type 5 tag read path.
 */
...
enum {
  kReaderType5ReadMultipleCmdMax = 14u,
  kReaderType5ReadMultipleShortFieldLen = 2u,
  kReaderType5ReadMultipleExtFieldLen = 4u,
};

} /* namespace */

static bool reader_tags_type5_cmd_append(uint8_t *cmd, uint16_t cmd_cap, uint16_t *pos,
                                         const uint8_t *src, uint16_t len) {
  if ((cmd == NERO_NFC_NULL) || (pos == NERO_NFC_NULL) || (src == NERO_NFC_NULL)) {
    return false;
  }
...
bool reader_tags_iso15693_inventory_step_impl(void) {
  return nfc_frontend_iso15693_inventory(reader_protocol_iso15693_transceive, g_uid15);
}

/*
 * Reads a single user-memory block across Type 5 ICs we support.
 * [T5T-ISO15693-ST25TV] DS13304 section 6.4.3 — ReadSingleBlock (opcode 0x20, 1-byte address).
 * [T5T-ISO15693-ST25DV] DS13519 section 7.6.6 — Read Single Block accepts 0x20 for blocks 0..0xFF;
 * extended 0x30 form is used above NFC_TAG_T5T_ISO15693_BLOCK_ADDR_1BYTE_MAX.
 */
int reader_tags_iso15693_user_read(uint16_t block, uint8_t *buf, uint8_t buf_len) {
  if (block <= NFC_TAG_T5T_ISO15693_BLOCK_ADDR_1BYTE_MAX) {
    int r = nfc_frontend_iso15693_read_block(reader_protocol_iso15693_transceive, g_uid15,
  ...
  return nfc_frontend_iso15693_ext_read_block(reader_protocol_iso15693_transceive, g_uid15, block,
                                              buf, buf_len);
}

int reader_tags_iso15693_user_read_multiple(uint16_t first_block, uint16_t block_count,
                                            uint8_t *buf, uint16_t buf_len) {
  uint8_t cmd[kReaderType5ReadMultipleCmdMax];
  uint8_t rx[1u + (NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX * ST25_T5_USER_BLOCK_SIZE)];
  uint16_t want;
  ...
  if ((buf == NERO_NFC_NULL) || (block_count == 0u) ||
      (block_count > (uint16_t)NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX) ||
  ...
  extended = (first_block > (uint16_t)NFC_TAG_T5T_ISO15693_BLOCK_ADDR_1BYTE_MAX) ||
             (count_field > (uint16_t)NFC_TAG_T5T_ISO15693_BLOCK_ADDR_1BYTE_MAX);
  const uint8_t flags = extended ? (uint8_t)(NFC_TAG_T5T_ISO15693_FLAGS_ADDRESSED |
                                             NFC_TAG_T5T_ISO15693_FLAG_PROTOCOL_EXTENSION)
                                 : (uint8_t)NFC_TAG_T5T_ISO15693_FLAGS_ADDRESSED;
  const uint8_t opcode = extended ? (uint8_t)NFC_TAG_T5T_ISO15693_CMD_EXT_READ_MULTIPLE
                                  : (uint8_t)NFC_TAG_T5T_ISO15693_CMD_READ_MULTIPLE;
  uint8_t uid_lsb[NFC_FRONTEND_ISO15693_UID_LEN];
  uint8_t block_fields[kReaderType5ReadMultipleExtFieldLen];
  ...
  if (!reader_tags_type5_cmd_append(cmd, sizeof(cmd), &n, uid_lsb, sizeof(uid_lsb))) {
    return -1;
  }
  if (extended) {
    ...
    block_fields[kReaderType5ReadMultipleShortFieldLen] =
      (uint8_t)(count_field & NFC_ISO7816_LOW_BYTE_MASK);
    block_fields[kReaderType5ReadMultipleShortFieldLen + 1u] =
      (uint8_t)(count_field >> NFC_ISO7816_U16_HIGH_BYTE_SHIFT);
    if (!reader_tags_type5_cmd_append(cmd, sizeof(cmd), &n, block_fields,
                                      (uint16_t)kReaderType5ReadMultipleExtFieldLen)) {
      return -1;
    }
  } else {
    ...
    if (!reader_tags_type5_cmd_append(cmd, sizeof(cmd), &n, block_fields,
                                      (uint16_t)kReaderType5ReadMultipleShortFieldLen)) {
      return -1;
    }
  }

  rlen = reader_protocol_iso15693_transceive(cmd, n, rx, (uint16_t)sizeof(rx),
                                             NFC_TAG_T5T_ISO15693_TRANSCEIVE_TIMEOUT_MS);
  if ((rlen < (int)(1u + want)) ||
      ((rx[0] & (uint8_t)NFC_TAG_T5T_ISO15693_RESP_FLAG_ERROR) != 0u)) {
    return -1;
  }
  if (!nero_nfc_copy_bytes(buf, buf_len, 0u, &rx[1], want)) {
    return -1;
  }
  return (int)want;
}

bool reader_tags_iso15693_user_write(uint16_t block, const uint8_t *data, uint8_t data_len) {
  if (block <= NFC_TAG_T5T_ISO15693_BLOCK_ADDR_1BYTE_MAX) {
    bool ok = nfc_frontend_iso15693_write_block(reader_protocol_iso15693_transceive, g_uid15,
                                                (uint8_t)block, data, data_len);
    if (ok) {
      return true;
    }
  }
  return nfc_frontend_iso15693_ext_write_block(reader_protocol_iso15693_transceive, g_uid15, block,
                                               data, data_len);
}
 ...
 * found inside the TLV payload. Uses the shared reader_tags_ndef_buf static buffer.
 */
static bool read_type5_tlv_area(const reader_tag_type5_info_t *type5, uint16_t *total_out,
  ...
  if ((type5 == NERO_NFC_NULL) || (total_out == NERO_NFC_NULL) ||
      (start_offset_out == NERO_NFC_NULL) || !type5->cc_valid || !type5->read_access_open ||
      (type5->data_area_size_bytes == 0u)) {
    return false;
  }
  start_offset = reader_tags_type5_cc_byte_len(type5);
  if (!nero_nfc_try_add_u16(start_offset, type5->data_area_size_bytes, &bytes_to_read)) {
    return false;
  }
  ...
  blocks_needed =
    (uint16_t)((bytes_to_read + (ST25_T5_USER_BLOCK_SIZE - 1u)) / ST25_T5_USER_BLOCK_SIZE);

  reader_tags_reset_detected_urls();

  for (uint16_t block = 0u; block < blocks_needed;) {
    uint8_t tmp[NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX * ST25_T5_USER_BLOCK_SIZE];
    const uint16_t remaining_blocks = (uint16_t)(blocks_needed - block);
    const uint16_t blocks =
      (remaining_blocks > (uint16_t)NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX)
        ? (uint16_t)NFC_TAG_T5T_ISO15693_READ_MULTIPLE_BLOCKS_MAX
        : remaining_blocks;
    const uint16_t chunk_len = (uint16_t)(blocks * (uint16_t)ST25_T5_USER_BLOCK_SIZE);
...
void reader_tags_read_dynamic_or_static_type5(void) {
  ...
  if (reader_tags_get_type5_info(&type5)) {
    nfc_tag_print_type5_debug(nero_nfc_log_putc, g_uid15, NFC_FRONTEND_ISO15693_UID_LEN, &type5);
  }

  if (read_type5_tlv_area(&type5, &total, &start_offset) && total >= start_offset) {
    nero_nfc_log_line("\r\n  ── Byte-level explainer (read-back) ──");
    {
      const uint16_t blocks_read = (uint16_t)((total + (uint16_t)ST25_T5_USER_BLOCK_SIZE - 1u) /
                                              (uint16_t)ST25_T5_USER_BLOCK_SIZE);
      if (blocks_read > 1u) {
        nfc_tutorial_t5t_read_multiple_cmd(nero_nfc_log_putc, g_uid15, 0u, blocks_read);
      } else {
        nfc_tutorial_t5t_read_cmd(nero_nfc_log_putc, g_uid15, 0u);
      }
    }
    nfc_tutorial_t5t_cc(nero_nfc_log_putc, reader_tags_ndef_buf, (uint8_t)start_offset);
    nfc_tutorial_ndef_tlv(nero_nfc_log_putc, &reader_tags_ndef_buf[start_offset],
