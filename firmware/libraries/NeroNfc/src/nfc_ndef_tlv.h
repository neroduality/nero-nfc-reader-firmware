...
enum {
  ...
  /* T2T Op Spec 1.1 §2.3 RQ_T2T_MEM_014: one-byte L is 00h–FEh (not FFh). */
  NFC_NDEF_TLV_ONE_BYTE_LEN_MAX = 254u,
...
static inline uint16_t nfc_ndef_tlv_max_payload_for_data_area(
...
static inline nfc_ndef_tlv_status_t nfc_ndef_tlv_next(
    const uint8_t* data, uint16_t data_len, uint16_t start_offset,
    nfc_ndef_tlv_t* tlv_out, uint16_t* next_offset_out) {
  ...
  if ((data == NERO_NFC_NULL) || (tlv_out == NERO_NFC_NULL) ||
      (next_offset_out == NERO_NFC_NULL) || (start_offset > data_len)) {
  ...
  while (pos < data_len) {
    ...
    if (data[pos] == NFC_NDEF_TLV_EXTENDED_LEN) {
      if (!nero_nfc_span_ok((size_t)pos, NFC_NDEF_TLV_EXTENDED_LEN_FIELD_BYTES,
                            data_len)) {
        return NFC_NDEF_TLV_TRUNCATED;
      }
      len =
...
static inline nfc_ndef_tlv_status_t nfc_ndef_find_message_tlv(
    const uint8_t* data, uint16_t data_len, uint16_t start_offset,
  ...
  while (pos < data_len) {
    ...
    nfc_ndef_tlv_status_t status =
        nfc_ndef_tlv_next(data, data_len, pos, tlv_out, &next);
...
NERO_NFC_NODISCARD static inline bool nfc_ndef_build_message_tlv(
    const uint8_t* ndef, uint16_t ndef_len, uint8_t* out, uint16_t out_cap,
    uint16_t* out_len) {
  ...
  if ((ndef_len != 0u) &&
      !nero_nfc_copy_bytes(out, out_cap, pos, ndef, ndef_len)) {
    return false;
  }
