...
enum {
  NFC_NDEF_HDR_IL =
      0x08u, /* ID Length present — decode-only; not needed by encode callers */
  NFC_NDEF_HDR_CF =
      0x20u, /* Chunk Flag — decode-only; chunked records are rejected here */
...
NERO_NFC_STATIC_ASSERT(
    (sizeof(NFC_NDEF_URI_PREFIXES) / sizeof(NFC_NDEF_URI_PREFIXES[0])) ==
        NERO_NFC_URI_PREFIX_CODE_COUNT,
    "URI prefix table must match NFC Forum RTD URI 1.0 code count");

bool nfc_ndef_decode_uri_payload(const uint8_t* payload, uint32_t len,
  ...
  }
  {
    ...
    if (!nero_nfc_bounded_strlen(prefix, NERO_NFC_NDEF_DECODE_OUT_MAX,
                                 &prefix_len)) {
      return false;
    }
  ...
  for (uint32_t i = 1u; i < len; ++i) {
    if ((payload[i] == 0u) || !nfc_ndef_url_safe_char(payload[i]) ||
        (pos + 1u) >= out_cap) {
      out[0] = '\0';
      return false;
    }
    /* [RFC3986] §2.1 — a percent sign must introduce a "%" HEXDIG HEXDIG
     * triplet. */
    if (payload[i] == (uint8_t)'%') {
      if (((i + NFC_NDEF_URI_PCT_LO_OFFSET) >= len) ||
...
bool nfc_ndef_decode_text_payload(const uint8_t* payload, uint32_t len,
                                  char* out, uint16_t out_cap) {
  static const char K_UTF16_TEXT_RECORD[] = "UTF-16 text record";
  uint8_t lang_len;
  uint16_t pos = 0u;
  size_t text_start = 0u;

  if ((payload == NERO_NFC_NULL) || (out == NERO_NFC_NULL) || (out_cap == 0u) ||
      (len < 1u)) {
    return false;
  }
  out[0] = '\0';
  ...
  if ((payload[0] & NFC_NDEF_TEXT_STATUS_UTF16) != 0u) {
    return nero_nfc_copy_bytes(out, out_cap, 0u, K_UTF16_TEXT_RECORD,
                               (uint16_t)sizeof(K_UTF16_TEXT_RECORD));
  }
  if (!nero_nfc_try_add_size(1u, (size_t)lang_len, &text_start) ||
      (text_start > len)) {
    return false;
  }
...
nfc_ndef_record_status_t nfc_ndef_record_next(const uint8_t* msg,
                                              uint16_t msg_len, uint16_t pos,
                                              nfc_ndef_record_t* rec_out,
                                              uint16_t* next_pos_out) {
  uint8_t hdr;
  bool sr;
  bool il;
  uint8_t type_len;
  ...
  if ((msg == NERO_NFC_NULL) || (rec_out == NERO_NFC_NULL) ||
      (next_pos_out == NERO_NFC_NULL) || (pos > msg_len)) {
  ...
  if (sr) {
    ...
    type_offset =
        il ? NFC_NDEF_RECORD_SR_TYPE_OFFSET_IL : NFC_NDEF_RECORD_SR_TYPE_OFFSET;
  } else {
    ...
    type_offset = il ? NFC_NDEF_RECORD_LONG_TYPE_OFFSET_IL
                     : NFC_NDEF_RECORD_LONG_TYPE_OFFSET;
  }
  ...
  if (il) {
    ...
    if (!nero_nfc_try_add_size((size_t)pos, (size_t)(type_offset - 1u),
                               &id_len_pos) ||
        !nero_nfc_span_ok(id_len_pos, 1u, msg_len)) {
      return NFC_NDEF_RECORD_TRUNCATED;
    }
    id_len = msg[id_len_pos];
    if (!nero_nfc_try_add_size((size_t)off, (size_t)id_len, &next_off) ||
        (next_off > UINT16_MAX)) {
      return NFC_NDEF_RECORD_TRUNCATED;
    }
