...
enum {
  kReaderSecurityKeyAbortSw1 = (uint8_t)NFC_CCID_MSG_PC_TO_RDR_SECURE,
  kReaderSecurityKeyAbortSw2 = 0x17u,
  kReaderSecurityKeyDebugApduPreviewLen = (uint16_t)NFC_ISO7816_SELECT_AID_MAX,
  kReaderSecurityKeySelectApduBufLen = 24u,
  kReaderSecurityKeySelectRespBufLen = (uint16_t)NFC_ISO14443_ATS_MAX,
};

bool reader_security_key_select_app_ex(const uint8_t *aid, uint8_t aid_len, const char *name,
                                       uint8_t p2, bool add_le_00, bool verbose,
                                       bool follow_get_response, uint8_t *rsp_out, uint16_t rsp_cap,
                                       uint16_t *rsp_len_out) {
  uint8_t apdu[kReaderSecurityKeySelectApduBufLen];
  uint8_t resp[kReaderSecurityKeySelectRespBufLen];
  int rlen;
  bool ok;
  uint16_t di;
  ...
  for (;;) {
    ...
    if (reader_security_key_ccid_abort_pending()) {
      if ((rsp_out != NERO_NFC_NULL) && (rsp_len_out != NERO_NFC_NULL) &&
        ...
        rsp_out[0] = kReaderSecurityKeyAbortSw1;
        rsp_out[1] = kReaderSecurityKeyAbortSw2;
        *rsp_len_out = NFC_ISO7816_SW_STATUS_WORD_LEN;
      }
      return false;
    }
    rlen =
      reader_security_key_send_apdu_timeout_ex(apdu, apdu_len, resp, (uint16_t)sizeof(resp),
                                               SECURITY_KEY_SHORT_FRAME_MS, follow_get_response);
    ok = nfc_iso7816_response_sw_ok(resp, rlen);
    if (ok || tried_cid_reopen || !reader_security_key_iso_dep_probe_can_upgrade_cid() ||
        (rlen >= (int)NFC_ISO7816_SW_STATUS_WORD_LEN)) {
      break;
    }
    if (verbose) {
      nero_nfc_log_write("    ");
      nero_nfc_log_write(name);
      nero_nfc_log_write(": probe session gave no response; reopening ISO-DEP with CID "
                         "fallback\r\n");
    }
  ...
  if (!verbose) {
    if ((rlen >= (int)NFC_ISO7816_SW_STATUS_WORD_LEN) && (rsp_out != NERO_NFC_NULL) &&
  ...
  if (ok) {
  ...
  } else if (rlen >= (int)NFC_ISO7816_SW_STATUS_WORD_LEN) {
    uint8_t sw1 = 0u;
    uint8_t sw2 = 0u;
    ...
    do {
      ...
      (void)nero_nfc_snprintf(ndc, sizeof ndc, "%u", (unsigned)(uint32_t)(g_iso_dep_last_inf_off));
    ...
    for (di = 0u; di < (uint16_t)g_iso_dep_raw_rx_len; di++) {
      do {
        char nhx[NERO_NFC_FORMAT_SNPRINTF_HEX_BYTE_CAP];
        (void)nero_nfc_snprintf(nhx, sizeof nhx, "%02X", (unsigned)(uint8_t)(g_iso_dep_raw_rx[di]));
        nero_nfc_log_write(nhx);
      } while (0);
      nero_nfc_log_putc(' ');
    }
    nero_nfc_log_write("apdu_rx=");
    for (di = 0u; di < (uint16_t)rlen && di < kReaderSecurityKeyDebugApduPreviewLen; di++) {
...
bool reader_iso_dep_select_app(const uint8_t *aid, uint8_t aid_len, const char *name) {
  return reader_security_key_select_app_ex(aid, aid_len, name, 0x00u, true, true, true);
}
...
bool reader_iso_dep_select_ndef_app(void) {
  for (uint8_t i = 0u; i < (uint8_t)NFC_PCSC_NDEF_APP_SELECT_VARIANT_COUNT; ++i) {
    ...
    if (reader_security_key_select_app_ex(variant.aid, variant.aid_len, "NDEF", variant.p2,
                                          variant.add_le_00, true, true)) {
      return true;
    }
  }
  return false;
...
bool reader_security_key_select_fido_probe(bool verbose, bool follow_get_response, uint8_t *rsp_out,
  ...
  for (uint8_t i = 0u; i < (uint8_t)NFC_CTAP_FIDO_APP_SELECT_VARIANT_COUNT; ++i) {
    ...
    if (reader_security_key_select_app_ex(variant.aid, variant.aid_len, "FIDO/FIDO2", variant.p2,
                                          variant.add_le_00, verbose, follow_get_response, rsp_out,
                                          rsp_cap, rsp_len_out)) {
      return true;
    }
  }
