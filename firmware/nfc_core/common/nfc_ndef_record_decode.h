 ...
 * decode path and the firmware/host encode paths so the wire layout never drifts. */
enum {
...
NERO_NFC_NODISCARD bool nfc_ndef_decode_uri_payload(const uint8_t *payload, uint32_t len, char *out,
                                                    uint16_t out_cap);

NERO_NFC_NODISCARD bool nfc_ndef_decode_text_payload(const uint8_t *payload, uint32_t len,
...
typedef enum {
  ...
  NFC_NDEF_RECORD_UNSUPPORTED = 4, /* [NDEF] intentional scope limit: CF/chunked; no reassembly */
#ifdef __cplusplus
};
#else
...
NERO_NFC_NODISCARD nfc_ndef_record_status_t nfc_ndef_record_next(const uint8_t *msg,
