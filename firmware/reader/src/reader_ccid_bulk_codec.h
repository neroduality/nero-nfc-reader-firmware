...
/* USB CCID Rev 1.10 — bulk command classification and failure response encoding. */

static inline uint8_t reader_ccid_response_msg_for_bulk_command(uint8_t msg_type) {
  switch (msg_type) {
...
NERO_NFC_NODISCARD static inline bool reader_ccid_bulk_command_requires_rfu_zero(uint8_t msg_type) {
  switch (msg_type) {
  case (uint8_t)NFC_CCID_MSG_PC_TO_RDR_SLOTSTATUS:
  case (uint8_t)NFC_CCID_MSG_PC_TO_RDR_ICC_POWER_OFF:
  case (uint8_t)NFC_CCID_MSG_PC_TO_RDR_GET_PARAMETERS:
...
static inline uint16_t reader_ccid_bulk_xfr_level_parameter(const uint8_t *frame) {
  if (frame == NERO_NFC_NULL) {
    return 0u;
  }
  return (
    uint16_t)(((uint16_t)frame[NFC_CCID_BULK_LEVEL_PARAM2_OFFSET]) |
              ((uint16_t)frame[NFC_CCID_BULK_LEVEL_PARAM3_OFFSET] << NFC_CCID_U32_SHIFT_BYTE1));
}

static inline uint16_t reader_ccid_encode_command_failed_response(uint8_t *work, size_t work_cap,
                                                                  uint8_t msg_type, uint8_t seq8,
                                                                  uint8_t icclvl,
                                                                  uint8_t err_code) {
  ...
  const auto failed_icclvl = (uint8_t)(NFC_CCID_ICC_CMD_FAIL | icclvl);

  if (rsp_msg == (uint8_t)NFC_CCID_MSG_RDR_TO_PC_DATA_RATE_AND_CLOCK) {
    (void)reader_ccid_encode_data_rate_clock_response(work, work_cap, rsp_msg, seq8, failed_icclvl,
                                                      err_code);
    return (uint16_t)(NFC_CCID_BULK_HEADER_LEN + NFC_CCID_DATA_RATE_CLOCK_PAYLOAD_LEN);
  }
  if ((rsp_msg == (uint8_t)NFC_CCID_MSG_RDR_TO_PC_DATABLOCK) ||
      (rsp_msg == (uint8_t)NFC_CCID_MSG_RDR_TO_PC_ESCAPE)) {
    nfc_ccid_encode_data_block_header(work, rsp_msg, seq8, 0u, failed_icclvl, 0u);
    work[NFC_CCID_BULK_LEVEL_PARAM2_OFFSET] = err_code;
...
static inline uint16_t reader_ccid_encode_slot_absent_response(uint8_t *work, size_t work_cap,
                                                               uint8_t msg_type, uint8_t slot8,
                                                               uint8_t seq8,
                                                               uint8_t slot_missing_error) {
  if (work == NERO_NFC_NULL) {
    return 0u;
  }
  const uint8_t rsp_msg = reader_ccid_response_msg_for_bulk_command(msg_type);
  const auto icclvl = (uint8_t)(NFC_CCID_ICC_CMD_FAIL | NFC_CCID_ICC_NO_ICC);

  if (rsp_msg == (uint8_t)NFC_CCID_MSG_RDR_TO_PC_DATA_RATE_AND_CLOCK) {
    (void)reader_ccid_encode_data_rate_clock_response(work, work_cap, rsp_msg, seq8, icclvl,
                                                      slot_missing_error);
    work[NFC_CCID_BULK_SLOT_OFFSET] = slot8;
    return (uint16_t)(NFC_CCID_BULK_HEADER_LEN + NFC_CCID_DATA_RATE_CLOCK_PAYLOAD_LEN);
  }
