...
/*
 * writer_tag_write_type5.c — ISO15693 Type 5 NDEF write path.
 */
 ...
 */
enum { WRITER_TYPE5_MIN_MLEN_UNITS = 8u };

static const uint8_t WRITER_TYPE5_SHORT_CC_TEMPLATE[NFC_TAG_T5T_CC_LEN_SHORT] =
    {NFC_FORUM_CC_MAGIC, NFC_T5T_CC_VER_ACCESS, 0x00u, 0x00u};

static int iso15693_transceive_glue(void* context, const uint8_t* tx,
                                    uint16_t tx_len, uint8_t* rx,
                                    uint16_t rx_max, uint16_t timeout_ms) {
  if (((tx == NERO_NFC_NULL) && (tx_len != 0u)) || (rx == NERO_NFC_NULL)) {
    return -1;
  }
  return nfc_frontend_iso15693_transceive((nfc_frontend_t*)context, tx, tx_len,
                                          rx, rx_max, timeout_ms);
}

bool writer_tag_write_type5_inventory_step(void) {
  return nfc_frontend_iso15693_inventory(iso15693_transceive_glue,
                                         WRITER_FRONTEND, WRITER_APP_UID15);
}
...
static int iso15693_read(uint8_t block, uint8_t* buf, uint8_t buf_len) {
  return nfc_frontend_iso15693_read_block(iso15693_transceive_glue,
                                          WRITER_FRONTEND, WRITER_APP_UID15,
                                          block, buf, buf_len);
}

static bool iso15693_write(uint8_t block, const uint8_t* data,
                           uint8_t data_len) {
   ...
   */
  (void)nfc_frontend_iso15693_write_block(iso15693_transceive_glue,
                                          WRITER_FRONTEND, WRITER_APP_UID15,
                                          block, data, data_len);
  ...
  for (uint8_t i = 0u; i < data_len; i++) {
    if (!nero_nfc_span_ok((size_t)(i), 1u, (size_t)(data_len)) ||
        rb[i] != data[i]) {
      return false;
    }
  }
...
static bool writer_type5_write_unit(uint16_t unit, const uint8_t* data,
                                    uint8_t data_len) {
  if ((data == NERO_NFC_NULL) || (data_len != NFC_STORAGE_TYPE5_BLOCK_SIZE) ||
      (unit > UINT8_MAX)) {
    return false;
  }
 ...
 *
 * CC magic E1h is the NFC Forum one-byte block-address mode per [T5T-ISO15693]
 * section 4.3.1. MLEN counts T5T_Area size in NFC_TAG_T5T_AREA_SIZE_UNIT_BYTES
 * units. GetSystemInfo block count when available comes from [T5T-ISO15693]
 * section 10.3.4 / [T5T-ISO15693-ST25DV] section 7.6.23 (ST25TV02KC per
 * [T5T-ISO15693-ST25TV] DS13304 does not implement it).
 */
bool writer_tag_write_type5_impl(void) {
  ...
  uint8_t cc_block[NFC_TAG_T5T_CC_LEN_EXTENDED] = {NFC_FORUM_CC_MAGIC,
                                                   NFC_T5T_CC_VER_ACCESS,
                                                   0x00u,
                                                   0x00u,
                                                   0x00u,
                                                   0x00u,
  ...
  /*
   * Probe tag geometry. [T5T-ISO15693] section 10.3.4 Get System Info is
   * mandatory on compliant VICCs (including ST25TV02KC); if a non-compliant
   * VICC does not answer, fall back to a conservative block size.
   */
  if (!nfc_frontend_iso15693_get_system_info(iso15693_transceive_glue,
                                             WRITER_FRONTEND, WRITER_APP_UID15,
                                             &nb_blocks, &blk_size)) {
  ...
  }
  {
    ...
    int existing_cc_len =
        iso15693_read(0u, existing_cc, NFC_STORAGE_TYPE5_BLOCK_SIZE);
    if (existing_cc_len >= (int)(NFC_TAG_T5T_CC_LEN_SHORT)) {
      nfc_tag_type5_info_t cc_info;
      const bool ndef_magic = (existing_cc[0] == NFC_FORUM_CC_MAGIC) ||
                              (existing_cc[0] == NFC_T5T_CC_MAGIC_8BYTE);
      ...
      if (ndef_magic &&
        ...
        int cc_tail_len =
            iso15693_read(1u, existing_cc + NFC_STORAGE_TYPE5_BLOCK_SIZE,
      ...
      if (cc_info.cc_valid && !cc_info.write_access_open) {
        nero_nfc_log_line(
            "  ERROR: Type 5 tag reports write access restricted");
        return false;
      }
      if (cc_info.cc_valid) {
        cc_len = (uint16_t)(existing_cc_len);
        first_data_block = (uint16_t)(cc_len / NFC_STORAGE_TYPE5_BLOCK_SIZE);
        if (!nero_nfc_copy_bytes(cc_block, sizeof(cc_block), 0u, existing_cc,
                                 cc_len)) {
          return false;
        }
      }
   ...
   */
  mlen = (uint8_t)((total + NFC_TAG_T5T_AREA_SIZE_UNIT_BYTES - 1u) /
                   NFC_TAG_T5T_AREA_SIZE_UNIT_BYTES);
  mlen = NERO_NFC_MAX(mlen, WRITER_TYPE5_MIN_MLEN_UNITS);
  if (nb_blocks > first_data_block) {
    uint16_t avail_bytes = (uint16_t)((nb_blocks - first_data_block) *
                                      NFC_STORAGE_TYPE5_BLOCK_SIZE);
    uint16_t mlen_max =
        (uint16_t)(avail_bytes / NFC_TAG_T5T_AREA_SIZE_UNIT_BYTES);
    mlen_max = NERO_NFC_MIN(mlen_max, UINT8_MAX);
    if (mlen > mlen_max) {
      mlen = (uint8_t)(mlen_max);
    }
  }
  if ((cc_len == (uint16_t)(NFC_TAG_T5T_CC_LEN_EXTENDED)) ||
      (((cc_block[0] == NFC_FORUM_CC_MAGIC) ||
        (cc_block[0] == NFC_T5T_CC_MAGIC_8BYTE)) &&
       (cc_block[NFC_TAG_T5T_CC_MLEN_BYTE_INDEX] >= mlen))) {
    write_cc = false;
  } else {
    if ((cc_block[0] != NFC_FORUM_CC_MAGIC) &&
        (cc_block[0] != NFC_T5T_CC_MAGIC_8BYTE)) {
      if (!nero_nfc_copy_bytes(cc_block, sizeof(cc_block), 0u,
                               WRITER_TYPE5_SHORT_CC_TEMPLATE,
                               sizeof(WRITER_TYPE5_SHORT_CC_TEMPLATE))) {
        return false;
      }
    }
    cc_block[NFC_TAG_T5T_CC_MLEN_BYTE_INDEX] = mlen;
  ...
  if (!nfc_storage_ceil_units_u16(total, NFC_STORAGE_TYPE5_BLOCK_SIZE,
                                  &blocks_needed) ||
      (blocks_needed == 0u)) {
  ...
  if (nb_blocks != 0u) {
    ...
    const uint32_t data_area_raw =
        (nb_blocks > first_data_block)
            ? ((uint32_t)(nb_blocks - first_data_block) *
               NFC_STORAGE_TYPE5_BLOCK_SIZE)
            : 0u;
    if (data_area_raw > UINT16_MAX) {
      nero_nfc_log_line(
          "  ERROR: Type 5 geometry exceeds supported writer range");
      return false;
    }
    const uint16_t data_area_size = (uint16_t)(data_area_raw);
    if (!nfc_storage_type5_data_blocks(cc_len, data_area_size, blk_size,
                                       nb_blocks, &first_data_block,
                                       &last_data_block) ||
        (((uint32_t)(first_data_block) + (uint32_t)(blocks_needed)-1u) >
         last_data_block)) {
...

  {
    ...
    if (write_cc) {
      ...
      nfc_tutorial_t5t_write_cmd(nero_nfc_log_putc, NERO_NFC_NULL,
                                 (uint8_t)(first_data_block), tlv,
                                 NFC_STORAGE_TYPE5_BLOCK_SIZE);
    }
    nfc_tutorial_ndef_tlv(nero_nfc_log_putc, tlv, total);

    if (write_cc &&
        !iso15693_write(0u, cc_block, NFC_STORAGE_TYPE5_BLOCK_SIZE)) {
