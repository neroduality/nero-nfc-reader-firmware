...
bool reader_security_key_select_fido_for_host_webauthn(
    uint8_t* rsp_out, uint16_t rsp_cap, uint16_t* rsp_len_out,
  ...
  for (uint8_t step = 0u;
       step < (uint8_t)(NFC_CTAP_FIDO_WEBAUTHN_SELECT_STEP_COUNT); ++step) {
    ...
    if (step > 0u) {
      if (step_info.log_before_prep == NFC_CTAP_FIDO_SELECT_LOG_REOPEN) {
        nero_nfc_log_line(
            "[CCID-FIDO] FIDO SELECT(Le=00) failed; full reopen before retry");
      } else if (step_info.log_before_prep ==
                 NFC_CTAP_FIDO_SELECT_LOG_LAST_CHANCE) {
        nero_nfc_log_line(
            "[CCID-FIDO] FIDO SELECT(reopen + Le=00 + no-Le) failed; "
            "last-chance reopen");
      }
    ...
    if (reader_security_key_select_app_ex(
            ...
            variant.add_le_00, true, follow_get_response, rsp_out, rsp_cap,
            rsp_len_out)) {
      return true;
    }
  }
